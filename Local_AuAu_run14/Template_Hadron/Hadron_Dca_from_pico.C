...
#include "TRandom3.h"
#include "TNtuple.h"
//#include "Binning_MB_data_Diven.h"
#include "../Binning_MB.h"

const int mh2Nhist=3;
const int mh2Ndca=3;
Bool_t Draw_print_plots_flag=false;
...
TH1F *mh1data_HFT_hadron_dcaXY_old[Npt];
TH1F *mh1data_HFT_hadron_dcaZ_old[Npt];

TH2F *mh2data_HFT_hadron_dcaXY_old;
TH2F *mh2data_HFT_hadron_dcaZ_old;
...
int Hadron_Dca_from_pico(){
  ...
  TFile * inFile_data_old= new TFile("Hadron_Dca_Data_Diven_sim_pi.root","READ");
  TFile * File_data_hadron= new TFile("Dca_hadron.root","RECREATE");
  ...
  // mh2data_HFT_hadron_dcaXY_2=(TH2F *) inFile_data_pico->Get("KaonDcaXy_Pt_0");
  // mh2data_HFT_hadron_dcaZ_2=(TH2F *) inFile_data_pico->Get("KaonDcaZ_Pt_0");
   
  // mh2data_HFT_hadron_dcaXY->Add(mh2data_HFT_hadron_dcaXY_2);
  // mh2data_HFT_hadron_dcaZ->Add(mh2data_HFT_hadron_dcaZ_2);

  //return 0;   

  mh2data_HFT_hadron_dcaXY_old=(TH2F *) inFile_data_old->Get(Form("hadron_dcaXyVs_pT_data"));
  mh2data_HFT_hadron_dcaZ_old=(TH2F *) inFile_data_old->Get(Form("hadron_dcaZVs_pT_data"));
  ...
  for(Int_t ipt=0;ipt<Npt;ipt++)
    {
      ...
      //     cout<< mh1data_HFT_hadron_dcaXY[ipt] ->GetName()<<endl;
      mh1data_HFT_hadron_dcaXY[ipt]->Rebin(5);
      mh1data_HFT_hadron_dcaZ[ipt]->Rebin(5);
       
       
      // mh1data_HFT_hadron_dcaXY[ipt]->Scale(1,"width");
      // mh1data_HFT_hadron_dcaZ[ipt]->Scale(1,"width");
      ...
      // get The old dcaXY from data driven peakage from guanna

      mh1data_HFT_hadron_dcaXY_old[ipt]  = (TH1F *)  mh2data_HFT_hadron_dcaXY_old->ProjectionY(Form("mh1data_HFT_pi_dcaXyVsPt%i",ipt),Xbin_ptlow_data[ipt],Xbin_pthigh_data[ipt]);
      mh1data_HFT_hadron_dcaZ_old[ipt]  = (TH1F *) mh2data_HFT_hadron_dcaZ_old->ProjectionY(Form("mh1data_HFT_pi_dcaZVsPt%i",ipt),Xbin_ptlow_data[ipt],Xbin_pthigh_data[ipt]);
      // sim
       
      mh1data_HFT_hadron_dcaXY_old[ipt]->SetMarkerColor(4);
      mh1data_HFT_hadron_dcaXY_old[ipt]->SetMarkerStyle(24);
      mh1data_HFT_hadron_dcaXY_old[ipt]->SetLineColor(4);
      //      mh1data_HFT_hadron_dcaXY_old[ipt]->Scale(1,"width");
       
      mh1data_HFT_hadron_dcaZ_old[ipt]->SetMarkerColor(4);
      mh1data_HFT_hadron_dcaZ_old[ipt]->SetMarkerStyle(24);
      mh1data_HFT_hadron_dcaZ_old[ipt]->SetLineColor(4);
      //  mh1data_HFT_hadron_dcaZ_old[ipt]->Scale(1,"width");
  ...
  // TCanvas *c5=new TCanvas("c5","",1000,1200);
  // c5->Divide(3,2);
   
  TH1F *       mh1data_HFT_hadron_dcaXY_old_ratio[Npt];
  for(Int_t ipt=0;ipt<Npt;ipt++)
    {
      //     cout<< " "<<ipt<<endl;
      c3->cd(ipt+1);
      ...
      mh1data_HFT_hadron_dcaXY_old[ipt]->GetXaxis()->SetRangeUser(-0.1,0.1);
      mh1data_HFT_hadron_dcaZ_old[ipt]->GetXaxis()->SetRangeUser(-0.1,0.1);
