...
void Charm_spectra_D0_Dpm()
{
  ...
  TFile * inFile=  new TFile("cHadron.root","READ"); // we will take 0-80% D0 from this root file
  //TFile * inFile=  new TFile("D0.root","READ"); // we will take 0-80% D0 from this root file
  TFile * inFile_Lc=  new TFile("Lc_spectra/out/Lc_10_80.root","READ"); // not used
  ...
  TH2F *hh_D0=new TH2F("hh_D0","",100,0.1,20,100,0.5e-11,1);
  hh_D0->SetTitle("");
  ...
  for( int ibin=0;ibin<D0_levy_powerLaw->GetNbinsX();ibin++)
      ...
      else
	{
      ...
      //cout<<"levy is : "<<F_levy_D0_0_80_levy->Eval(D0_levy_powerLaw->GetBinCenter(ibin+1))<<" powerlaw is "<< pl_0_80->Eval(D0_levy_powerLaw->GetBinCenter(ibin+1))<<" error is "<< 0.5*abs(F_levy_D0_0_80_levy->Eval(D0_levy_powerLaw->GetBinCenter(ibin+1))-pl_0_80->Eval(D0_levy_powerLaw->GetBinCenter(ibin+1)))<<endl;
	}
