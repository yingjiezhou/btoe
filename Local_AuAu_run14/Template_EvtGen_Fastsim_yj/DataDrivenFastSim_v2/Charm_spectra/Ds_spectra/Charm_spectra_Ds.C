...
void Charm_spectra_Ds()
{
  ...
  TGraphErrors *Ds_0_10_Nesim_sts=(TGraphErrors *) inFile_Nesim->Get("corrected_yield_staterror_ptshifted_cen_0_10"); // from fuchuang, to fit
  ...
  for(int i=0;i<Ds_0_10_Nesim_sts->GetN();i++)
    {
      ...
      Ds_0_10_Nesim_sts->SetPointError(i,0,sqrt(y_err*y_err+y_err_sys*y_err_sys)*10); // 也就是说，开始的时候是系统误差和统计误差一起计算的，现在是充填值，两者误差加起来你
