...
void B_fit_Minuit(){
  ...
  TFile *input_data_DCAZ =new TFile("./Input/Data_20170925_DCAZ.root","READ");
  ...
  TFile *input_phe_hijing_DCAZ=new TFile("./Input/Hijing_Phe_20170925_DCAZ.root","READ");
...
void chi2(Int_t &npar,Double_t *gin,Double_t &func,Double_t *par,Int_t iflag){
  ...
  for(int k=0;k<mc1_charm->GetNbinsX();k++){
    ...
    //    continue;
    //   cout<< " bin\t"<< k<<"\t"<<  mc1_charm->GetBinCenter(k+1)<<"\n";
    // if(k<19&&12<k)
    //   continue;
