...
void B_fit_Minuit(){
  ...
  TFile *input_data_DCAXY =new TFile("./Input/Data_1_26.root","READ");
  TFile *input_data_DCAZ =new TFile("./Input/Data_20171030_DCAZ.root","READ");
  TFile *input_purity =new TFile("Input/purity_MB.root","READ");

  

  TFile *input_fastsim=new TFile("./Input/File_fastsim.root","READ");
  TFile *input_phe_hijing_DCAXY=new TFile("./Input/Hijing_Phe_1_26.root","READ");
  TFile *input_phe_hijing_DCAZ=new TFile("./Input/Hijing_Phe_20171030_DCAZ.root","READ");
  ...
  for(Int_t ihist=0;ihist<mh2Nhist;ihist++) {
      ...
      if(Fit_DCAXY)
	{
      ...
      if(Fit_DCAZ)
	{
	  sprintf(buf,"mh1rDca_hist%ipt%i_D",ihist+2,ipt);
	  mh1Charm_dca[ihist][ipt]=(TH1F *)  input_fastsim->Get(buf);
	  sprintf(buf,"mh1rDca_hist%ipt%i_B",ihist+2,ipt);
	  mh1Bottom_dca[ihist][ipt]=(TH1F *)  input_fastsim->Get(buf);
	}
  ...
  for(Int_t ihist=0;ihist<mh2Nhist;ihist++) {
      ...
      if(Fit_DCAXY)
	{
	  ...
	  mh1data_inclusive[ihist][ipt]=(TH1F *)  input_data_DCAXY->Get(buf);
	  //   sprintf(buf,"electron_data_Dca_Unlike_like%i%i_rebin",ihist,ipt);
	  sprintf(buf,"Corrected_Phe_data%i",ipt);
	  mh1data_phe[ihist][ipt]=(TH1F *)  input_data_DCAXY->Get(buf);
	}
      if(Fit_DCAZ)
	{
	  sprintf(buf,"electron_data_Dca_inclusive%i%i_rebin",ihist,ipt);
	  mh1data_inclusive[ihist][ipt]=(TH1F *)  input_data_DCAZ->Get(buf);
	  //   sprintf(buf,"electron_data_Dca_Unlike_like%i%i_rebin",ihist,ipt);
	  sprintf(buf,"Corrected_Phe_data%i",ipt);
	  mh1data_phe[ihist][ipt]=(TH1F *)  input_data_DCAZ->Get(buf);
	}
  ...
  for(Int_t ihist=0;ihist<mh2Nhist;ihist++) {
      ...
      if(Fit_DCAXY)
	{
	  ...
	  mh1Phe_dca[ihist][ipt]=(TH1F *)  input_phe_hijing_DCAXY->Get(buf);
	}

       if(Fit_DCAZ)
	{
	  sprintf(buf,"Corrected_hijing_single%i",ipt);
	  mh1Phe_dca[ihist][ipt]=(TH1F *)  input_phe_hijing_DCAZ->Get(buf);
	}
  ...
  TCanvas *c6=new TCanvas("c6","",2000,1800);
  TCanvas *c8=new TCanvas("c8","",1200,1000);
  ...
  for(Int_t ipt=pt_start;ipt<pt_end;ipt++) {
    ...
    gMinuit->mnparm(3,"frac 3",vstart[3],step[3],purity_kunsu[ipt]-0.03,purity_kunsu[ipt]+0.03,ierflg);
  ...
  c6->SaveAs(Form("Output/Fit_all_DCAXY_%i_DCAZ_%i.pdf",Fit_DCAXY,Fit_DCAZ));
  c8->SaveAs(Form("Output/Fit_all_Ratio_DCAXY_%i_DCAZ_%i.pdf",Fit_DCAXY,Fit_DCAZ));
