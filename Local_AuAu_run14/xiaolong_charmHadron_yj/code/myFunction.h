...
//__________________________________________________________________________________________________________________
double getMean(TF1* fun, double x_lw, double x_up, const int nbins = 10000) {
	double width = (x_up - x_lw)/nbins;
	double xsum=0, sum=0;
	for(int i=0; i<nbins; i++) {
		double x = x_lw + (i+1)*0.5*width;
		xsum += fun->Eval(x)*x*width;
		sum += fun->Eval(x)*width;
	}
	return sum!=0 ? xsum/sum : 0;
}
double getMean(TH1F* h1) {
	double xsum=0, sum=0;
	for(int i=0; i<h1->GetNbinsX(); i++) {
		double width = h1->GetBinWidth(i+1);
