...
TH1F *mh1data_HFT_inclusive_rebin[mh2Ndca][Ntrig][Npt];
...
char buf[1024];
//viod SetHistogram_TH1F(*);
//void SetHistogram_TH1F(TH1F *hist_1F, Double_t Xmin,Double_t Xmax,Double_t Ymin,Double_t Ymax,char* Title, char* XTitle,char* YTitle );
...
int Correct_hijing_and_data(){
  ...
  //TFile * inFile_simu_hijing= new TFile("../../run14/simulation/Fraction_Fit/Correct_Phe_to_inclusive/Input/hijing_1_26.root","READ"); // 


  
  //TFile * inFile_simu_hijing= new TFile("Input/hijing_1_12.root","READ");
  // TFile * inFile_simu_hijing= new TFile("Input/hijing_1_26.root","READ");

  //TFile * inFile_data_MB= new TFile("Input/hist_1_3.root","READ");
  //   TFile * inFile_data_MB= new TFile("Input/hist_noTof_1_25.root","READ");
  //    TFile * inFile_data_MB= new TFile("Input/hist_1_26with_Tof.root","READ"); // final
  TFile * inFile_data_MB= new TFile("Input/hist_MB_20170920.root","READ"); 
  //TFile * inFile_data_MB= new TFile("hist_mb_nsigma_05.root","READ");
  // TFile * inFile_data_MB= new TFile("Input/hist_12_28.root","READ");
  //  TFile * inFile_data_HT= new TFile("/Users/xiaozhibai/work/Analysis/run14/simulation/Fraction_Fit/Correct_Phe_to_inclusive/Input/hist_ht_1_16.root","READ");// final

     TFile * inFile_data_HT= new TFile("Input/hist_ht_20170921.root","READ");

  //TFile * inFile_data_HT= new TFile("Input/hist_ht_tight_nsig.root","READ");

  // TFile * inFile_data_HT= new TFile("/Users/xiaozhibai/work/Analysis/run14/simulation/Fraction_Fit/Correct_Phe_to_inclusive/Input/hist_ht_1_16.root","READ");
  
  // TFile * inFile_data_phe_relative= new TFile("Input/Phe_relative_combine.root","READ");
  ...
  for(Int_t idca=0;idca<mh2Ndca;idca++)
    {
      mh2Hijing_pi0_e_Same_MC[idca]=(TH2F *) inFile_simu_hijing->Get(mh2Name_pi0_e_same_MC[idca]);
      ...
      mh2Hijing_pi0_e[idca]=(TH2F *) inFile_simu_hijing->Get(mh2Name_pi0_e[idca]);
  ...
  for(Int_t idca=0;idca<mh2Ndca;idca++)
      ...
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
	  ...
	  mh1Hijing_gamma_e_Pt[idca][ipt]->SetLineColor(2);
	  ...
	  if(idca==1)
	    mh1Hijing_pi0_e_Pt[idca][ipt]->GetXaxis()->SetTitle("DCA_{Z} (cm)");
	  ...
	  mh1Hijing_pi0_e_Pt_same_MC[idca][ipt]->SetTitle(mh1Title[ipt]);
  ...
  for(Int_t idca=0;idca<mh2Ndca;idca++)
      ...
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
      ...
      c3->SaveAs(Form("Plots/Data_single_%i_dca.pdf",idca));
      c4->SaveAs(Form("Plots/Data_phe_gamma_%i_dca.pdf",idca));
    }
  // section 2, rebin
  for(Int_t idca=0;idca<mh2Ndca;idca++)
    {
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
	  ...
	  if(idca==1)
	    mh1Hijing_pi0_e_Pt_rebin[idca][ipt]->GetXaxis()->SetTitle("DCA_{Z} (cm)");   
  ...
  TH1F * mh1Hijing_gamma_e_Pt_unlike_like_rebin_ratio[mh2Ndca][Npt];
  TH1F * mh1Hijing_gamma_e_Pt_same_MC_rebin_ratio[mh2Ndca][Npt];

  TH1F * mh1Hijing_pi0_e_Pt_unlike_like_rebin_ratio[mh2Ndca][Npt];
  TH1F * mh1Hijing_pi0_e_Pt_same_MC_rebin_ratio[mh2Ndca][Npt];
  
  TH1F * mh1Hijing_eta_e_Pt_unlike_like_rebin_ratio[mh2Ndca][Npt];
  ...
  // TCanvas *c42=new TCanvas("c42","",1200,1400);
  // c42->Divide(4,2);

  TCanvas *c52=new TCanvas("c52","",1200,1400);
  c52->Divide(4,2);

  // TCanvas *c62=new TCanvas("c62","",1200,1400);
  // c62->Divide(4,2);
  ...
  for(Int_t idca=0;idca<mh2Ndca;idca++)
      ...
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
      ...
      //  c42->SaveAs("Plots/Data_DcaXY_phe_gamma_rebin.pdf");
      c52->SaveAs(Form("Plots/Data_phe_pi0_rebin_%i_dca.pdf",idca));
      // c62->SaveAs("Plots/Data_DcaXY_phe_eta_rebin.pdf");
      ...
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
	  ...
	  //  mh1Hijing_gamma_e_Pt_unlike_like_rebin_ratio[idca][ipt]->GetYaxis()->SetRangeUser(0,1.4);

	  mh1Hijing_gamma_e_Pt_same_MC_rebin_ratio[idca][ipt]->GetXaxis()->SetTitle("DCA_{XY} (cm)");
	  if(idca==1)
	    mh1Hijing_gamma_e_Pt_same_MC_rebin_ratio[idca][ipt]->GetXaxis()->SetTitle("DCA_{Z} (cm)");
      ...
      // TCanvas *c33=new TCanvas("c33","",1200,1400);
      // c33->Divide(4,2);
      ...
      // TCanvas *c53=new TCanvas("c53","",1200,1400);
      // c53->Divide(3,2);
  ...
  TH1F* Single_Electron_hijing[mh2Ndca][Npt];
  TH1F* Phe_Electron_hijing[mh2Ndca][Npt];
  TH1F* Phe_Single_Electron_hijing[mh2Ndca][Npt];
  ...
  for(Int_t idca=0;idca<mh2Ndca;idca++)
      ...
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
	  ...
	  Single_Electron_hijing[idca][ipt]->Add(mh1Hijing_gamma_e_Pt_rebin[idca][ipt],pT_spectra_gamma_ratio_rebin_1->GetBinContent(ipt+1));
	  ...
	  Phe_Single_Electron_hijing[idca][ipt]->Add(Phe_Electron_hijing[idca][ipt]);
	  ...
	  Single_Electron_hijing[idca][ipt]->GetYaxis()->SetRangeUser(0.1*Phe_Electron_hijing[idca][ipt]->GetMinimum(),10*Single_Electron_hijing[idca][ipt]->GetMaximum());
	  ...
	  if(idca==1)
	    Phe_Single_Electron_hijing[idca][ipt]->GetXaxis()->SetTitle("DCA_{Z} (cm)");
  ...
  for(int idca=0;idca<mh2Ndca;idca++)
    {
      for(Int_t itrg=0;itrg<Ntrig-1;itrg++)
	{
	  for(Int_t ipt=0;ipt<Npt;ipt++)
	    {
	      ...
	      // mh1data_HFT_like[idca][itrg][ipt]->Sumw2();
	      // mh1data_HFT_unlike_like[idca][itrg][ipt]->Sumw2(); 
  ...
  // TCanvas *c65=new TCanvas("c65","",1200,1400);
  // c65->Divide(4,2);
  ...
  for(int idca=0;idca<mh2Ndca;idca++)
      ...
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
	  ...
	  // mh1data_HFT_unlike_like_rebin[2][ipt]->Draw("samePE");
  ...
  // TCanvas *c66=new TCanvas("c66","",1200,1400);
  // c66->Divide(4,2);

  // TCanvas *c76=new TCanvas("c76","",1200,1400);
  // c76->Divide(4,2);
  ...
  for(int idca=0;idca<mh2Ndca;idca++)
      ...
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
	  ...
	  Phe_Electron_hijing[idca][ipt]->Scale(1./Phe_Electron_hijing[idca][ipt]->Integral());
	  ...
	  Hijing_Single_electron[idca][ipt]->Add(Single_Electron_hijing[idca][ipt]);
	  Hijing_phe_electron[idca][ipt]->Add(Phe_Electron_hijing[idca][ipt],1); 

	  Hijing_Single_phe_electron_ratio[idca][ipt]->Add(Single_Electron_hijing[idca][ipt]);
	  Hijing_Single_phe_electron_ratio[idca][ipt]->Divide(Phe_Electron_hijing[idca][ipt]);
      
	  Hijing_phe_electron_flip_ratio[idca][ipt]->Add(mh1data_HFT_unlike_like_rebin[idca][Trg][ipt]);
	  Hijing_phe_electron_ratio[idca][ipt]->Add(mh1data_HFT_unlike_like_rebin[idca][Trg][ipt]);

	  Hijing_phe_electron_flip_ratio[idca][ipt]->Divide(Hijing_phe_electron_flip[idca][ipt]);
	  Hijing_phe_electron_ratio[idca][ipt]->Divide(Hijing_phe_electron[idca][ipt]);


	  Corrected_hijing_single[idca][ipt]->Add(Hijing_Single_electron_flip[idca][ipt]);
	  Corrected_Phe_data[idca][ipt]->Add(mh1data_HFT_unlike_like_rebin[idca][Trg][ipt]);

	  if(ipt<=4)
	    {
	      Corrected_hijing_single[idca][ipt]->Multiply(Hijing_phe_electron_flip_ratio[idca][ipt]);
	    }
	  else
	    {
	      Corrected_hijing_single[idca][ipt]->Multiply(Hijing_phe_electron_flip_ratio[idca][4]);
	    }
  ...
  for(Int_t idca=0;idca<mh2Ndca;idca++)
      ...
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
	  ...
	  if(idca==1)
	    {
	      Hijing_phe_electron_flip_ratio[idca][ipt]->GetXaxis()->SetTitle("DCA_{Z} cm");
	    }
	  ...
	  if(idca==1)
	    {
	      Hijing_Single_phe_electron_ratio[idca][ipt]->GetXaxis()->SetTitle("DCA_{Z} cm");
	    }
      ...
      for(Int_t ipt=0;ipt<Npt;ipt++)
	{
	  ...
	  Corrected_hijing_single[idca][ipt]->Write();
...
for(Int_t ipt=0;ipt<Npt;ipt++)
      {
        ...
        outFile_data->cd();      
        mh1data_HFT_inclusive_rebin[idca][Trg][ipt]->Write();
        mh1data_HFT_unlike_like_rebin[idca][Trg][ipt]->Write();
        Corrected_Phe_data[idca][ipt]->Write();
