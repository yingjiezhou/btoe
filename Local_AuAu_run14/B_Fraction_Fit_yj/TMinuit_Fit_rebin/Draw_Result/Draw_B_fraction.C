...

{
  ...
  TFile *input_run14_DCAXY_new =new TFile("../Output/Fit_B_fraction_DCA_0.root","READ"); // zyj new, only stat??
  TFile *input_run14_DCAZ_new =new TFile("../Output/Fit_B_fraction_DCA_1.root","READ");
	...
	B_fraction_MB->SetBinContent(ipt+1,B_fraction_sys->GetBinContent(ipt+1));// zyj make bin content sys=sts!!! diff is because sys if from Fit_B_fraction_MB_default_2.root, not Fit_B_fraction_MB.root, why? sys is from different method, can we?
  ...
  TGraphErrors *  gr_B_fraction_dcaxy_new=new TGraphErrors(B_fraction_MB_dcaxy); // TH1F->TGraphErrors, why?
  ...
  B_fraction_sys->Write();
    ...
    for(Int_t ipoint=0;ipoint<gr_B_fraction_MB->GetN();ipoint++)
      {
	gr_B_fraction_MB->SetPointError(ipoint,gr_B_fraction_MB->GetErrorX(ipoint), gr_B_fraction_MB->GetErrorY(ipoint)); // zyj no need, right?
    ...
    for(Int_t ipoint=0;ipoint<run12_pp_sys->GetN();ipoint++)
      {
	run12_pp_sys->SetPointError(ipoint,0.1,0.1, run12_pp_sys->GetErrorYlow(ipoint),run12_pp_sys->GetErrorYhigh(ipoint));// x error should be same, no need to show again, here set to be 0.1  zyj
...
//  Gp->SetPallet();
    ...
    // not used D0 here zyj
    TGraphErrors *Ge_D0 = new TGraphErrors("InPut/e_D0.dat"); // not used here, zyj
  ...
  return ; //zyj
    ...
    legend ->AddEntry(gr_B_fraction_dcaxy_new,"Included Ds,Lc","P");// only stat error
    legend ->AddEntry(gr_B_fraction_dcaz_new,"Fit DCA_{Z}","P");// only stat error
