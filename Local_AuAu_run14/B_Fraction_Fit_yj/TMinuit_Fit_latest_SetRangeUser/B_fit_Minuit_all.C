...
const int IHIST=0; // zyj 0 means we only use DCAxy
...
void B_fit_Minuit_all(){
  ...
  //double purity_kunsu[Npt] =  {0.1, 0.13, 0.22, 0.14, 0.21, 0.25, 0.3, 0.43}; // zyj, used for TMunit fit
  //======zyj=======
  //double purity_kunsu[Npt] =  {0.1, 0.13, 0.04, 0.07, 0.11, 0.13, 0.16, 0.24};// zyj 1-purity
  double purity_kunsu[Npt] =  {0.06, 0.1, 0.04, 0.07, 0.11, 0.13, 0.16, 0.24};// zyj 1-purity
  ...
  //======zyj=======
  TString outFileName="Output_tunePtBin_yj_allChange_qa_xzbin_noinc";
  TFile *Output_tunePtBin_xz_file=new TFile(Form("%s/Fit_B_fraction_DCA_%i.root",outFileName.Data(),IHIST),"RECREATE");
  //======zyj=======
  TFile *input_purity;
  //TFile *input_fastsim=new TFile("./Input/File_fastsim.root","READ");
  //======zyj=======
  //TFile *input_fastsim=new TFile("./Input/File_fastsim_xz_rebin.root","READ");// I did, need add _zyj later
  //TFile *input_fastsim=new TFile("./Input/File_fastsim_yj.root","READ");// I did, need add _zyj later
  TFile *input_fastsim=new TFile("./Input/File_fastsim_yj_xzbin.root","READ");// I did, need add _zyj later
  //======zyj=======
  TFile *input_phe_hijing=new TFile("./Input/Photonic_electron_template.root","READ");
  //TFile *input_phe_hijing=new TFile("./Input/Photonic_electron_template_yj.root","READ");
  //======zyj=======
  //TFile *input_hadron_data=new TFile("./Input/Dca_hadron.root","READ");
  //TFile *input_hadron_data=new TFile("./Input/Dca_hadron_misIDH.root","READ");
  //TFile *input_hadron_data=new TFile("./Input/Dca_hadron_allH.root","READ");
  //TFile *input_hadron_data=new TFile("./Input/Dca_hadron_allH_moreStat_v2.root","READ");
  TFile *input_hadron_data=new TFile("./Input/Dca_hadron_qa_HFT_more_xzbin.root","READ");
  
  //======zyj=======
  TFile *input_data=new TFile("Input/Data_inclusive_photonic.root","READ");
  //TFile *input_data=new TFile("Input/Data_inclusive_photonic_yj.root","READ");
  ...
  //=== zyj add===
  /*
   TCanvas *cyj=new TCanvas("cyj","",900,1000);
   cyj->Divide(4,2);
   */
  //=== zyj add===
  ...
  for(Int_t ipt=pt_start;ipt<pt_end;ipt++) {
    ...
    // Output_tunePtBin_xz_file_kunsu->cd();
    // mc1_charm->Write();
    // mc2_bottom->Write();
    // mc3_phe->Write();
    // mc4_hadron->Write();
    ...
    /*
     ...
     mc3_phe->Draw();
     mc1_charm->Draw("same");
     mc2_bottom->Draw("same");
     mc4_hadron->Draw("same");
     */
    //=== zyj add===
    ...
    gMinuit->mnparm(0,"frac 0",1,step[0],0,5*data->GetMaximum(),ierflg);// zyj why is 1, than ??
    gMinuit->mnparm(1,"frac 1",vstart[1],step[1],0.000,1.,ierflg);
    gMinuit->mnparm(2,"frac 2",vstart[2],step[2],0.000,1,ierflg);
    //gMinuit->mnparm(3,"frac 3",vstart[3],step[3],0.000,1,ierflg); // zyj not useful???
    gMinuit->mnparm(3,"frac 3",vstart[3],step[3],purity_kunsu[ipt]-0.1,purity_kunsu[ipt]+0.35,ierflg); // default
    ...
    cout<<"======================================zyj TMunit fit output =========================================="<<endl;
    ...
    //====zyj====
    //mc2_bottom is the normalized bottom spectrum, scale factor to get back
    //====zyj====
    ...
    c5->SaveAs(Form("%s/B_Fit_%i_%i_pT_Fit_and_ratio.pdf",outFileName.Data(), IHIST,ipt));
  ...
  //===zyj===
  //cyj->SaveAs("Template_compare.pdf");
  //return ;
  c6->SaveAs(Form("%s/Fit_allpT_%i.pdf",outFileName.Data(), IHIST));
  c8->SaveAs(Form("%s/Fit_allpT_Ratio_%i.pdf",outFileName.Data(), IHIST));
  ...
  c4->SaveAs(Form("%s/Fraction_%i.pdf",outFileName.Data(), IHIST));
  ...
  c7->SaveAs(Form("%s/Npe_Phe_ratio_%i.pdf",outFileName.Data(), IHIST));
  
  Output_tunePtBin_xz_file->cd();
  b_fraction_MB->Write();
  ...
  Output_tunePtBin_xz_file->Close();
