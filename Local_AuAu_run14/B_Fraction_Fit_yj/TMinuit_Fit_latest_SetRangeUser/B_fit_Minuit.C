...
void B_fit_Minuit(){
  ...
  //===zyj===
  TFile *input_data_yj=new TFile("./Input/Data_1_26.root","READ"); // = xz
  ...
  for(Int_t ihist=0;ihist<Ndca;ihist++) {
    for(Int_t ipt=0;ipt<Npt;ipt++) {
      ...
      cout<<buf<<endl;
      mh1data_inclusive[ihist][ipt]=(TH1F *)input_data_yj->Get(buf);
      //====zyj=====
  ...
  for(Int_t ipt=pt_start;ipt<pt_end;ipt++) {
    ...
    data->GetYaxis()->SetRangeUser(Dca_Min[ipt],Dca_Max[ipt]); // zyj setrange
    data->Draw("PE");
