...
void B_fit_Minuit(){
  ...
  TFile *Output_file=new TFile("OutPut_xzlatestfastSim_used/Fit_B_fraction_MB.root","RECREATE");
  TFile *input_data;
  TFile *input_purity;
  ...
  TFile *input_fastsim=new TFile("./Input/File_fastsim_xz_latest.root","READ");
  //===zyj====
  ...
  TFile *Output_file_kunsu=new TFile("OutPut_xzlatestfastSim_used/For_Kunsu.root","RECREATE");
  ...
  if(MB_trg){
    sprintf(Trg,"MB");
    input_data=new TFile("./Input/Data_1_26.root","READ"); // = xz
    //     Output_file
  ...
  if(HT_trg)
  {
    ...
    Output_file=new TFile("OutPut_xzlatestfastSim_used/Fit_B_fraction_HT.root","RECREATE");
    pt_start=3;
    pt_end=6;
  ...
  for(Int_t ihist=0;ihist<mh2Nhist;ihist++) {
    for(Int_t ipt=0;ipt<Npt;ipt++) {
      ...
      cout<<"zyj jjjjjjjjjjjjjjjjjjjj"<<buf<<endl;
  ...
  for(Int_t ipt=pt_start;ipt<pt_end;ipt++) {
    ...
    //Fit_Ratio[ipt]= (TH1F * ) data->Clone(Form("Fit_Ratio")); // zyj commentted
    data->GetXaxis()->SetTitle("DCA_{XY} (cm)");
    ...
    pad1->cd();
    //====zyj======
    //cout<<"here is waht I want ::::"<<data->GetMinimum()<<" and is the "<<data->GetMaximum()<<"jjjjjjjjj"<<endl;
    //data->GetYaxis()->SetRangeUser(0.5*data->GetMinimum(), 10*data->GetMaximum());// not used
    //====zyj======
    ...
    c5->SaveAs(Form("OutPut_xzlatestfastSim_used/B_Fit_%i_pT_Fit_and_ratio.pdf",ipt));
  ...
  c4->SaveAs("OutPut_xzlatestfastSim_used/Fraction.pdf");
  ...
  c7->SaveAs("OutPut_xzlatestfastSim_used/Npe_Phe_ratio.pdf");
