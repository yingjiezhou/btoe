...
void B_fit_Minuit(){
  ...
  TFile *input_fastsim=new TFile("./Input/File_fastsim_11_3.root","READ");
  TFile *input_phe_hijing=new TFile("./Input/Hijing_phe_gamma_pi0_pi_16d_a.root","READ");
  TFile *input_hadron_data=new TFile("./Input/Hadron_Dca_11_3.root","READ");

  //   TFile *input_phe_hijing=new TFile("Input/Hijing_phe_gamma_pi0_pi_15c.root","READ");
  if(MB_trg)
    {
      sprintf(Trg,"MB");
      input_data=new TFile("./Input/Dca_inclusive_photonic_MB_11_3.root","READ");
  ...
  //  for(Int_t ipt=0;ipt<1;ipt++) {
   for(Int_t ipt=pt_start;ipt<pt_end;ipt++) {
    ...
    //phe from hijing
    //mc2=(TH1F*)mh1Phe_dca[IHIST][ipt]->Clone(mcname2+mh1Phe_dca[IHIST][ipt]->GetName());
    // phe from data
    ...
    //starting values
    double vstart[4]={100000,0.1,0.1,0.01}; //frac
    double step[4]={1,0.1,0.1,0.01}; //starting step
    gMinuit->mnparm(0,"frac 0",vstart[0],step[0],1e1,1e10,ierflg);
    ...
    TString *str3 = new TString("Phe frac");
    ...
    gMinuit->mnpout(3,*str3,frac_phe,fracErr_phe,dum1,dum2,ierflg);
    ...
    TH1F *mc1_cl = (TH1F*)mc2_bottom->Clone("mc1_cl");
    mc1_cl->Scale(frac_btoe*frac_norm);
    mc1_cl->SetLineColor(6);

    TH1F *mc2_cl = (TH1F*)mc3_phe->Clone("mc2_cl");
    mc2_cl->Scale(frac_phe*frac_norm);
    mc2_cl->SetLineColor(8);
    
    TH1F *mc3_cl = (TH1F*)mc4_hadron->Clone("mc3_cl");
    mc3_cl->Scale((1-frac_ctoe-frac_btoe-frac_phe)*frac_norm);
    mc3_cl->SetLineColor(4);

    
    TH1F *mc_all_cl = (TH1F*)mc1_charm_cl->Clone("mc_all_cl");
    mc_all_cl->Add(mc1_cl);
    mc_all_cl->Add(mc2_cl);
    mc_all_cl->Add(mc3_cl);

    mc_all_cl->SetLineColor(1);

    mc1_charm_cl->DrawClone("histsame");
    mc1_cl->DrawClone("histsame");
    mc2_cl->DrawClone("histsame");
    mc3_cl->DrawClone("histsame");
    ...
    sprintf(buf,"F(PHE)^{Fit}=%4.3f#pm%4.3f",frac_phe,fracErr_btoe);
    drawLatex(0.13,0.7,buf,62,0.04,2);

    sprintf(buf,"F(Hadron)^{Fit}=%4.3f",1-frac_ctoe-frac_btoe-frac_phe);
    ...
    legend->AddEntry(mc1_cl,"B #rightarrow e","l");
    legend->AddEntry(mc2_cl,"PHE","l");
    legend->AddEntry(mc3_cl,"Hadron","l");
...
void chi2(Int_t &npar,Double_t *gin,Double_t &func,Double_t *par,Int_t iflag){
  ...
  for(int k=0;k<mc1_charm->GetNbinsX();k++){
    double y0 = mc1_charm->GetBinContent(k+1);
    double y1 = mc2_bottom->GetBinContent(k+1);
    double y2 = mc3_phe->GetBinContent(k+1);
    double y3 = mc4_hadron->GetBinContent(k+1);
    ...
    double ycomb = par[0]*(par[1]*y0+par[2]*y1+ par[3]*y2 + (1-par[1]-par[2]-par[3])*y3); //
       //    double ycomb = par[0]*(par[1]*y0+par[2]*y1+ par[3]*y2 + 0.2*y3); //
    
    double ey0 = par[0]*par[1]*mc1_charm->GetBinError(k+1);
    double ey1 = par[0]*par[2]*mc2_bottom->GetBinError(k+1);
    double ey2 = par[0]*(1-par[1]-par[2]-par[3])*mc3_phe->GetBinError(k+1);
    double ey3 = par[0]*par[3]*mc4_hadron->GetBinError(k+1);
    double eTemp=sqrt(ey0*ey0+ey1*ey1+ey2*ey2+ey3*ey3);
