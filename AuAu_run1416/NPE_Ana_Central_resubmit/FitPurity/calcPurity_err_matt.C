...
void calcPurity_err_matt(int rs=1,int save=0, int doAllFits=0)
{
  /////////////////////////////////////////////////////////////////////////////////////////
  ...
  double purity_matt_mva[9]={0, 0, 0, 0.22, 0.176, 0.128, 0.123, 0.059, 0.};
  ...
  hpur2->GetYaxis()->SetRangeUser(0,1.);
  hpur2->GetYaxis()->SetTitle("Hadron Fraction");
  hpur2->GetXaxis()->SetTitle("#it{p}_{T}  [GeV/#it{c}]");
  hpur2->SetMarkerStyle(25);
  hpur22->SetMarkerStyle(25);
  hpur3->SetMarkerStyle(25);
  ...
  hpur22->SetMarkerColor(kRed);
  hpur33->SetMarkerColor(kRed);
  hpur22->SetLineColor(kRed);
  hpur33->SetLineColor(kRed);
  hpur3->SetLineColor(kBlue);
  ...
  TLegend *leg = new TLegend(0.60,0.64,0.9,0.9);
  ...
  leg->Draw("same");
  c1->SaveAs("hadron_fraction_matt.pdf");
  return ;
}
