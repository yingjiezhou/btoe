               ...
               int pt,int eta, int rs,float cutLow,float cutHigh,double ptLow ,double ptHigh)
{
  ...
  if( pt<4 || pt==7) g2 = new RooAddPdf("g2","g2",g2_1,g2_2,frac2); //0814 pion
  ...
  RooRealVar sigma55("sigma55","sigma55",1,1,5.);
  ...
  if(pt==2) PID->setRange("range_pi",-8,-1);
  if(pt==3) PID->setRange("range_pi",-8,2);
  ...
  //if(pt==9) PID->setRange("range_pi",-7,3);// zyj (-5,3) , pt ==7
  ...
  if(pt==3)PID->setRange("range_2pi",1,7);
  if(pt==4)PID->setRange("range_2pi",0,4.);
  ...
  */
  ...
  // zyj to calculate error
  /*
  TH1F *yd[6];
  for(auto i=0; i<6; i++){
    yd[i]= new TH1F(Form("yield%i", i),Form("yield%i", i), 1, 0.5, 1.5);
    yd[i]->Sumw2();
    yd[i]->SetBinContent(yields[i]);
    yd[i]->SetBinError(yields_errors[i]);
  }
   */
