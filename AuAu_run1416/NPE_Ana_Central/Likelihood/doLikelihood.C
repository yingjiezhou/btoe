...
void doLikelihood(){
  ...
  TFile *file_e = new TFile("./root/weights_electron_highPt_B10.root"); // matt
  TFile *file_p = new TFile("./root/weights_pion_highPt_B10_mix.root");//matt 
  //TFile *file_e = new TFile("../../NPE_Ana_yj/Likelihood/root_yj/weights_electron_highPt_B10.root");
  //TFile *file_p = new TFile("../../NPE_Ana_yj/Likelihood/root_yj/weights_pion_highPt_B10.root");//smooth
  ...
  //sprintf(dFile1,"../../NPE_Ana_yj/SPlot/Kshort/root_yj/out.root"); // matt
  sprintf(dFile1,"../SPlot/Kshort/root/out.root"); // zyj
  ...
  for(int i=0; i<ch1->GetEntries();i++){
    ...
    if(!(nsige>-1 && nsige<3))continue; // zyj 0814, I think should be include 
    ...
    if(1){//nsige>-1 && nsige<3){
      if(probe_charge*tag_charge<0){
        if(p/emc0>0.3 && p/emc0<1.5){
          if(dll>0.45)  hFit_e_3->Fill(pt,nsige); //  PHE number after mva cut, because we are in the PhoEtree loop
        }
  ...
  TFile file("./root_yj/Ks_pion_yj_nn.root","RECREATE");
  hFit_e_3->Add(hFit_e_WS_3,-1); // zyj add dll>0.45 cut, will not be used
  hFit_e_3->Write();
  hFit_pi_3->Write();
  hFit_pi_4->Write();
  hFit_pi_5->Write();
  ...
  //leg->Draw("same");
  canv2->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Elec_pion_Probability.pdf");
  //========================================================================
  ...
  line->Draw("same hist");
  leg->Draw("same");
  canv4->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Cut_Effi.pdf");
  //========================================================================
  ...
  ROC->Draw("P");
  cpp->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/ROC.pdf");
  //========================================================================

  TCanvas *canvsig = new TCanvas ("canvsig","significance");
  sign->GetXaxis()->SetTitle("Likelihood Cut");
  sign->GetXaxis()->SetRangeUser(-1, 0.5);
  sign->GetYaxis()->SetTitle("Significance");
  sign->Draw("P");
  TLine *linee = new TLine(-1,1,0.5,1);
  linee->SetLineColor(kRed);
  linee->SetLineStyle(7);
  linee->Draw("same");
  
  canvsig->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Signi.pdf");
  //========================================================================

  DLL_p->GetXaxis()->SetTitle("Likelihood Response"); // zyj e->p
  DLL_p->GetYaxis()->SetTitle("A. U.");
  DLL_p->SetLineColor(kRed);
  ...
  DLL_p_post->GetYaxis()->SetRangeUser(0, 0.33);
  DLL_p_post->GetXaxis()->SetTitle("Likelihood Response"); //zyj  e -> p
  DLL_p_post->GetYaxis()->SetTitle("A. U.");
  ...
  leg2->Draw("same");
  canv1->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Response_Comparision.pdf");
  canv1->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Response_Comparision.C");
  //========================================================================
  ...
  //0819
  char ptbin[50];
  sprintf(ptbin,"#it{p}_{T} #in [%1.1f,%1.1f] GeV/c",0.6, 8.5);
  txt->AddText(ptbin);
  //cp->addObject(txt);
  DLL_p_post->Draw();
  txt->Draw("same");
  DLL_e_post->Draw("same"); // with DLL_e_post cut
  leg2->Draw("same");
  cp->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Response_Comparision_post.pdf");
  cp->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Response_Comparision_post.C");
  ...
  for(int i = 1;i<numPtBins-1;i++){ // zyj eff for different pt bin
    ...
    if(htemp1->Integral()>0)pt_eff_e->SetBinContent(i,htemp1->Integral(bin1,max)/htemp1->Integral()); // likelihood eff as a func of pt
    ...
    if(htemp2->Integral()>0)pt_eff_p->SetBinContent(i,htemp2->Integral(bin1,max)/htemp2->Integral());
    ...
    if(htemp1->Integral()>0)pt_eff_e1->SetBinContent(i,htemp11->Integral()/htemp1->Integral());
    pt_eff_e1->SetBinError(i,error2);
    
    if(htemp1->Integral()>0)pt_eff_e2->SetBinContent(i,htemp11->Integral(bin1,max)/htemp1->Integral()); //bin1 = htemp1->FindBin(0.2);
    ...
    if(htemp1->Integral()>0)pt_eff_e3->SetBinContent(i,htemp1->Integral(bin1,max)/htemp1->Integral());
    ...
    if(htemp1->Integral(bin2,max)>0)pt_eff_r->SetBinContent(i,htemp1->Integral(bin1,max)/htemp1->Integral(bin2,max)); //    int bin2 = htemp1->FindBin(0.4); // ??? shoule be 0??

    pt_eff_r->SetBinError(i,0.01);
    
    bin1 = htemp1->FindBin(0.5);
    if(htemp1->Integral()>0)pt_eff_e4->SetBinContent(i,htemp1->Integral(bin1,max)/htemp1->Integral());
    ...
    if(htemp1->Integral()>0)pt_eff_e5->SetBinContent(i,htemp1->Integral(bin1,max)/htemp1->Integral());
  ...
  TLegend *leg11 = new TLegend(0.7,0.2,0.88,0.4);
  ...
  line->Draw("same hist");
  leg11->Draw("same");
  lat.DrawLatex(2,0.4,"Likelihood Response>0.45");
  canv5->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Efficiency_pt.pdf");
  canv5->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Efficiency_pt.C");
  //=========================================================================

  /*
  TCanvas *canv51 = new TCanvas ("canv51","DLL Rat");
  pt_eff_r->Draw("PE");
  canv51->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/DLL_Rat.pdf");
  */
  //=========================================================================

  TCanvas *canv55 = new TCanvas ("canv55","e Likelihood Efficiency comp");
  pt_eff_e->GetYaxis()->SetTitle("Likelihood Efficiency");
  pt_eff_e->GetYaxis()->SetRangeUser(0,1.5);
  pt_eff_e->GetXaxis()->SetTitle("#it{p}_{T} [GeV]");
  pt_eff_e->Draw("PE");
  line->Draw("same hist");
  ...
  pt_eff_e1->SetMarkerStyle(24);
  pt_eff_e2->SetMarkerStyle(24);
  pt_eff_e->SetMarkerStyle(24);
  pt_eff_e3->SetMarkerStyle(24);
  pt_eff_e4->SetMarkerStyle(24);
  pt_eff_e5->SetMarkerStyle(24);
  ...
  leg1->AddEntry(pt_eff_e4,"#Delta#it{LL}(e-#pi)>0.5","LPE");
  leg1->AddEntry(pt_eff_e5,"#Delta#it{LL}(e-#pi)>0.6","LPE");
  ...
  leg1->Draw("same");
  canv55->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Efficiency_diff_MVACut.pdf");
  canv55->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Efficiency_diff_MVACut.C");
  //=========================================================================
  ...
  canv6->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/corr_electron.pdf");
  //=========================================================================
  ...
  hOverlap->Draw();
  canv9->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/overlap.pdf");
  //=========================================================================

  TCanvas *canv25 = new TCanvas ("canv52","DLL nsige effi");
  TLegend *leg12 = new TLegend(0.3,0.5,0.5,0.5);
  leg12->AddEntry(nsig_eff_e,"Electrons","PEL");
  leg12->AddEntry(nsig_eff_p,"Pions ","PEL");
  ...
  line->Draw("same hist");
  lat.DrawLatex(1,0.5,"Likelihood>0.45");
  leg12->Draw("same");
  canv25->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/Likelihood_Efficiency_Nsigma_e.pdf");
  //=========================================================================
  ...
  canv7->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/nsige_dll_dist.pdf");
  //=========================================================================
  ...
  canv77->cd(2);
  DLL_p_pt->Draw("COLZ");
  canv77->SaveAs("plots_likelihood_yj_B10_TuneCut_nn/pt_dll_dist.pdf");
