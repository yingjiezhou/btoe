...
void fitdata1D(TH1F *data,TH1F *data_e,TH1F* data_emis,TH1F *data_pi, TH1F *data_2pi,TH1F *data_k,TH1F *data_p,TH1F* data_D,TH1F* data_jpsi,TH1F* data_B,int daughter,int mag, int save,
	       ...
	       int pt,int eta,double ptLow ,double ptHigh,double eff_b, int pur_dir, int HT, double e_const, int sys)
{
...
//Signals
    ...
    //else 
    if(sys!=3)e_ratio.setRange(e_const-0.01,e_const+0.01);
    else e_ratio.setRange(e_const-0.05,e_const+0.05);
