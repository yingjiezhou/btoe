...
public:
  ...
  private:
    StPicoNpeAnaMaker() {}
    void readNextEvent();
    void fillHistogram(StPicoTrack*, TVector3, int, int, TString);
    void fillMyHistogram(StPicoTrack*, TVector3, bool, bool, bool, bool, bool, double);
    void fillPairHistogram(StPicoTrack*, TVector3,  bool, bool, bool, bool, int,double);
    void fillEventTree(float, float, float, float, float, float, float);
    void fillTree(StPicoTrack*, TVector3,  bool, bool, bool, bool,double,float, float, float, float, float, float, float);
    void fillEHTree(StPicoTrack*, StPicoTrack*,TVector3, bool, bool, bool, bool, float, float, float, float, float, float, float, int, double);

    void fillJPsiTree(StElectronPair *,StPicoTrack*,StPicoTrack*, TVector3,  bool, bool, bool, bool, int, int, double);
    void fillKShortTree(StElectronPair *,StPicoTrack*,StPicoTrack*, TVector3,  bool, bool, bool, bool, int, int, double);
    void fillPhiTree(StElectronPair *,StPicoTrack*,StPicoTrack*, TVector3,  bool, bool, bool, bool, int, int, double);
    void fillLambdaTree(StElectronPair *,StPicoTrack*,StPicoTrack*, TVector3,  bool, bool, bool, bool, int, int, double);
    void fillPhoETree(StElectronPair *,StPicoTrack*,StPicoTrack*, TVector3,  bool, bool, bool, bool, int, int, double,float,float,float,float,float,float,float);
    void fillBKGTree(StPicoTrack*, TVector3,  bool, bool, bool, bool, double,float, float, float, float, float, float, float);
    bool isGoodPair(StElectronPair const*) const;
    bool checkHadronDca(StPicoTrack*, TVector3, double, double); 
    bool isIso(StPicoTrack const *);
    void getFmsEventPlane();
    ...
    float mEP1;
    //ZDC related                                                                                                                                                                                  
    float ZDCSMD(int eastwest,int verthori,int strip) const;
    Float_t ZDCSMD_GetPosition(int eastwest,int verthori,int strip);
    TVector2 ZDCSMD_QEst();
    TVector2 ZDCSMD_QWst();
    TVector2 get_recenteredQ(const TVector2& Q, int cent, int day, int iep);
    TVector2 get_shiftedQ (const TVector2& Q, int cent, int iep);
    //TVector2 get_shifted2Q(const TVector2& Q, int cent, int iep);                                                                         
    float getZDCEPCor(float psi_a, float psi_b);
    int get_runIndex(int run);
    int get_centbShit(int ntrk);
    int get_zdcXbin(float zdcx);
    TFile* m_fOut;
    int NRUNS;
    std::map<int,int> mGoodRunMap;
    int ZDCCalib;
    int EPCalib;
	...
	NCENTSHIFT=40,
        ...
        NEFF=3,
        NTEST=5,
        NINDX=NRAP*NPHI*NEFF
    };
    TH1F* ZDCSMDHist_east_v[8];
    TH1F* ZDCSMDHist_east_h[8];
    TH1F* ZDCSMDHist_west_v[8];
    TH1F* ZDCSMDHist_west_h[8];
    TProfile2D* ZDCSMDBeamCenter;
    TProfile2D* ZDCSMDBeamCenterFilled;
    TH1F* ZDCEPRecenterEntries;
    TProfile2D* ZDCEPRecenter_x[NCENT];
    TProfile2D* ZDCEPRecenter_y[NCENT];
    TProfile2D* ZDCEPRecenterPass1_x[NCENT];
    TProfile2D* ZDCEPRecenterPass1_y[NCENT];
    TProfile2D* ZDCEPShift_cos[NCENT];
    TProfile2D* ZDCEPShift_sin[NCENT];

    TH1F* ZDCEPPsi   ;//[NCENT][NEP];
    TH1F* ZDCEPPsi_rc;//[NCENT][NEP];
    TH1F* ZDCEPPsi_sh;//[NCENT][NEP];
    TH1F* ZDCEPdPsi   [NCENT];
    TH1F* ZDCEPdPsi_rc[NCENT];
    TH1F* ZDCEPdPsi_sh[NCENT];
    TH1F* ZDCEPRes   [NCENT];
    TH1F* ZDCEPRes_rc[NCENT];
    TH1F* ZDCEPRes_sh[NCENT];
    Double_t     ZDCSMDCenterex,ZDCSMDCenterey;           // ZDCSMD Beam Center                                                                          
    Double_t     ZDCSMDCenterwx,ZDCSMDCenterwy;
    // -------------- USER variables -------------------------
    ...
    TH2F * hJpsip_cons_ws;
    TH2F * hJpsip_1_cons;
    TH2F * hJpsip_1_cons_ws;
    TH2F * hJpsip_2_cons;
    TH2F * hJpsip_2_cons_ws;
    //TH1F * hBTOWId;
    //TH1F * hRunID;
    //TH1F * hBAdc;
    //TH2F * hDsmAdc;
    ...
    TH2F * hHadronV2gweight_fms;
    TH2F * hInvRes;
    TH2F * hInvRes_fms;
    TH2F * hInvResgweight;
    TH2F * hInvResgweight_fms;
    TH2F * hHftRatio_d;
    TH2F * hHftRatio_n;
    ...
    TH2F * hEventPlane_Res_fms;
    TH1F * hEventPlane_fms;
    TH1F * hEventPlane_Corr1_fms;
    TH1F * hEventPlane_Corr2_fms;
    ...
    float trigger_id,sig_trigger_id,run_id,event_id,event_centrality, event_qy,event_qy1, event_qy2, event_qx,event_qx1, event_qx2, event_qx1_fms,event_qy1_fms, event_qx2_fms,event_qy2_fms, event_qx_fms,event_qy_fms,sig_chi2,sig_mva,sig_mva_ht,sig_dca,sig_dcaz,sig_p,sig_pt,sig_eta,sig_phi,sig_nsige,sig_nsigpi,sig_nsigk,sig_nsigp,sig_dcaxy,sig_beta,sig_emc0,sig_neta,sig_nphi,sig_charge,sig_phidist, sig_phi2,sig_zdist, sig_yloc, sig_triggercat, event_cen, event_g_qy,event_g_qy1, event_g_qy2, event_g_qx,event_g_qx1, event_g_qx2, sig_isiso,event_ep1, event_gweight;
    ...
    float phoe_trigger_id,phoe_event_gweight,phoe_event_centrality,phoe_event_qy,phoe_event_qy1, phoe_event_qy2, phoe_event_qx,phoe_event_qx1,phoe_probe_chi2,phoe_tag_chi2, phoe_event_qx2,phoe_run_id,PhoE_M,phoe_probe_ncan,phoe_probe_phi,phoe_probe_mva,phoe_pair_decayradius, phoe_pair_dca,phoe_pair_theta,phoe_pair_phi,phoe_pair_costheta,phoe_probe_p,phoe_probe_pt,phoe_probe_nsige,phoe_probe_nsigpi,phoe_probe_nsigk,phoe_probe_nsigp,phoe_probe_dcaxy,phoe_probe_dca,phoe_probe_beta,phoe_probe_emc0,phoe_probe_neta,phoe_probe_nphi,phoe_probe_charge, phoe_tag_charge,phoe_tag_mva,phoe_tag_beta, phoe_probe_phidist, phoe_probe_zdist, phoe_probe_yloc,phoe_triggercat, phoe_isiso, phoe_probe_dcaz;
    ...
    TH2F * run_centrality_corrx_fms;
    TH2F * run_centrality_corry_fms;
