...
Int_t StPicoNpeAnaMaker::Init()
{
   ...
   */
  string const dirFile = mInputFileList.Data();
  if (dirFile.find(".list") != string::npos) {
    ...
    pos = dirFile.find_first_of(" ");
    if (pos!=string::npos) dirFile.erase(pos,dirFile.length()-pos);
  ...
  h1dEvent = new TH1I("h1dEvent", "Number of Events", 12, -1, 12);
  ...
  hJpsip_cons = new TH2F("hJpsip_cons","hJpsip_cons",500,4.,5,300,2.95,3.25);
  hJpsip_cons_ws = new TH2F("hJpsip_cons_ws","hJpsip_cons_ws",500,4,5,300,2.95,3.25);
  hJpsip_1_cons = new TH2F("hJpsip_cons_1","hJpsip_1_cons",500,4,5,300,2.95,3.25);
  hJpsip_1_cons_ws = new TH2F("hJpsip_1_cons_ws","hJpsip_1_cons_ws",500,4,5,300,2.95,3.25);
  hJpsip_2_cons = new TH2F("hJpsip_2_cons","hJpsip_2_cons",500,4,5,300,2.95,3.25);
  hJpsip_2_cons_ws = new TH2F("hJpsip_2_cons_ws","hJpsip_2_cons_ws",500,4,5,300,2.95,3.25);
...
Int_t StPicoNpeAnaMaker::Finish()
{
  ...
  hJpsip_2_ws->Write();
  hJpsip_cons->Write();
  hJpsip_cons_ws->Write();
  hJpsip_1_cons->Write();
  hJpsip_1_cons_ws->Write();
  hJpsip_2_cons->Write();
  hJpsip_2_cons_ws->Write();
  //hQaPt->Write();
  //hQaEta->Write();
  //hQaDca->Write();
  //hQaVxZ->Write();
...
Int_t StPicoNpeAnaMaker::Make()
{
   ...
   */
  if (picoDst->event()->isTrigger(450103)) {
  ...
  h1dEvent->Fill(-1);
  //if(!isMB && !isHLT1 && !isHLT2 && !isHLT3 && !isCentral5)return kStOk; //Selecting events with correct trigger IDs
  if(!isCentral5)return kStOk; //Selecting events with correct trigger IDs
  ...
  //if(isCentral5)h1dEvent->Fill(7.1);
  //if(isMB && !isCentral5)h1dEvent->Fill(8.1);
  //if(!isMB && isCentral5)h1dEvent->Fill(9.1);
  ...
  for(int iTrack = 0 ; iTrack<nTracks;iTrack++){
    ...
    if (mNpeCuts->isGoodInclusiveElectron(track)){
      if(mNpeCuts->myHist()){
        fillMyHistogram(track,pVtx,isCentral5,isHLT1,isHLT2,isHLT3, isMVA, MVADec);// Quick histograms
      }
      if(mNpeCuts->signalTree()){
        if(mNpeCuts->isBEMCElectron(track)){
          ...
          if((isMVA || isHT>0) && track->nSigmaElectron()>-1 && track->nSigmaElectron()<3)fillTree(track,pVtx,isCentral5,isHLT1,isHLT2,isHLT3,MVADec,centrality,Q_X,Q_Y,Q1_X,Q1_Y,Q2_X,Q2_Y); // Filling trees
        }
      }
    }
    // ================= BKG tree + hist ====================
    if(mNpeCuts->bkgTree()){
      if (mNpeCuts->isUnbiasedTrack(track) && mNpeCuts->isTPCHadron(track) && (trigCat>1 || (track->nSigmaPion()>6 &&  (mass < (0.019+0.003)) && (mass > (0.019-0.003)))||iTrack==t1||iTrack==t2||iTrack==t3||iTrack==t4||iTrack==t5||iTrack==t1+1||iTrack==t2+1||iTrack==t3+1||iTrack==t4+1||iTrack==t5+1))fillBKGTree(track,pVtx,isCentral5,isHLT1,isHLT2,isHLT3,MVADec,centrality,Q_X,Q_Y,Q1_X,Q1_Y,Q2_X,Q2_Y);
    }
      ...
      for(int j = 0; j<idxPicoPartnerEs.size();j++){
        ...
        if(elecPair){
          if(mNpeCuts->isGoodElectronPairLoose(elecPair)){
            ...
            fillPairHistogram(tp,pVtx,isCentral5,isHLT1,isHLT2,isHLT3,tt->charge()*tp->charge(),MVADec);
          }
        }
        elecPair->Delete();
      }
      ...
      if(elecPair){
        if(mNpeCuts->isGoodJPsi(elecPair)){
          ...
          for(int k = 0; k<idxPicoTaggedPrs.size();k++){
            ...
            TLorentzVector ltt(ttMom.x(),ttMom.y(),ttMom.z(),ttMom.Mag());
            TLorentzVector ltp(tpMom.x(),tpMom.y(),tpMom.z(),tpMom.Mag());
            TLorentzVector pc = ltk+ltt+ltp;
            TLorentzVector jpsi = ltt+ltp;
            double p_x = tpMom.x()+ttMom.x();
            double p_y = tpMom.y()+ttMom.y();
            double p_z = tpMom.z()+ttMom.z();
            double p_cons = sqrt(p_x*p_x + p_y*p_y + p_z*p_z);
            TLorentzVector jpsi_cons(p_x,p_y,p_z,sqrt(p_cons*p_cons + 3.097*3.097));
            TLorentzVector pc1 = ltk+jpsi_cons;
            if(fabs(pc.PseudoRapidity())<1.5){
              if(jpsi.Pt()<1){
            ...
            if(fabs(pc1.PseudoRapidity())<1.5){
              if(jpsi_cons.Pt()<1){
                if(tt->charge()*tp->charge()<0)hJpsip_cons->Fill(pc1.M(),jpsi_cons.M());
                if(tt->charge()*tp->charge()>0)hJpsip_cons_ws->Fill(pc1.M(),jpsi_cons.M());
              }
              if(jpsi_cons.Pt()>=1&&jpsi.Pt()<2){
                if(tt->charge()*tp->charge()<0)hJpsip_1_cons->Fill(pc1.M(),jpsi_cons.M());
                if(tt->charge()*tp->charge()>0)hJpsip_1_cons_ws->Fill(pc1.M(),jpsi_cons.M());
              }
              if(jpsi_cons.Pt()>=2){
                if(tt->charge()*tp->charge()<0)hJpsip_2_cons->Fill(pc1.M(),jpsi_cons.M());
                if(tt->charge()*tp->charge()>0)hJpsip_2_cons_ws->Fill(pc1.M(),jpsi_cons.M());
              }
            }
          }
        }
    ...
    for(int i = 0;i<idxPicoTaggedEs.size();i++){
      ...
      for(int j = 0; j<idxPicoUnbiasedEs.size();j++){
        ...
        if(elecPair){
          ...
          if(mNpeCuts->isGoodJPsi(elecPair)){
            fillJPsiTree(elecPair,tp,tt,pVtx,isCentral5,isHLT1,isHLT2,isHLT3,tt->charge()*tp->charge(),nCan,MVADec);
            nCan++;
          }
        }
      }
      ...
      for(int j = 0; j<idxPicoUnbiasedEs.size();j++){
        ...
        if(elecPair){
          ...
          if(mNpeCuts->isGoodElectronPair(elecPair)){
            fillPhoETree(elecPair,tp,tt,pVtx,isCentral5,isHLT1,isHLT2,isHLT3,tt->charge()*tp->charge(),nCan,MVADec,centrality,Q_X,Q_Y,Q1_X,Q1_Y,Q2_X,Q2_Y);
            nCan++;
          }
        }
        elecPair->Delete();
    ...
    for(int i = 0;i<idxPicoTaggedPs.size();i++){
      ...
      if(elecPair){
        ...
        if(mNpeCuts->isGoodPionPair(elecPair) && tprobe->gPt()>2.){
          fillKShortTree(elecPair,tprobe,tt,pVtx,isCentral5,isHLT1,isHLT2,isHLT3,tt->charge()*tprobe->charge(),nCan,MVADec);
        }
      }
      elecPair->Delete();
    }
...
void StPicoNpeAnaMaker::fillPairHistogram(StPicoTrack* track,TVector3 pVtx,bool isCentral5,bool isHLT1, bool isHLT2, bool isHLT3, int qq, double MVADec){
  float pt = track->gPt();
  ...
  if(qq<0){
    if(((pt<2.5 && mNpeCuts->isTOFElectron(track)) || pt>=2.5 ) && ((isHLT1 && adc0>=adcCut[1]) || (isHLT2 && adc0>=adcCut[2]) || (isHLT3 && adc0>=adcCut[3]))){
      ...
      if((pt<2.5 && mNpeCuts->isTOFElectron(track) && isCentral5&& mNpeCuts->isBEMCElectron(track)) ||
         (pt>=2.5 && pt<3.5 && isCentral5&& mNpeCuts->isBEMCElectron(track)) ||
         (pt>=3.5 && isCentral5&& mNpeCuts->isBEMCElectron(track))){
...
void StPicoNpeAnaMaker::fillMyHistogram(StPicoTrack* track, TVector3 pVtx,bool isCentral5,bool isHLT1, bool isHLT2, bool isHLT3, bool isMVA, double MVADec){
  float pt = track->gPt();
  TVector3 const pMom = track->gMom(pVtx, mPicoDstMaker->picoDst()->event()->bField());
  float ptot = pMom.Mag();
  double MVADec2 = 0;//mNpeCuts->getDLL_highPt(track);
...
void StPicoNpeAnaMaker::fillJPsiTree(StElectronPair * epair,StPicoTrack* tp,StPicoTrack* track,TVector3 pVtx,bool isCentral5,bool isHLT1, bool isHLT2, bool isHLT3, int qq, int can, double MVADec){
  float pt = tp->gPt();
...
void StPicoNpeAnaMaker::fillKShortTree(StElectronPair * epair,StPicoTrack* tp,StPicoTrack* track,TVector3 pVtx,bool isCentral5,bool isHLT1, bool isHLT2, bool isHLT3, int qq, int can, double MVADec){
  float pt = tp->gPt();
  TVector3 const pMom = tp->gMom(pVtx, mPicoDstMaker->picoDst()->event()->bField());
  float ptot = pMom.Mag();
  ...
  if(index2tof>=0) {
    StPicoBTofPidTraits *tofPid = mPicoDstMaker->picoDst()->btofPidTraits(index2tof);
    if(tofPid){
      TVector3 vec_dp(epair->positionX(),epair->positionY(),epair->positionZ());
      beta = mNpeCuts->getTofBeta(tp,vec_dp);
      //beta = tofPid->btofBeta();
      YLoc = tofPid->btofYLocal();
      mass = ptot*ptot*(1/beta/beta-1);
    }
  ...
  if(tindex2tof>=0) {
    ...
    if(ttofPid){
      TVector3 vec_dp(epair->positionX(),epair->positionY(),epair->positionZ());
      tbeta = mNpeCuts->getTofBeta(track,vec_dp);
      //tbeta = ttofPid->btofBeta();
    }
  }
  float emc0 = 0;
...
void StPicoNpeAnaMaker::fillPhiTree(StElectronPair * epair,StPicoTrack* tp,StPicoTrack* track, TVector3 pVtx,bool isCentral5,bool isHLT1, bool isHLT2, bool isHLT3, int qq, int can, double MVADec){
  float pt = tp->gPt();
  TVector3 const pMom = tp->gMom(pVtx, mPicoDstMaker->picoDst()->event()->bField());
  float ptot = pMom.Mag();
  float beta = 0;
...
void StPicoNpeAnaMaker::fillLambdaTree(StElectronPair * epair,StPicoTrack* tp,StPicoTrack* track,TVector3 pVtx,bool isCentral5,bool isHLT1, bool isHLT2, bool isHLT3, int qq, int can, double MVADec){
  float pt = tp->gPt();
  TVector3 const pMom = tp->gMom(pVtx, mPicoDstMaker->picoDst()->event()->bField());
  float ptot = pMom.Mag();
  float beta = 0;
...
void StPicoNpeAnaMaker::fillPhoETree(StElectronPair * epair,StPicoTrack* tp,StPicoTrack* track,TVector3 pVtx,bool isCentral5,bool isHLT1, bool isHLT2, bool isHLT3, int qq, int can, double MVADec,float cen,float qx, float qy,float qx1, float qy1,float qx2, float qy2){
  float pt = tp->gPt();
  TVector3 const pMom = tp->gMom(pVtx, mPicoDstMaker->picoDst()->event()->bField());
  float ptot = pMom.Mag();
  float beta = 0;
  ...
  if(index2toft>=0) {
    ...
    if(tofPidt){
      TVector3 vec_dp(epair->positionX(),epair->positionY(),epair->positionZ());
      betat = mNpeCuts->getTofBeta(track,vec_dp);
...
void StPicoNpeAnaMaker::fillBKGTree(StPicoTrack* tp,TVector3 pVtx,bool isCentral5,bool isHLT1, bool isHLT2, bool isHLT3, double MVADec,float cen,float qx, float qy,float qx1, float qy1,float qx2, float qy2){
  float pt = tp->gPt();
  TVector3 const pMom = tp->gMom(pVtx, mPicoDstMaker->picoDst()->event()->bField());
  float ptot = pMom.Mag();
  float beta = 0;
...
void StPicoNpeAnaMaker::fillTree(StPicoTrack* tp,TVector3 pVtx,bool isCentral5,bool isHLT1, bool isHLT2, bool isHLT3,double MVADec, float cen, float qx, float qy,float qx1, float qy1,float qx2, float qy2){
  float pt = tp->gPt();
  TVector3 const pMom = tp->gMom(pVtx, mPicoDstMaker->picoDst()->event()->bField());
  float ptot = pMom.Mag();
  float beta = 0;
