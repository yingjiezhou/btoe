...
#include "../StPicoEvent/StPicoMcEvent.h"
...
Int_t StPicoMcNpeAnaMaker::Make()
{
    ...
    mRefMultCorr->init(17062047);//picoDst->event()->runId());
	...
	for(int iTrack=0;iTrack<nMcTracks;iTrack++){                                                  
	    ...
	    if(!mcParentTrk)continue;//changed for hijing
	    float parentGid  = mcParentTrk->GePid();
	    if(parentGid==1 || parentGid==147 || parentGid==149){
		...
		if(idTrack!=-999){
			...
			if((mctrack->GePid()==3 || mctrack->GePid()==2))idxPicoTaggedEs.push_back(iTrack);
