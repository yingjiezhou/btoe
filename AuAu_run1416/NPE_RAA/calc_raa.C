void calc_raa(){
  ...
  TFile * npeRaa = new TFile("../forYifei/electronRAA/finalBtoERaa.root");
  ...
  TFile* fi1 = new TFile("NPE_RAA.root", "READ"); // phenix NPE raa
  ...
  for( int i = 2; i<6;i++){
    ...
    total[i-2]=bval*sqrt(er1*er1/temp1/temp1 + er2h*er2h/temp2/temp2 + r_e*r_e/r/r); // ratio err + raa error
