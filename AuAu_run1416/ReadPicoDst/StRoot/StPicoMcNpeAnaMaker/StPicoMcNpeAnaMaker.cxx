...
Int_t StPicoMcNpeAnaMaker::Init()
{
  ...
  EH_tree->Branch("eh_hft1",&eh_hft1,"eh_hft1/F");
  EH_tree->Branch("eh_hft2",&eh_hft2,"eh_hft2/F");
  //EH_tree->Branch("eh_chi2",&eh_chi2,"eh_chi2/F");
  //EH_tree->Branch("eh_h_chi2",&eh_h_chi2,"eh_h_chi2/F");
  ...
  EH_tree->Branch("eh_isiso",&eh_isiso,"eh_isiso/F"); // not fitted here zyj
  ...
  if (isGoodEvent()){
    ...
    while (1)//Needed if below block is not used
    {
      ...
      if(fabs(track->nSigmaElectron())<3){//mNpeCuts->getDLL(track)> mNpeCuts->cutMVA()){
        ...
        //  }
        //else{
        mIsoEs.push_back(iTrack);// 找iso 的时候，是看有没有配对的partner，所以这里的cut 比较松
        //}
      }
      
      rTracks.push_back(track->id());
      if(isPass(track,pVtx) && track->isHFTTrack())idxPicoTaggedPs.push_back(iTrack); //没用到
      iTrack++;
    }
    iTrack=0;
    ...
    for(int iTrack=0;iTrack<nMcTracks;iTrack++){//
      ...
      if(mctrack)fillGenTree(mctrack,pVtx,Q_X,Q_Y,Q1_X,Q1_Y,Q2_X,Q2_Y);
      //e
      if(!(mctrack->GePid()==3 || mctrack->GePid()==2))continue;
      //if(mctrack->parentId() == Pico::USHORTMAX)continue;
      bool isSingle=false;
      float parentGid=147;
      if(mctrack->parentId() == Pico::USHORTMAX) isSingle=true;
      //else {cout<<"e w/o parent, why zyjjjjjjj"<<endl;continue;}
      else {
        StPicoMcTrack *mcParentTrk = (StPicoMcTrack*)picoDst->mctrack(mctrack->parentId()-1);
        if(!mcParentTrk)continue;
        parentGid  = mcParentTrk->GePid();
      }//changed for hijing
      if(isSingle || parentGid==1 || parentGid==7 || parentGid==17){
        //if(mctrack)fillGenTree(mctrack,pVtx,Q_X,Q_Y,Q1_X,Q1_Y,Q2_X,Q2_Y);
        if(mctrack->assoId()==Pico::USHORTMAX)continue;
        Int_t geid = mctrack->GePid();
        Int_t idTrack = -999;
        isRcTrack(mctrack,picoDst,idTrack, arr); // 是重建出来的track，因为有可能geant 产生了，但是没有被重建出来
        if(idTrack!=-999){
          StPicoTrack *track = (StPicoTrack*)picoDst->track(idTrack); //
          double MVADec = getDLL(track);
          if(isPass(track,pVtx)){
            if((mctrack->GePid()==3 || mctrack->GePid()==2))idxPicoTaggedEs.push_back(iTrack); //找被重建出来的e 的trakc
            if(track->isHFTTrack())idxPicoTaggedEsMC.push_back(iTrack); // 没用到
            //fillTree(track,mctrack,pVtx,isMB,isHLT1,isHLT2,isHLT3,MVADec,Q_X,Q_Y,Q1_X,Q1_Y,Q2_X,Q2_Y); // Filling trees
          }
        }
      }
    }
    //填配对的电子的信息，single 的不填？
    for(int i = 0; i<idxPicoTaggedEs.size();i++){
      ...
      for(int j = 0; j<idxPicoTaggedEs.size();j++){
        ...
        //if(!isMcSibling(mctrack,mctrack1,picoDst))continue;//changed for hijing
        if(mctrack->parentId()!=Pico::USHORTMAX&& mctrack1->parentId()!=Pico::USHORTMAX && mctrack->parentId() == mctrack1->parentId() && mctrack->parentId() != mctrack->mcId() &&  mctrack1->parentId() != mctrack1->mcId()){ //配对
          Int_t idTrack1 = -999;
...
bool StPicoMcNpeAnaMaker::isGoodTrack(StPicoTrack const * const trk) const
{
  return 1;/*
            trk->gPt() > 0 &&
            trk->gPt() < 20 &&
            trk->nHitsFit() >= 20 && trk->nHitsDedx()>=15 && fabs(getDca(trk)) < 1 &&
            (float)trk->nHitsFit()/(float)trk->nHitsMax() > 0.52;
            */}
//-----------------------------------------------------------------------------
...
int StPicoMcNpeAnaMaker::getMcTrack(StPicoTrack const * const PicoTrack, StPicoDst const * const  PicoDst)
{
  for(int i_Rc =0; i_Rc<PicoDst->numberOfMcTracks(); ++i_Rc){
    StPicoMcTrack *Trk = (StPicoMcTrack*)PicoDst->mctrack(i_Rc);
    if(Trk->assoId() == PicoTrack->id())return i_Rc;
  }
  return -1;
}
...
bool StPicoMcNpeAnaMaker::isMCElectron(StPicoMcTrack const *trk){
  if(trk->GePid()==2 || trk->GePid()==3)return true;
  return false;
}
...
void StPicoMcNpeAnaMaker::fillTree(StPicoTrack* tp,StPicoMcTrack *tmc,StThreeVectorF pVtx,bool isMB,bool isHLT1, bool isHLT2, bool isHLT3,double MVADec,double Q_X,double Q_Y,double Q1_X,double Q1_Y,double Q2_X,double Q2_Y){
  ...
  sig_Pxl1Truth = tmc->Pxl1Truth(); //StPicoMcTrack
  ...
  sig_pt = pt;
  sig_p = ptot;
  //sig_isiso = 1;//isIso(tp,tmc);//changed for hijing
  sig_isiso = isIso(tp,tmc);//isIso(tp,tmc);//changed for hijing, zyj changed
  event_qx = Q_X;//qx;
  event_qx1 = Q1_X;//qx1;
  event_qx2 = Q2_X;//qx2;
  event_qy = Q_Y;//qy;
  ...
  if(tmc->parentId() == tmc->mcId()){
    ...
    if(mcParentTrk->parentId() != Pico::USHORTMAX && mcParentTrk->parentId() != mcParentTrk->mcId()){
      ...
      gparentphi = mcgParentTrk->Mom().phi();
...
void StPicoMcNpeAnaMaker::fillGenTree(StPicoMcTrack *tmc,StThreeVectorF pVtx,double Q_X,double Q_Y,double Q1_X,double Q1_Y,double Q2_X,double Q2_Y){
  ...
  gen_head=tempp3;
  int parentGid=0; // 相当于，他的parent id 是不存在的，这里人为设定就是0
  int gparentGid=0;
  int ggparentGid=0;
  float parentPt=0;
  float gparentPt=0;
  float ggparentPt=0;
  if(tmc->parentId() == tmc->mcId()){// 这种情况是不会存在的？？
    parentGid  = tmc->GePid();
  }
  else if(tmc->parentId() != Pico::USHORTMAX){
    StPicoMcTrack *mcParentTrk = (StPicoMcTrack*)mPicoDstMaker->picoDst()->mctrack(tmc->parentId()-1);
...
void StPicoMcNpeAnaMaker::fillEHTree(StPicoTrack* tp,StPicoMcTrack *tmc,StPicoTrack* th,StPicoMcTrack *tmc1,StThreeVectorF pVtx,bool isMB,bool isHLT1, bool isHLT2, bool isHLT3,double Q_X,double Q_Y,double Q1_X,double Q1_Y,double Q2_X,double Q2_Y){
  ...
  int refMult = mPicoEvent->refMult();
  int gRefMult= mPicoEvent->grefMult();
  eh_hft1 = tp->isHFTTrack();
  eh_hft2 = th->isHFTTrack();
  eh_vz = vZ;
  eh_vy = vY;
  eh_vx = vX;
  eh_refmult = refMult;
...
bool StPicoMcNpeAnaMaker::isIso(StPicoTrack* tp, StPicoMcTrack* tmc){
  ...
  for(int i=0;i<mIsoEs.size();i++){
    ...
    if(mass<0.15 && dca<0.2 && prod<0 && decayr<9){
      if(!isMcTrack(track1,mPicoDstMaker->picoDst()))dec = false; // 在mc 中有对应的track ，而不是假的重建出来的track
      else{
