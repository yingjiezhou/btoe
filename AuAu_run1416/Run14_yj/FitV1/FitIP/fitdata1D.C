...
void fitdata1D(TH1F *data,TH1F *data_e,TH1F* data_emis,TH1F *data_pi,TH1F* data_D,TH1F* data_B,int daughter,int mag, int save,
	       char outputfile[],char datfile[],char datc[],char dir1[],char dir2[],char dir3[],char dir4[],char dir5[],char dir6[],
	       int pt,int eta, int phi,float cutLow,float cutHigh,double ptLow ,double ptHigh,double qq, int pur_dir, int HT, double e_const)
{
...
//Signals
	...
	RooRealVar sigma11("sigma11","sigma11",1,0.1,15); 
 	RooRealVar sigma111("sigma111","sigma111",10,0.1,150);
	RooRealVar frac1("frac1","frac1",0.1,0.0,1);
	if(0){
    ...
    RooRealVar b_ratio("b_ratio","b_ratio",0.05,0,0.1);
...
//Now fit All Data
    ...
    txt->SetFillColor(0);
    txt->SetFillStyle(0);
   TPaveText* txt1 = new TPaveText(0.56,0.45,0.9,0.8,"BRNDC");
    ...
    if(1){  
        sprintf(ptbin,"#eta #in [%1.1f,%1.1f]",ptLow,ptHigh);
        sprintf(entr,"#Delta#phi #in [%1.1f,%1.1f]",cutLow,cutHigh);
        txt->AddText(ptbin);
        txt->AddText(entr);
	//txt1->AddText(entr);
	txt1->AddText(chi2);
...
//Objects for saving yields and so forth
    ...
    cout << "PDF fractions " <<b_int << " "<<c_int << " "<<e_int << " " <<h_int << " " <<endl; 
    cout << "PDF fractions " <<b_int1 << " "<<c_int1 << " "<<e_int1 << " " <<h_int1 << " " <<endl; 
    if (save==1){
        ofstream output1 (datfile,ios::app);
        if(output1.is_open()){
            /*output1 <<  qq << " " << eta << " " << phi <<" " <<  yields[2] << " " << yields_errors[2] << " " << pur1 << " " << pur11 << " " << 
	    ...
	    output1 <<  qq << " " << eta << " " << phi <<" "<< yields[3] << " " << yields_errors[3]  <<" " << yields[2] << " " << yields_errors[2] << 	"\n";
            output1.close();
