...
/////Minuit Options
    ...
    const double _start = -13;  
    const double _end = 8;//13;
...
//Signals
    ...
    RooRealVar sigma1("sigma1","sigma1",1,0.1,1.2);
    RooRealVar sigma11("sigma11","sigma11",1,0,1.2);    
    RooGaussian g1("g1","g1",*PID,mean1,sigma1);
    //RooGaussian g1("g1","g1",*PID,mean1,sigma1);
    RooRealVar mean2("mean2","mean2",-3,-5,0);
    if(pt>2)RooRealVar mean2_2("mean2_2","mean2_2",-10,-13,0);
    if(pt<=2)RooRealVar mean2_2("mean2_2","mean2_2",-10,-13,-6);

    RooRealVar sigma2("sigma2","sigma2",1,0.1,2.);  
    RooRealVar sigma2_2("sigma2_2","sigma_2",1,0.3,2.);    
    RooRealVar sigma22("sigma22","sigma22",1.,0.,3.);
    
    RooRealVar sigma222("sigma222","sigma222",1,0.01,2.);
    RooRealVar mean22("mean22","mean22",-1,-3,0);

    RooRealVar frac2("frac2","frac2",0.3,0.0,1);
    RooRealVar frac22("frac22","frac22",0.3,0.,1);
    RooGaussian g2_1("g2_1","g2_1",*PID,mean2,sigma2);
    RooGaussian g2_2("g2_2","g2_2",*PID,mean2_2,sigma2_2);
    //RooAddPdf g2_pr("g2_pr","g2_pr",g2_1,g2_2,frac2);
    //RooGaussian g2("g2","g2",*PID,mean22,sigma222);
     
    //RooAddPdf g2("g2","g2",g2_1,g2_2,frac2);
    RooGaussian g2("g2","g2",*PID,mean2,sigma2);
   
    RooRealVar mean3("mean3","mean3",-3,-13,-2);
    RooRealVar sigma3("sigma3","sigma3",1,0.,5.);  
...
//Background
    ...
    RooRealVar pi_signal_yield_2("pi_signal_yield_2","pi_signal_yield_2",00,00,1000000000);
    RooRealVar pi2_signal_yield("pi2_signal_yield","pi2_signal_yield",000,0,1000000000);
  ...
  yieldList.add(pi_signal_yield_2);
    yieldList.add(pi2_signal_yield);
...
//Now fit Data
    ...
    sigma2.setConstant();
    mean2_2.setConstant();
    sigma2_2.setConstant();


    PID->setRange("range_2pi",1,7);  
    ...
    PID->setRange("range_p",-10,0); 
    if(pt==3)PID->setRange("range_p",-8,0);     
...
//Now fit All Data
    ...
    if(pt>3)completePDF_k.plotOn(PIDFrame_k,Components(RooArgSet(k_signal)),LineStyle(6),LineColor(kYellow-2),FillColor(kGreen-2));
...
//Objects for saving yields and so forth
    ...
    double pur1 = e1_1->getVal()*yields[0] / (had1_2->getVal()*yields[5]  +e1_1->getVal()*yields[0]  + had1_1->getVal()*yields[1]+ had3_1->getVal()*yields[2] + had3_1->getVal()*yields[3] + had4_1->getVal()*yields[4]) ;
    ...
    prettycanvasAll(c1,PIDFrame,pullPlot,0);    
