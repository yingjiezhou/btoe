...
//Signals
    ...
    RooRealVar sigma2("sigma2","sigma2",1,0.1,2.5);    
    ...
    if(pt<6 || pt==7)RooAddPdf g2("g2","g2",g2_1,g2_2,frac2);
...
//Now fit Data
    ...
    if(pt>6) PID->setRange("range_pi",-5,3);
    if(pt>7) PID->setRange("range_pi",-5,3);
...
//Objects for saving yields and so forth
    ...
    if (save==1){
        ...
        if(output1.is_open()){
            output1 <<  pt <<" " <<  yields[0] <<" "<< yields_errors[0] << " " << pur1  << " " << eff << " " << yields[1] << " " << yields_errors[1] <<" " << had1_1->getVal()<< "\n";
            output1.close();
