...
/////Minuit Options
    ...
    if(pt>0)const double _start = -10.5;  
...
//Now fit Data
    ...
    if(pt==1)mean1.setConstant();
    if(pt==1)sigma1.setConstant();
    ...
    if(pt<3)PID->setRange("range_pi",-3,1.5); 
    ...
    if(pt>3)sigma2_22.setRange(sigma2_22.getVal()-sigma2_22.getError(),sigma2_22.getVal()+sigma2_22.getError());
    ...
    //sigma3.setConstant();
    //mean3.setConstant();
    if(pt>8)sigma3.setRange(sigma3.getVal()-sigma3.getError(),sigma3.getVal()+sigma3.getError());
    if(pt>8)mean3.setRange(mean3.getVal()-mean3.getError(),mean3.getVal()+mean3.getError());
    ...
    //mean4.setConstant();
    //sigma4.setConstant();
    if(pt>8)sigma4.setRange(sigma4.getVal()-sigma4.getError()*2,sigma4.getVal()+sigma4.getError()*2);
    if(pt>8)mean4.setRange(mean4.getVal()-mean4.getError()*2,mean4.getVal()+mean4.getError()*2);
