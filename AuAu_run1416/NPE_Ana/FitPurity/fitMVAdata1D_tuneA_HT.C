...
/////Minuit Options
    ...
    if(pt>0)const double _start = -4.5;  
...
//Signals
    ...
    if(pt<7)RooRealVar mean2_2("mean2_2","mean2_2",-2,-5,-0.5);
    if(pt>6)RooRealVar mean2_2("mean2_2","mean2_2",-2,-5,-1);
...
//Background
    ...
    if(0)if(pt<8 && pt>2)shapeList.add(k_signal);
    ...
    if(0)if(pt<8 && pt>2)yieldList.add(k_signal_yield);
