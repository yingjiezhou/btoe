...
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    char cut_bkg[500];
    char cut_bkg1[500];
    if(pt>0)sprintf(cut_s,"probe_neta>0 && probe_nphi>0 && probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);
    if(pt>0)sprintf(cut_b,"probe_neta>0 && probe_nphi>0 && probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge>0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);
    if(pt>0)sprintf(cut_bkg,"probe_neta>0 && probe_nphi>0 && probe_beta>0 && probe_nsige<0 && probe_emc0>0 && probe_bkgcat==1 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);
    if(pt==0)sprintf(cut_s,"probe_neta>0 && probe_nphi>0 && probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",2,8.5);
    if(pt==0)sprintf(cut_b,"probe_neta>0 && probe_nphi>0 && probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge>0 && probe_pt>%1.1f && probe_pt<%1.1f ",2,8.5);
    if(pt==0)sprintf(cut_bkg,"probe_triggercat>1 && probe_triggercat<5 && probe_emc0>0 && probe_bkgcat==1 && probe_pt>%1.1f && probe_pt<%1.1f ",2,8.5);
    if(pt==0)sprintf(cut_bkg1,"(probe_neta>0||probe_nphi>0) && probe_emc0>0 && probe_bkgcat==1 && probe_pt>%1.1f && probe_pt<%1.1f ",2,8.5);
    cout << "Using cuts " << cut_s << endl;
    char file[100] = "root/weights_electron_HT.root";
    char file_pion[100] = "root/weights_pion_HT.root";
//////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////
    ...
    TH1F* hSig_povere = new TH1F("hSig_povere","hSig_povere",50,0,5);
    TH1F* hSig_nsige = new TH1F("hSig_nsige","hSig_nsige",100,-13,13);
    TH1F* hSig_deltabeta = new TH1F("hSig_deltabeta","hSig_deltabeta",50,-0.05,0.05);
    TH1F* hSig_zdist = new TH1F("hSig_zdist","hSig_zdist",100,-6,6);
    TH1F* hSig_phidist = new TH1F("hSig_phidist","hSig_phidist",250,-0.1,0.1);
    TH1F* hSig_hits = new TH1F("hSig_hits","hSig_hits",15,0,15);
    TH1F* hSig_povere_ws = new TH1F("hSig_povere_ws","hSig_povere_ws",50,0,5);
    TH1F* hSig_nsige_ws = new TH1F("hSig_nsige_ws","hSig_nsige_ws",100,-13,13);
    TH1F* hSig_deltabeta_ws = new TH1F("hSig_deltabeta_ws","hSig_deltabeta_ws",50,-0.05,0.05);
    TH1F* hSig_zdist_ws = new TH1F("hSig_zdist_ws","hSig_zdist_ws",100,-6,6);
    TH1F* hSig_phidist_ws = new TH1F("hSig_phidist_ws","hSig_phidist_ws",250,-0.1,0.1);
    TH1F* hSig_hits_ws = new TH1F("hSig_hits_ws","hSig_hits_ws",15,0,15);
    ...
    ch1->Project("hSig_deltabeta","(1/probe_beta - 1/(probe_p / TMath::Sqrt(probe_p*probe_p+0.00051099907*0.00051099907)))/ (1/probe_beta)",cut_s); 
    ch1->Project("hSig_zdist","probe_zdist",cut_s); 
    ch1->Project("hSig_phidist","probe_phidist",cut_s); 
    ch1->Project("hSig_hits","probe_neta+probe_nphi",cut_s); 

    ch1->Project("hSig_povere_ws","probe_p/probe_emc0",cut_b); 
    ch1->Project("hSig_nsige_ws","probe_nsige",cut_b); 
    ch1->Project("hSig_deltabeta_ws","(1/probe_beta - 1/(probe_p / TMath::Sqrt(probe_p*probe_p+0.00051099907*0.00051099907)))/ (1/probe_beta)",cut_b); 
    ch1->Project("hSig_zdist_ws","probe_zdist",cut_b); 
    ch1->Project("hSig_phidist_ws","probe_phidist",cut_b); 
    ch1->Project("hSig_hits_ws","probe_neta+probe_nphi",cut_b); 

    TH1F* hBkg_povere = new TH1F("hBkg_povere","hBkg_povere",50,0,5);
    TH1F* hBkg_nsigpi = new TH1F("hBkg_nsigpi","hBkg_nsigpi",100,-13,13);
    TH1F* hBkg_deltabeta = new TH1F("hBkg_deltabeta","hBkg_deltabeta",50,-0.001,0.001);
    TH1F* hBkg_zdist = new TH1F("hBkg_zdist","hBkg_zdist",200,-6,6);
    TH1F* hBkg_phidist = new TH1F("hBkg_phidist","hBkg_phidist",25,-0.1,0.1);
    TH1F* hBkg_hits = new TH1F("hBkg_hits","hBkg_hits",20,0,20);
    ch2->Project("hBkg_povere","probe_p/probe_emc0",cut_bkg1,"",40000000); 
    ch2->Project("hBkg_nsigpi","probe_nsigpi",cut_bkg,"",40000000); 
    ch2->Project("hBkg_deltabeta","(1/probe_beta - 1/(probe_p / TMath::Sqrt(probe_p*probe_p+0.13957018*0.13957018)))/ (1/probe_beta)",cut_bkg,"",40000000);
    ch2->Project("hBkg_zdist","probe_zdist",cut_bkg,"",40000000); 
    ch2->Project("hBkg_phidist","probe_phidist",cut_bkg,"",40000000); 
    ch2->Project("hBkg_hits","probe_neta+probe_nphi",cut_bkg,"",40000000); 
// OS-SS subtraction  
...
//////////////////////////////////////////////////////////////////
    ...
    ZERO(hSig_phidist);
    ZERO(hSig_hits);
    hSig_deltabeta->Rebin();
  
//CALLING FITTING SCRIPT TO DO FIT////////////////////////////////
//////////////////////////////////////////////////////////////////     
    if(pt>0)fitdata1D(hSig_povere,hSig_nsige,hSig_deltabeta,hSig_zdist,hSig_phidist,hSig_hits,save,file,pt,binning[pt-1],binning[pt]);
    if(pt==0)fitdata1D(hSig_povere,hSig_nsige,hSig_deltabeta,hSig_zdist,hSig_phidist,hSig_hits,save,file,pt,2,8.5);
    //if(pt>0)fitdata1D_pion(hBkg_povere,hBkg_nsigpi,hBkg_deltabeta,hBkg_zdist,hBkg_phidist,hBkg_hits,save,file_pion,pt,binning[pt-1],binning[pt]);
    //if(pt==0)fitdata1D_pion(hBkg_povere,hBkg_nsigpi,hBkg_deltabeta,hBkg_zdist,hBkg_phidist,hBkg_hits,save,file_pion,pt,2,8.5);
    //End Fit
}//End
...
void doLikelihood(){
    ...
    double binning[numPtBins+1]={1.5,2,2.5,3.5,4.5,5.5,8.5};
    char dFile[500];
    char dFile1[500];
    ...
    TH2F* DLL_e_2D = new TH2F("DLL_e_2D","DLL_e_2D",200,0,20,100,-0.3,0.3);
    TH2F* DLL_e_ws_2D = new TH2F("DLL_e_ws_2D","DLL_e_ws_2D",200,0,20,100,-0.3,0.3);
    TH2F* DLL_p_2D = new TH2F("DLL_p_2D","DLL_e_2D",200,0,20,100,-0.3,0.3);*/
    TH1F* DLL_e = new TH1F("DLL_e","DLL_e",100,-4,4);
    TH1F* DLL_e_ws = new TH1F("DLL_e_ws","DLL_e_ws",100,-4,4);
    TH1F* DLL_p = new TH1F("DLL_p","DLL_p",100,-4,4);
    TH2F* DLL_e_2D = new TH2F("DLL_e_2D","DLL_e_2D",200,0,20,400,-1,5);
    TH2F* DLL_e_ws_2D = new TH2F("DLL_e_ws_2D","DLL_e_ws_2D",200,0,20,400,-1,5);
    TH2F* DLL_p_2D = new TH2F("DLL_p_2D","DLL_p_2D",200,0,20,100,-4,4);

    TH1F* DLL_pr = new TH1F("DLL_pr","DLL_pr",100,-4,4);
    TH2F* DLL_pr_2D = new TH2F("DLL_pr_2D","DLL_pr_2D",200,0,20,400,-1,5);
    TH1F* DLL_2p = new TH1F("DLL_2p","DLL_2p",100,-4,4);
    TH2F* DLL_2p_2D = new TH2F("DLL_2p_2D","DLL_2p_2D",200,0,20,400,-1,5);
    TH1F* DLL_k = new TH1F("DLL_k","DLL_k",100,-4,4);
    TH2F* DLL_k_2D = new TH2F("DLL_k_2D","DLL_k_2D",200,0,20,400,-1,5);
    ...
    TH1F* eff_e = new TH1F("eff_e","eff_e",1000,-4,4);
    TH1F* sign = new TH1F("sign ","sign ",1000,-4,4);
    TH1F* line = new TH1F("line","line",100,-4,40);
    TH1F* eff_p = new TH1F("eff_p","eff_p",1000,-4,4);

    TH2F* e_corr_nsige_beta = new TH2F("e_corr_nsige_beta","",200,-13,13,200,-0.2,0.2);
    ...
    TH2F* e_corr_beta_povere = new TH2F("e_corr_beta_povere","",200,-0.2,0.2,200,0,5);
    TH2F* e_corr_phidist_zdist = new TH2F("e_corr_phidist_zdist","",200,-0.1,0.1,200,-30,30);
    TH2F* e_corr_nsige_beta_ws = new TH2F("e_corr_nsige_beta_ws","",200,-13,13,200,-0.2,0.2);
    ...
    TH2F* e_corr_beta_povere_ws = new TH2F("e_corr_beta_povere_ws","",200,-0.2,0.2,200,0,5);
    TH2F* e_corr_phidist_zdist_ws = new TH2F("e_corr_phidist_zdist_ws","",200,-0.1,0.1,200,-30,30);
   
    TH2F* p_corr_nsige_beta = new TH2F("p_corr_nsige_beta","",200,-13,13,200,-0.03,0.03);
    ...
    TFile *file_e = new TFile("./root/weights_electron_HT.root");
    TFile *file_p = new TFile("./root/weights_pion_HT.root");
    ...
    electron_weight_hits=(TH1F*)file_e->Get("electron_weight_smdhits");
    ...
    pion_weight_hits=(TH1F*)file_p->Get("pion_weight_smdhits");
    ...
    Float_t probe_triggercat=0;
    ch1 -> SetBranchAddress( "probe_p" , &p );
    ch1 -> SetBranchAddress( "probe_triggercat" , &p );
	...
	if(!(pt>2))continue; 
	if(fabs(zdist)>5)continue;
	//if(!(probe_triggercat>1))continue;
	if(!(neta>0 && nphi>0))continue;
	...
	int bin6 = electron_weight_hits->FindBin(neta+nphi);
	...
	double weight6= electron_weight_hits->GetBinContent(bin6);	
	...
	bin6 = pion_weight_hits->FindBin(neta+nphi);
	double weight11= 0.7*pion_weight_povere->GetBinContent(bin1);
	double weight22= pion_weight_nsigpi->GetBinContent(bin2);
	double weight33= pion_weight_deltabeta->GetBinContent(bin3);
	double weight44= pion_weight_zdist->GetBinContent(bin4);
	double weight55= pion_weight_phidist->GetBinContent(bin5);
	double weight66= pion_weight_hits->GetBinContent(bin6);
	//double dll = log(weight1*weight2*weight3*weight4*weight5+1.) - log(weight11*weight22*weight33*weight44*weight55+1.); 
	//double dll = TMath::Log(w1*weight1+w2*weight2+w3*weight3+w4*weight4+w5*weight5+w6*weight6) - TMath::Log(w1*weight11+w2*weight22+w3*weight33+w4*weight44+w5*weight55+w6*weight66);
	double sum1 = weight1*weight2*weight3*weight4*weight5*weight6;
	double sum2 = weight11*weight22*weight33*weight44*weight55*weight66;
	if(sum1<1e-21)sum1+=1e-21;
	if(sum2<1e-21)sum2+=1e-21;
	double rati = (sum1)/(sum1+sum2);
	if(rati>=1) rati = 1. - 1.e-15;    
	double dll = -1./15*TMath::Log(1.0/rati-1.);
	if(dll==0)cout << "HERE electron " << rati << " " << probe_charge*tag_charge << " " <<weight3  << " " << weight4 << endl;
	if(dll>300 || dll<-300)dll=2.5;
        ////////////////////DLL
    ...
    ch2 -> SetBranchAddress( "probe_triggercat" , &probe_triggercat );
    Float_t bkgcat=0;
	...
	bin5 = pion_weight_phidist->FindBin(phidist);
	bin6 = pion_weight_hits->FindBin(neta+nphi);
	double weight11= pion_weight_povere->GetBinContent(bin1);
	double weight22= pion_weight_nsigpi->GetBinContent(bin2);
	double weight33= pion_weight_deltabeta->GetBinContent(bin3);
	double weight44= pion_weight_zdist->GetBinContent(bin4);
	double weight55= pion_weight_phidist->GetBinContent(bin5);
	double weight66= pion_weight_hits->GetBinContent(bin6);
	//double dll = log(weight1*weight2*weight3*weight4*weight5+1) - log(weight11*weight22*weight33*weight44*weight55+1);
	//double dll = TMath::Log(weight1+weight2+weight3+weight4+weight5+weight6) - TMath::Log(weight11+weight22+weight33+weight44+weight55+weight66);		
	//double dll = TMath::Log(w1*weight1+w2*weight2+w3*weight3+w4*weight4+w5*weight5+w6*weight6) - TMath::Log(w1*weight11+w2*weight22+w3*weight33+w4*weight44+w5*weight55+w6*weight66);
	double sum1 = weight1*weight2*weight3*weight4*weight5*weight6;
	double sum2 = weight11*weight22*weight33*weight44*weight55*weight66;
	if(sum1<1e-21)sum1+=1e-21;
	...
	if(rati>=1) rati = 1. - 1.e-15;    
	double dll = -1./15*TMath::Log(1.0/rati-1.);
	if(dll==0)cout << "HERE pion " << rati << " " << weight1 << " " << weight2 <<  endl;
        ////////////////////DLL
	if(bkgcat==1){
	    p_corr_nsige_beta->Fill(nsige,dbeta);
	    p_corr_nsige_povere->Fill(nsige,p/emc0);
	    p_corr_povere_zdist->Fill(p/emc0,zdist);
	    p_corr_phidist_zdist->Fill(phidist,zdist);
	    p_corr_beta_povere->Fill(dbeta,p/emc0);
	    pion_povere->Fill(p/emc0);
	    pion_nsige->Fill(nsige);
	    pion_nsigpi->Fill(nsigpi);
	    pion_deltabeta_p->Fill(dbeta);
	    pion_deltabeta_e->Fill(dbeta_e);
	    pion_zdist->Fill(zdist);
	    pion_phidist->Fill(phidist);
	    LL_pp->Fill((weight1*weight2*weight3*weight4*weight5));
	    LL_pe->Fill((weight11*weight22*weight33*weight44*weight55));
	}
	if(dll>300 || dll<-300)dll=-2.5;
	if(bkgcat==1 && nsige<0){
	    DLL_p->Fill(dll);
	    DLL_p_2D->Fill(pt,dll);
	}
	if(bkgcat==2&& nsige>6){
	    DLL_2p->Fill(dll);
    ...
    leg->AddEntry(DLL_e,"Electrons","PEL");
    leg->AddEntry(DLL_p,"Pions","PEL");
  
    TCanvas *canv2 = new TCanvas ("canv2","weights",1600,800);
    canv2->Divide(4,2);
    ...
    pion_weight_hits->SetLineColor(kRed);

    electron_weight_povere->GetYaxis()->SetTitle("Likelihood");
    electron_weight_nsige->GetYaxis()->SetTitle("Likelihood");
    pion_weight_nsigpi->GetYaxis()->SetTitle("Likelihood");
    electron_weight_deltabeta->GetYaxis()->SetTitle("Likelihood");
    electron_weight_zdist->GetYaxis()->SetTitle("Likelihood");
    electron_weight_phidist->GetYaxis()->SetTitle("Likelihood");
    ...
    pion_weight_phidist->GetXaxis()->SetTitle("#Delta#phi");
    electron_weight_hits->GetXaxis()->SetTitle("N_{#eta} + N_{#phi}");
    canv2->cd(1);
   ...
   electron_weight_zdist->Draw("same hist");
    ...
    electron_weight_phidist->Draw("same hist");   
    leg->Draw("same");
    canv2->cd(4);
    ...
    pion_deltabeta_e->GetYaxis()->SetTitle("Likelihood");
    ...
    electron_deltabeta_p->GetYaxis()->SetTitle("Likelihood");
    ...
    leg->Draw("same");
    canv2->cd(8);
    electron_weight_hits->Draw("hist");
    pion_weight_hits->Draw("same hist");
    leg->Draw("same");
	...
	if(temp1>0 && temp2>0){
	    double temp = temp1/DLL_e->Integral(1,max)*1/sqrt(temp1) / sqrt(temp1/DLL_e->Integral(1,max)*1/sqrt(temp1)+67.8*temp2/DLL_p->Integral(1,max)*1/sqrt(temp2));
	    sign->SetBinContent(i,temp);
	}
    }
    eff_p->SetLineColor(kRed);
    eff_p->SetMarkerColor(kRed);
    eff_e->GetYaxis()->SetTitle("#Delta#it{LL}(e-#pi) Efficiency");
    eff_e->GetXaxis()->SetTitle("#Delta#it{LL}(e-#pi) Cut");
    eff_e->GetXaxis()->SetRangeUser(-2,2);
    eff_e->Draw("PE");
    eff_p->Draw("same");
    line->SetLineStyle(7);
    line->Draw("same hist");
    sign->SetLineColor(kGreen-2);
    //sign->DrawNormalized("same hist",10);
    leg->Draw("same");

    TCanvas *canv1 = new TCanvas ("canv1","DLL");
    DLL_e->GetXaxis()->SetTitle("#Delta#it{LL}(e-#pi)");
	...
	pt_eff_e->SetBinError(i,htemp1->Integral(bin1,max)/htemp1->Integral()*1/sqrt(htemp1->Integral()));
	pt_eff_p->SetBinError(i,htemp2->Integral(bin1,max)/htemp2->Integral()*1/sqrt(htemp2->Integral()));
    ...
    pt_eff_e->GetYaxis()->SetTitle("#Delta#it{LL}(e-#pi) Efficiency");
    ...
    line->Draw("same hist");
    leg11->Draw("same");
    lat.DrawLatex(2,0.5,"#Delta#it{LL}(e-#pi)>0.0");
    TCanvas *canv55 = new TCanvas ("canv55","DLL");
    pt_eff_e->GetYaxis()->SetTitle("#Delta#it{LL}(e-#pi) Efficiency");
    pt_eff_e->GetYaxis()->SetRangeUser(0,1.1);
    pt_eff_e->GetXaxis()->SetTitle("#it{p}_{T}");
    pt_eff_e->Draw("PE");
    pt_eff_p->Draw("same PE");
    line->Draw("same hist");
    ...
    leg1->AddEntry(pt_eff_e2,"#Delta#it{LL}(e-#pi)>0.10","LPE");
    leg1->AddEntry(pt_eff_e3,"#Delta#it{LL}(e-#pi)>0.15","LPE");
    leg1->AddEntry(pt_eff_e4,"#Delta#it{LL}(e-#pi)>0.20","LPE");
    leg1->AddEntry(pt_eff_e5,"#Delta#it{LL}(e-#pi)>0.25","LPE");
    pt_eff_e->Draw("same PE");
