...
void makeWeights()
{
    ...
    sprintf(dFile,"../../production/NPE_Tuples_TuneB6.root");
    //sprintf(dFile,"../Ks.root");
    TFile *f_D = new TFile(dFile);
    TTree *t = f_D->Get("Lambda");
...
void AddModel(RooWorkspace* ws){
  ...
  //RooExponential bkgModel("bkgModel","bkgModel",Lambda_M,bg1);
  RooPolynomial bkgModel("bkgModel","bkgModel",Lambda_M,RooArgList(bg1,bg2,bg3));
  //RooChebychev bkgModel("bkgModel","bkgModel",Lambda_M,RooArgList(bg1,bg2));
...
void AddData(RooWorkspace* ws, TTree* t){
    ...
    RooRealVar* Lambda_M = ws->var("Lambda_M");  
    ...
    RooRealVar probe_nsigp("probe_nsigp", "probe_nsigp", -13, 13);
    ...
    RooRealVar Lambda_triggercat("Lambda_triggercat", "Lambda_triggercat", 0,10); 
    ...
    variables->add(probe_nsigp);
    variables->add(probe_charge);
    variables->add(tag_charge);
    variables->add(Lambda_triggercat);
    //variables->add(tag_trigger);
    //variables->add(tag_mva);
    //variables->add(probe_mva);
    //variables->add(pair_dca);
    RooDataSet* data = new RooDataSet("data","data",*variables,Import(*t),Cut("probe_charge*tag_charge<0&&  abs(probe_nsigp)<2")) ; // && Lambda_M>0.45 && Lambda_M<0.55
...
void DoSPlot(RooWorkspace* ws,TTree *tree){
  ...
  std::cout << std::endl <<  "Yield of Lambda is " 
  ...
  for(Int_t i=0; i < 10; i++)
    {
      std::cout << "Lambda Weight   " << sData->GetSWeight(i,"Yield") 
...
void MakePlots(RooWorkspace* ws){
    ...
    //cdata->Divide(1,3); 
    cdata->cd(1);
    RooPlot* frame = Lambda_M->frame(Bins(100)) ; 
    data->plotOn(frame ) ; 
...
void Write(RooWorkspace* ws, TTree* tree){
    ...
    RooDataSet* data = (RooDataSet*) ws->data("dataWithSWeights");
    TTree* Lambda_tree_sw = new TTree("Lambda_tree_sw","Lambda_tree_sw");//tree->CloneTree(0);
    float weight;
    float bkgweight;
    float mass;
    float pt;
    ...
    float phidist;
    float trig;
    Lambda_tree_sw->Branch("Lambda_M",&mass,"Lambda_M/F") ;
    Lambda_tree_sw->Branch("Lambda_triggercat",&trig,"Lambda_triggercat/F") ;
    Lambda_tree_sw->Branch("sWeight",&weight,"sWeight/F") ;
    Lambda_tree_sw->Branch("sWeight_bkg",&bkgweight,"sWeight_bkg/F") ;
    Lambda_tree_sw->Branch("probe_pt",&pt,"probe_pt/F") ;
    Lambda_tree_sw->Branch("probe_p",&p,"probe_p/F") ;
    Lambda_tree_sw->Branch("probe_emc0",&emc0,"probe_emc0/F") ;
    Lambda_tree_sw->Branch("probe_nphi",&nphi,"probe_nphi/F") ;
    Lambda_tree_sw->Branch("probe_neta",&neta,"probe_neta/F") ;
    Lambda_tree_sw->Branch("probe_charge",&charge,"probe_charge/F") ;
    Lambda_tree_sw->Branch("probe_beta",&beta,"probe_beta/F") ;
    Lambda_tree_sw->Branch("probe_nsige",&nsige,"probe_nsige/F") ;
    Lambda_tree_sw->Branch("probe_nsigpi",&nsigpi,"probe_nsigpi/F") ;
    Lambda_tree_sw->Branch("probe_zdist",&zdist,"probe_zdist/F") ;
    Lambda_tree_sw->Branch("probe_phidist",&phidist,"probe_phidist/F") ;
    data->get()->Print();
    RooRealVar* Yield_sw = ws->var("Yield_sw");
    RooRealVar* bkgYield_sw = ws->var("bkgYield_sw");
    RooRealVar* Lambda_M = ws->var("Lambda_M");
    RooRealVar* Lambda_triggercat = ws->var("Lambda_triggercat");
	...
	Lambda_M = (RooRealVar*)row->find(Lambda_M->GetName());	
	mass = Lambda_M->getVal();
	Lambda_triggercat = (RooRealVar*)row->find(Lambda_triggercat->GetName());	
	trig = Lambda_triggercat->getVal();
