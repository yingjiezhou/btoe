...
float StNpeCuts::getTofBeta(StPicoTrack const * const trk, StThreeVectorF const kfVtx) const
{
...
bool StNpeCuts::isTOFElectron(StPicoTrack const *trk) const {
    ...
    if (tofPid) {
        ...
        if (beta < 1e-4) {
            StThreeVectorF const btofHitPos = tofPid->btofHitPos();
            StPhysicalHelixD helix = trk->helix();
StThreeVectorF testThree = getpVtx();
            float pathLength = tofPathLength(&testThree, &btofHitPos, helix.curvature());
            float tof = tofPid->btof();
...
bool StNpeCuts::isBEMCElectron(StPicoTrack const *trk) const {
    ...
    float eoverp = trk->gMom(getpVtx(), mPicoDst2->event()->bField()).mag()/Emc->e0();
...
float StNpeCuts::getDca(StPicoTrack const *trk) const {
    return trk->dcaGeometry().helix().curvatureSignedDistance(getpVtx());
}
...
float StNpeCuts::getMVADec(StPicoTrack const *track){
    ...
    StThreeVectorF const pMom = track->gMom(mPicoDst2->event()->primaryVertex(), mPicoDst2->event()->bField());   
    float ptot = pMom.mag();
    float eoverp = 0;
    float phiDist = 0;
    float zDist = 0;
	...
	StPicoEmcPidTraits * Emc = mPicoDst2->emcPidTraits(track->emcPidTraitsIndex());
	eoverp = ptot/Emc->e0();
	emc0 = Emc->e0();
	phiDist = Emc->phiDist();
	zDist = Emc->zDist();
