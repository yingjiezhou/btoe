...
*/  
    for (i=0;i<4;i++){
	cout <<"Total errors " << sqrt(total[i]*total[i]+totalsys[i]*totalsys[i])/vals[i] << " " << totalold[i]/valsold[i] << " " << fb->GetBinCenter(i+6) << " " <<(gDukeB->Eval(fb->GetBinCenter(i+6))-vals[i])/sqrt(total[i]*total[i]+totalsys[i]*totalsys[i])<<endl;
    }
    gPad->RedrawAxis();
