...
mLength(std::numeric_limits<float>::quiet_NaN())
{
    ...
    // normal method
    //pair<double,double> ss = electronHelix.pathLengths(partnerHelix,10*micrometer,10);
    pair<double,double> ss = electronHelix.pathLengths(partnerHelix,0.01*micrometer,10);
...
//StThreeVectorF const posDCA1 = (kAtDcaToPartner + ((pAtDcaToElectron - kAtDcaToPartner)*0.5));
//StThreeVectorF const posDCA2 = (pAtDcaToElectron + ((kAtDcaToPartner-pAtDcaToElectron)*0.5));
//cout << posDCA1.mag() - posDCA2.mag() << " " << Position.mag() << " " << posDCA1.mag() << " " << posDCA2.mag() << endl;
    mAngle = epairMomAtDca.angle(Position-pVtx);
    mLength = TMath::Sin(mAngle)*Position.mag();
