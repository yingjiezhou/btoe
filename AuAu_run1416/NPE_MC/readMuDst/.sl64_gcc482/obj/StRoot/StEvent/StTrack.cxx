...
void StTrack::setIdTruth() // match with IdTruth
{
    ...
    // loop again over hits and count the number of HFT hits on the track
    mNumHft = mNumGoodHft = 0;
    for ( StHit* h : vh ) 
      {
	if ( kPxlId == h->detector() ||
	     kIstId == h->detector() || 
	     kSstId == h->detector() ) 
	  {
	    mNumHft += 1;                                   // hit is on the HFT
	    if ( tkBest == h->idTruth() ) mNumGoodHft += 1; // and hit belongs on dominant contributor
	  }
      }
