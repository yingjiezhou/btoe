...
void DCA_Plots_old_central5(int save = 1 , int weight_data=1){
  /////////////////////////////////////////////////////////////////////////////////////////
  ...
  TFile *fi = new TFile("root_yj/NewWeights_yj_central5_v3.root");
  //TFile *fi = new TFile("root_yj/NewWeights_yj_central5.root");
  //TFile *fi = new TFile("root_yj/NewWeights_yj_full.root");
  auto pi0_weights = (TH1F*) fi->Get("pi0_weights");
  auto gamma_weights = (TH1F*) fi->Get("gamma_weights");
  auto eta_weights = (TH1F*) fi->Get("eta_weights");
  
  auto pi0_new_weights = (TH1F*) fi->Get("pi0_new_weights");
  auto eta_new_weights = (TH1F*) fi->Get("eta_new_weights");
  auto gamma_new_weights = (TH1F*) fi->Get("gamma_new_weights");
  //=========================================================================
  ...
  //gamma_weights->Scale(1./_scale_);pt_ni
  ...
  TLegend *leg1 = new TLegend(0.6,0.75,0.8,0.9);
  leg1->AddEntry(pi0_new_weights,"#pi^{0} weight","PE");
  leg1->AddEntry(eta_new_weights,"#eta weight","PE");
  leg1->AddEntry(gamma_new_weights,"#gamma weight","PE");
  leg1->Draw("same");
  cfit->SetLogy();
  cfit->SaveAs("plots_yj_c5_v3_3/new_weight_comp.pdf");
  //=========================================================================
  ...
  TH1F *comp11 = new TH1F("comp11","",numPtBins,binning);
  TH1F *comp22 = new TH1F("comp22","",numPtBins,binning);
  TH1F *comp33 = new TH1F("comp33","",numPtBins,binning);
  TH1F *comp44 = new TH1F("comp44","",numPtBins,binning);
  TH1F *comp55 = new TH1F("comp55","",numPtBins,binning);
  ...
  char dFile3[500];
  sprintf(dFile3,"root/MC_newiso_full_new.root");
  //sprintf(dFile3,"/Volumes/YJMAC/Matt_production/MC.root");
  TFile *f_D3 = new TFile(dFile3);
  TChain *ch11 =(TChain *)f_D3->Get("EH");
  char dFile1[500];
  ...
  sprintf(dFile1,"/Volumes/YJMAC/matt_production/NPE_Tuples_Central5_resubmit.root");
  //sprintf(dFile1,"/Users/zhouyingjie/Local/PWG/PWGYJ/NPE/RCF_Run14_Ana_yj/Matt/NPE_Ana_yj/production/NPE_Tuples_TuneA10.root");
  
  TFile *f_D1 = new TFile(dFile1);
  auto ch3 = (TChain*)f_D1->Get("PhoE");
  auto ch33 = (TChain*)f_D1->Get("Signal"); // not used here
  ...
  float s_vz;
  float s_parentphi;
  float event_centrality;
  //======================================================================
  ch3->Project("h1","probe_pt:log10(abs(probe_dca)+0.000000001)","probe_charge*tag_charge<0 && probe_mva>0 && probe_nsige>-1 && probe_nsige<3 && (tag_beta!=0 || probe_beta!=0) && event_centrality>7")  ;
  ch3->Project("h2","probe_pt:log10(abs(probe_dca)+0.000000001)","probe_charge*tag_charge>0 && probe_mva>0 && probe_nsige>-1 && probe_nsige<3 && (tag_beta!=0 || probe_beta!=0)&& event_centrality>7")  ;
  ch3->Project("h11","pair_decayradius:probe_pt","probe_charge*tag_charge<0 && probe_mva>0 && probe_nsige>-1 && probe_nsige<3 && (tag_beta!=0 || probe_beta!=0)&& event_centrality>7")  ;
  ch3->Project("h22","pair_decayradius:probe_pt","probe_charge*tag_charge>0 && probe_mva>0 && probe_nsige>-1 && probe_nsige<3 && (tag_beta!=0 || probe_beta!=0)&& event_centrality>7")  ;
  ch3->Project("h111","PhoE_M:probe_pt","probe_charge*tag_charge<0 && probe_mva>0 && probe_nsige>-1 && probe_nsige<3 && (tag_beta!=0 || probe_beta!=0)&& event_centrality>7")  ;
  ch3->Project("h222","PhoE_M:probe_pt","probe_charge*tag_charge>0 && probe_mva>0 && probe_nsige>-1 && probe_nsige<3 && (tag_beta!=0 || probe_beta!=0)&& event_centrality>7")  ;
  h1->Add(h2,-1);
  h11->Add(h22,-1);
  h111->Add(h222,-1); // zyj used for comparsion with embedding
  //======================================================================
  ...
  for(int i =0;i<ch1->GetEntries();i++){
    ...
    if(s_head == 7){
      ...
      ww = pi0_new_weights->GetBinContent(bin)*0.0743312*5/3.66808;
      //int bin = pi0_weights->FindBin(pt_for_weight);
      //ww = pi0_weights->GetBinContent(bin)*0.561058*5;
      //ww = fpi->Eval(s_pt);
    } else if(s_head == 17){
      int bin = eta_new_weights->FindBin(s_pt);
      ww = eta_new_weights->GetBinContent(bin)*0.0743312*5*0.0069/0.0174*0.45/3.66808;
      //int bin = eta_weights->FindBin(pt_for_weight);
      //ww = eta_weights->GetBinContent(bin)*0.561058*0.0069/0.0174*0.45*5;
      //ww = feta->Eval(s_pt);
    } else if(s_head == 1){
    ...
    int bin_cor = comp->FindBin(s_pt);//pt bin
    ww*=corr[bin_cor-1];
    //if(s_ist==0)ww*=10;
    //============================================
    // Fill
    ...
    if((s_head==7)||(s_head==17) && s_gparentid==0){
      dcapt_all_ni->Fill(TMath::Log10(fabs(s_dcaxy)+0.000000001),s_pt,ww);
      if(s_iso)dcapt_all->Fill(TMath::Log10(fabs(s_dcaxy)+0.000000001),s_pt,ww); // zyj filled here,
    }
    if(s_head==1){// 我加上的gpid，我觉得，只要是乘上比例的，都要保证是直接gamma
    ...
    // component
    if(s_iso==0)continue; //为什么需要这个，fraction需要这个要求么。 就因为这个所以才会放在这个tree 下面
    if(s_head==0){
    ...
    if(s_head==17 && s_parentid==17 && s_gparentid==0){
      ...
      phirespt_eta->Fill((s_parentphi-s_phi),s_pt);
      cosphirespt_eta->Fill(cos(2*(s_parentphi-s_phi)),s_pt);
      if(s_px1*s_px2*s_ist*s_ssd==0)phirespt_eta_mm->Fill((s_parentphi-s_phi),s_pt);
      phi2D_eta->Fill(s_parentphi,s_phi,ww);
      pt2D_eta->Fill(s_pt,s_parentpt);
    }
    if((s_head==7)||(s_head==17) && s_gparentid==0){
    ...
    if(s_head==1 && s_parentid==1 && s_gparentid==0){
      ...
      cosphirespt_gamma->Fill(cos(2*(s_parentphi-s_phi)),s_pt);
      phirespt_gamma->Fill((s_parentphi-s_phi),s_pt);
      if(s_px1*s_px2*s_ist*s_ssd==0)phirespt_gamma_mm->Fill((s_parentphi-s_phi),s_pt);
      phi2D_gamma->Fill(s_parentphi,s_phi);
      pt2D_gamma->Fill(s_pt,s_parentpt,ww);
  ...
  auto pt_ni= (TH1F*)dcapt_ni->ProjectionY();
  auto dca= (TH1F*)dcapt->ProjectionX("dcapt_x",bin1,bin2);
  auto dca_ni= (TH1F*)dcapt_ni->ProjectionX("dcapt_ni_x",bin1,bin2);
  
  auto eta= (TH1F*)etapt->ProjectionX();
  auto eta_ni= (TH1F*)etapt_ni->ProjectionX();
  auto phi= (TH1F*)phipt->ProjectionX();
  auto phi_ni= (TH1F*)phipt_ni->ProjectionX();
  auto vz= (TH1F*)vzpt->ProjectionX();
  auto vz_ni= (TH1F*)vzpt_ni->ProjectionX();
  
  // zyj 0904
  dcapt_all->GetXaxis()->SetRangeUser(-4, -1);
  dcapt_all_ni->GetXaxis()->SetRangeUser(-4, -1);
  auto pt_all =(TH1F*)dcapt_all->ProjectionY();
  auto pt_all_ni =(TH1F*)dcapt_all_ni->ProjectionY();
  ...
  auto pt_eta_nw =(TH1F*)dcapt_eta_nw->ProjectionY();
  auto pt_pi0_nw =(TH1F*)dcapt_pi0_nw->ProjectionY();
  auto pt_eta_gamma_nw =(TH1F*)dcapt_eta_gamma_nw->ProjectionY();
  auto pt_pi0_gamma_nw =(TH1F*)dcapt_pi0_gamma_nw->ProjectionY();
  auto pt_gamma_nw =(TH1F*)dcapt_gamma_nw->ProjectionY();
  
  auto eta_eta_nw =(TH1F*)etapt_eta_nw->ProjectionX();
  auto eta_pi0_nw =(TH1F*)etapt_pi0_nw->ProjectionX();
  auto eta_gamma_nw =(TH1F*)etapt_gamma_nw->ProjectionX();
  auto phi_eta_nw =(TH1F*)phipt_eta_nw->ProjectionX();
  auto phi_pi0_nw =(TH1F*)phipt_pi0_nw->ProjectionX();
  auto phi_gamma_nw =(TH1F*)phipt_gamma_nw->ProjectionX();
  auto vz_eta_nw =(TH1F*)vzpt_eta_nw->ProjectionX();
  auto vz_pi0_nw =(TH1F*)vzpt_pi0_nw->ProjectionX();
  auto vz_gamma_nw =(TH1F*)vzpt_gamma_nw->ProjectionX();
  
  auto pt_eta_nw_ni =(TH1F*)dcapt_eta_nw_ni->ProjectionY();
  auto pt_pi0_nw_ni =(TH1F*)dcapt_pi0_nw_ni->ProjectionY();
  auto pt_gamma_nw_ni =(TH1F*)dcapt_gamma_nw_ni->ProjectionY();
  auto phi_eta_nw_ni =(TH1F*)phipt_eta_nw_ni->ProjectionX();
  auto phi_pi0_nw_ni =(TH1F*)phipt_pi0_nw_ni->ProjectionX();
  auto phi_gamma_nw_ni =(TH1F*)phipt_gamma_nw_ni->ProjectionX();
  auto eta_eta_nw_ni =(TH1F*)etapt_eta_nw_ni->ProjectionX();
  auto eta_pi0_nw_ni =(TH1F*)etapt_pi0_nw_ni->ProjectionX();
  auto eta_gamma_nw_ni =(TH1F*)etapt_gamma_nw_ni->ProjectionX();
  auto vz_eta_nw_ni =(TH1F*)vzpt_eta_nw_ni->ProjectionX();
  auto vz_pi0_nw_ni =(TH1F*)vzpt_pi0_nw_ni->ProjectionX();
  auto vz_gamma_nw_ni =(TH1F*)vzpt_gamma_nw_ni->ProjectionX();
  
  auto dca_gamma_nw =(TH1F*)dcapt_gamma_nw->ProjectionX("dcaptdcapt_gamma_nw_x",bin1,bin2);
  auto dca_eta_nw =(TH1F*)dcapt_eta_nw->ProjectionX("dcaptdcapt_eta_nw_x",bin1,bin2);
  auto dca_pi0_nw =(TH1F*)dcapt_pi0_nw->ProjectionX("dcapt_pi0_nw_x",bin1,bin2);
  auto dca_gamma_nw_ni =(TH1F*)dcapt_gamma_nw_ni->ProjectionX("dcapt_gamma_nw_ni_x",bin1,bin2);
  auto dca_eta_nw_ni =(TH1F*)dcapt_eta_nw_ni->ProjectionX("dcapt_eta_nw_ni_x",bin1,bin2);
  auto dca_pi0_nw_ni =(TH1F*)dcapt_pi0_nw_ni->ProjectionX("dcapt_pi0_nw_ni_x",bin1,bin2);
  
  //======================================================================
  for(int i=0;i<9;i++){ // w/ weight and iso,
    ...
    comp->SetBinContent(i+1,temp7/temp8); // zyj for total iso eff
  ...
  TCanvas *ccomp = new TCanvas("ccomp","ccomp ept");
  comp1->GetYaxis()->SetRangeUser(0,1.2);
  comp1->GetXaxis()->SetTitle("#it{p}_{T} [GeV](e_pt)");
  ...
  leg->Draw("same");
  ccomp->SaveAs("plots_yj_c5_v3_3/PhoECom.pdf");
  //======================================================================
  ...
  for(int i =0;i<ch11->GetEntries();i++){
    ...
    if(s_head == 7){
      int bin = pi0_weights->FindBin(pt_for_weight);
      ww = pi0_weights->GetBinContent(bin)*0.0743312*5/3.66808;//0.0666488
      //int bin = pi0_new_weights->FindBin(s_pt);
      //ww = pi0_new_weights->GetBinContent(bin)*0.0174/80;
      //ww = fpi->Eval(s_pt);
    } else if(s_head == 17){
      int bin = pi0_weights->FindBin(pt_for_weight);//
      ww = eta_weights->GetBinContent(bin)*0.0743312*0.0069/0.0174*0.45*5/3.66808;
      //int bin = pi0_new_weights->FindBin(s_pt);
      //ww = pi0_new_weights->GetBinContent(bin)*0.0174/80;
      //ww = feta->Eval(s_pt);
    } else if(s_head == 1){
    ...
    if(s_head==1 && s_gparentid==0){
      ...
      pt2D_eh_gamma->Fill(s_pt,s_parentpt,ww);
    }
    if(s_head==7 && s_gparentid==0){// w/ weight, but PHE, not iso
      ...
      pt2D_eh_pi0->Fill(s_pt,s_parentpt,ww);
    }
    if(s_head==17 && s_gparentid==0){
      ...
      pt2D_eh_eta->Fill(s_pt,s_parentpt,ww);
    }
    
    if((s_head==17||s_head==7) && s_gparentid==0){
    ...
    if(s_head==1 && s_gparentid==0){ // gamma
      ...
      mass_eh_all->Fill(s_pt,s_m,ww*(1+1/(1.21361-1))); // weighted sum of all e
  ...
  for(int i=0;i<9;i++){
    int bin1 = pt_eh_pi0->FindBin(binning[i]);
    int bin2 = pt_eh_pi0->FindBin(binning[i+1]);
    double temp1 = pt_eh_pi0->Integral(bin1,bin2);
    double temp2 = pt_eh_eta->Integral(bin1,bin2);
    double temp3 = pt_eh_gamma->Integral(bin1,bin2);
    double temp4 = pt_eh_pi0_gamma->Integral(bin1,bin2);
    double temp5 = pt_eh_eta_gamma->Integral(bin1,bin2);
    double temp6 = temp1+temp2+temp3+temp4+temp5;
    if(temp6>0){
      comp11->SetBinContent(i+1,temp1/temp6);
      comp22->SetBinContent(i+1,temp2/temp6);
      comp33->SetBinContent(i+1,temp3/temp6);
      comp44->SetBinContent(i+1,temp4/temp6);
      comp55->SetBinContent(i+1,temp5/temp6);
    }
  }
  comp11->SetLineColor(kBlue);
  comp11->SetMarkerColor(kBlue);
  comp22->SetLineColor(kGreen-2);
  comp22->SetMarkerColor(kGreen-2);
  comp33->SetLineColor(kRed);
  comp33->SetMarkerColor(kRed);
  comp44->SetLineColor(kCyan);
  comp44->SetMarkerColor(kCyan);
  comp55->SetLineColor(kGray);
  comp55->SetMarkerColor(kGray);
  TCanvas *ccomp_ee = new TCanvas("ccomp_ee","ccomp gpt");
  comp11->GetYaxis()->SetRangeUser(0,1);
  comp11->GetXaxis()->SetTitle("#it{p}_{T} [GeV]");
  comp11->GetYaxis()->SetTitle("Relative Fraction");
  comp11->Draw("same");
  comp22->Draw("same");
  comp33->Draw("same");
  comp44->Draw("same");
  comp55->Draw("same");
  leg->Draw("same");
  ccomp_ee->SaveAs("plots_yj_c5_v3_3/PhoEComp.pdf"); // phe component constitute, fraction w/ weight
  //======================================================================
  
  //TFile f("root/DCA_plots.root","RECREATE");
  TFile f("root_yj/DCA_plots_yj_central5_v3_2_resubmit.root","RECREATE");
  ...
  comp11->Write("Pi0_ee_Comp");
  comp22->Write("Eta_ee_Comp");
  comp33->Write("Gamma_ee_Comp");
  comp44->Write("Pi0Gamma_ee_Comp");
  comp55->Write("EtaGamma_ee_Comp");
  f.Close();
  //} // end of if save
  //======================================================================
  
  calcEff(pt_ni,pt); // zyj shown in presentation
  ...
  TCanvas *ciso = new TCanvas("ciso","pt iso eff.");
  //pt_ni->GetYaxis()->SetRangeUser(0,1.5);
  //pt_ni->Draw("PE");
  //pt_pi0_nw_ni->Draw("same PE"); //有直方图，填了，就是没在这里画出来
  ...
  comp->Draw("same PE"); // for total embeding phe e iso eff

  leg5->Draw("same");
  TF1 *line = new TF1("line","1",0,100);
  line->SetLineStyle(7);
  line->Draw("same");
  ciso->SaveAs("plots_yj_c5_v3_3/DCAEff_Pt_Q.pdf"); // comp 就没填图， 本来是填各个成分的fraction的
  ciso->SaveAs("plots_yj_c5_v3_3/DCAEff_Pt_Q.C"); // comp 就没填图， 本来是填各个成分的fraction的
  //==========================================================================
  ...
  ciso1->SaveAs("plots_yj_c5_v3_3/PhiEff_Pt.pdf");
  ...
  ciso2->SaveAs("plots_yj_c5_v3_3/EtaEff_Pt.pdf"); // we have
  ...
  ciso3->SaveAs("plots_yj_c5_v3_3/VzEff_Pt.pdf");
  ...
  ciso33->SaveAs("plots_yj_c5_v3_3/DCAEff_Pt.pdf"); // at high pt
  //==========================================================================
  ...
  cpt->SaveAs("plots_yj_c5_v3_3/pt_comp_Q.pdf");
  //==========================================================================
  ...
  auto hx =(TH1F*)h1->ProjectionX(); // not used here
  ...
  auto dca_eh_all =(TH1F*)dcapt_eh_all->ProjectionX();
  auto dca_eh_all_m1 =(TH1F*)dcapt_eh_all_m1->ProjectionX();
  auto dca_eh_all_m4 =(TH1F*)dcapt_eh_all_m4->ProjectionX();
  auto dca_eh_gamma =(TH1F*)dcapt_eh_gamma->ProjectionX();
  auto dca_all =(TH1F*)dcapt_all->ProjectionX();
  auto dca_all_nw =(TH1F*)dcapt_all_nw->ProjectionX();
  auto dca_all_m1 =(TH1F*)dcapt_all_m1->ProjectionX();
  auto dca_all_m4 =(TH1F*)dcapt_all_m4->ProjectionX();
  ...
  auto dca_eta_gamma_nw =(TH1F*)dcapt_eta_gamma_nw->ProjectionX();
  auto dca_pi0_gamma_nw =(TH1F*)dcapt_pi0_gamma_nw->ProjectionX();
  ...
  c2D1->cd(2);
  dcapt_pi0_high->DrawClone("COLZ"); // don't know what is ???
  ...
  auto dcapt_pi0_low_x = (TH1F*)dcapt_pi0_low->ProjectionX();
  auto dcapt_pi0_high_x = (TH1F*)dcapt_pi0_high->ProjectionX();
  auto dcapt_eta_low_x = (TH1F*)dcapt_eta_low->ProjectionX();
  auto dcapt_eta_high_x = (TH1F*)dcapt_eta_high->ProjectionX();
  auto dcapt_gamma_low_x = (TH1F*)dcapt_gamma_low->ProjectionX();
  auto dcapt_gamma_high_x = (TH1F*)dcapt_gamma_high->ProjectionX();
  ...
  dcapt_gamma_high_x->Draw("same PE"); // don't know??
  c2D4->SaveAs("plots_yj_c5_v3_3/comp_gamma_dca.pdf");
  //==========================================================================
  ...
  dr_eh_all->Draw("COLZ");
  c3->SaveAs("plots_yj_c5_v3_3/e_radius_2D.pdf");
  //==========================================================================
  TCanvas *Dr = new TCanvas("Dr","PhoE Decay Radius");
  Dr->SetLogy();
  dr_eh_all->GetXaxis()->SetRangeUser(2,8.5);
  dr_eh_all->GetYaxis()->SetRangeUser(0.,20); // set 到20看看，和matt 的对比， 一开始是10
  h11->GetXaxis()->SetRangeUser(2,8.5);
  h11->GetYaxis()->SetRangeUser(0.,10);
  
  auto dr3 = (TH1F*)dr_eh_all->ProjectionY();
  auto hdr = (TH1F*) h11->ProjectionY();
  dr3->Rebin(10);
  hdr->Rebin(10);
  dr3->GetYaxis()->SetRangeUser(2e-5,2e-1);
  ...
  Dr->SaveAs("plot_yj_v2/PhoEDecayRadius.pdf");
  
  //==========================================================================

  double low1=2.0;
  double high1=8.5;
  TCanvas *c31 = new TCanvas ("c31","1D radius");
  ...
  auto dr_1_x =(TH1F*) dr_eh_all->ProjectionY();
  auto h11_1_x =(TH1F*) h11->ProjectionY();
  ...
  c31->SaveAs("plots_yj_c5_v3_3/PhoE_radius.pdf"); // zyj don't know
  //==========================================================================
  ...
  c33->SaveAs("plots_yj_c5_v3_3/sanity_check_mass_pt.pdf");
  ...
  auto mass_1_x =(TH1F*)  mass_eh_all->ProjectionY();
  auto h111_1_x =(TH1F*)  h111->ProjectionY();
  ...
  norm5(h111_1_x);
  mass_1_x->Draw("PE");
  h111_1_x->Draw("same PE"); //from data
  c331->SaveAs("plots_yj_c5_v3_3/PhoE_mass.pdf");
    return ;
  //==========================================================================
  ...
  auto dca_eh_all_m11 =(TH1F*)dcapt_eh_all_m1->ProjectionX("dca_eh_all_m11");
  dcapt_eh_all_m2->GetYaxis()->SetRangeUser(binning[0],binning[1]);
  auto dca_eh_all_m12 =(TH1F*)dcapt_eh_all_m2->ProjectionX("dca_eh_all_m12");
  dcapt_eh_all_m3->GetYaxis()->SetRangeUser(binning[0],binning[1]);
  auto dca_eh_all_m13 =(TH1F*)dcapt_eh_all_m3->ProjectionX("dca_eh_all_m13");
  dcapt_eh_all_m4->GetYaxis()->SetRangeUser(binning[0],binning[1]);
  auto dca_eh_all_m14 =(TH1F*)dcapt_eh_all_m4->ProjectionX("dca_eh_all_m14");
  ...
  auto dca_eh_all_m21 =(TH1F*)dcapt_eh_all_m1->ProjectionX("dca_eh_all_m21");
  dcapt_eh_all_m2->GetYaxis()->SetRangeUser(binning[1],binning[2]);
  auto dca_eh_all_m22 =(TH1F*)dcapt_eh_all_m2->ProjectionX("dca_eh_all_m22");
  dcapt_eh_all_m3->GetYaxis()->SetRangeUser(binning[1],binning[2]);
  auto dca_eh_all_m23 =(TH1F*)dcapt_eh_all_m3->ProjectionX("dca_eh_all_m23");
  dcapt_eh_all_m4->GetYaxis()->SetRangeUser(binning[1],binning[2]);
  auto dca_eh_all_m24 =(TH1F*)dcapt_eh_all_m4->ProjectionX("dca_eh_all_m24");
  ...
  auto dca_eh_all_m31 =(TH1F*)dcapt_eh_all_m1->ProjectionX("dca_eh_all_m31");
  dcapt_eh_all_m2->GetYaxis()->SetRangeUser(binning[2],binning[3]);
  auto dca_eh_all_m32 =(TH1F*)dcapt_eh_all_m2->ProjectionX("dca_eh_all_m32");
  dcapt_eh_all_m3->GetYaxis()->SetRangeUser(binning[2],binning[3]);
  auto dca_eh_all_m33 =(TH1F*)dcapt_eh_all_m3->ProjectionX("dca_eh_all_m33");
  dcapt_eh_all_m4->GetYaxis()->SetRangeUser(binning[2],binning[3]);
  auto dca_eh_all_m34 =(TH1F*)dcapt_eh_all_m4->ProjectionX("dca_eh_all_m34");
  ...
  auto dca_eh_all_m41 =(TH1F*)dcapt_eh_all_m1->ProjectionX("dca_eh_all_m41");
  dcapt_eh_all_m2->GetYaxis()->SetRangeUser(binning[3],binning[4]);
  auto dca_eh_all_m42 =(TH1F*)dcapt_eh_all_m2->ProjectionX("dca_eh_all_m42");
  dcapt_eh_all_m3->GetYaxis()->SetRangeUser(binning[3],binning[4]);
  auto dca_eh_all_m43 =(TH1F*)dcapt_eh_all_m3->ProjectionX("dca_eh_all_m43");
  dcapt_eh_all_m4->GetYaxis()->SetRangeUser(binning[3],binning[4]);
  auto dca_eh_all_m44 =(TH1F*)dcapt_eh_all_m4->ProjectionX("dca_eh_all_m44");
  ...
  auto dca_eh_all_m51 =(TH1F*)dcapt_eh_all_m1->ProjectionX("dca_eh_all_m51");
  dcapt_eh_all_m2->GetYaxis()->SetRangeUser(binning[4],binning[5]);
  auto dca_eh_all_m52 =(TH1F*)dcapt_eh_all_m2->ProjectionX("dca_eh_all_m52");
  dcapt_eh_all_m3->GetYaxis()->SetRangeUser(binning[4],binning[5]);
  auto dca_eh_all_m53 =(TH1F*)dcapt_eh_all_m3->ProjectionX("dca_eh_all_m53");
  dcapt_eh_all_m4->GetYaxis()->SetRangeUser(binning[4],binning[5]);
  auto dca_eh_all_m54 =(TH1F*)dcapt_eh_all_m4->ProjectionX("dca_eh_all_m54");
  ...
  auto dca_eh_all_m61 =(TH1F*)dcapt_eh_all_m1->ProjectionX("dca_eh_all_m61");
  dcapt_eh_all_m2->GetYaxis()->SetRangeUser(binning[5],binning[6]);
  auto dca_eh_all_m62 =(TH1F*)dcapt_eh_all_m2->ProjectionX("dca_eh_all_m62");
  dcapt_eh_all_m3->GetYaxis()->SetRangeUser(binning[5],binning[6]);
  auto dca_eh_all_m63 =(TH1F*)dcapt_eh_all_m3->ProjectionX("dca_eh_all_m63");
  dcapt_eh_all_m4->GetYaxis()->SetRangeUser(binning[5],binning[6]);
  auto dca_eh_all_m64 =(TH1F*)dcapt_eh_all_m4->ProjectionX("dca_eh_all_m64");
  ...
  auto dca_eh_all_m71 =(TH1F*)dcapt_eh_all_m1->ProjectionX("dca_eh_all_m71");
  dcapt_eh_all_m2->GetYaxis()->SetRangeUser(binning[6],binning[7]);
  auto dca_eh_all_m72 =(TH1F*)dcapt_eh_all_m2->ProjectionX("dca_eh_all_m72");
  dcapt_eh_all_m3->GetYaxis()->SetRangeUser(binning[6],binning[7]);
  auto dca_eh_all_m73 =(TH1F*)dcapt_eh_all_m3->ProjectionX("dca_eh_all_m73");
  dcapt_eh_all_m4->GetYaxis()->SetRangeUser(binning[6],binning[7]);
  auto dca_eh_all_m74 =(TH1F*)dcapt_eh_all_m4->ProjectionX("dca_eh_all_m74");
  ...
  auto dca_eh_all_m81 =(TH1F*)dcapt_eh_all_m1->ProjectionX("dca_eh_all_m81");
  dcapt_eh_all_m2->GetYaxis()->SetRangeUser(binning[7],binning[8]);
  auto dca_eh_all_m82 =(TH1F*)dcapt_eh_all_m2->ProjectionX("dca_eh_all_m82");
  dcapt_eh_all_m3->GetYaxis()->SetRangeUser(binning[7],binning[8]);
  auto dca_eh_all_m83 =(TH1F*)dcapt_eh_all_m3->ProjectionX("dca_eh_all_m83");
  dcapt_eh_all_m4->GetYaxis()->SetRangeUser(binning[7],binning[8]);
  auto dca_eh_all_m84 =(TH1F*)dcapt_eh_all_m4->ProjectionX("dca_eh_all_m84");
  ...
  auto dca_eh_all_m91 =(TH1F*)dcapt_eh_all_m1->ProjectionX("dca_eh_all_m91");
  dcapt_eh_all_m2->GetYaxis()->SetRangeUser(binning[8],binning[9]);
  auto dca_eh_all_m92 =(TH1F*)dcapt_eh_all_m2->ProjectionX("dca_eh_all_m92");
  dcapt_eh_all_m3->GetYaxis()->SetRangeUser(binning[8],binning[9]);
  auto dca_eh_all_m93 =(TH1F*)dcapt_eh_all_m3->ProjectionX("dca_eh_all_m93");
  dcapt_eh_all_m4->GetYaxis()->SetRangeUser(binning[8],binning[9]);
  auto dca_eh_all_m94 =(TH1F*)dcapt_eh_all_m4->ProjectionX("dca_eh_all_m94");
  norm(hx1);
  norm(hx2);
  norm(hx3);
  norm(hx4);
  ...
  c11->cd(10);
  leg2->Draw();
  c11->SaveAs("plots_yj_c5_v3_3/Gamma_InvYield_lin.pdf");
