...
void plot_newWeight_old(int save = 1, int weight_data=1){ //weight only for phoE pair comp. and reweight for weight.C, and if doing other comp. integrated over pt
  /////////////////////////////////////////////////////////////////////////////////////////
  ...
  TFile *fi = new TFile("root/Pi0_Gamma_Ratio.root"); // matt
  ...
  //============================================================================
    ...
    for(int i =0;i<ch1->GetEntries();i++){
      ...
      if(s_head == 7 && s_gparentid==0){
        ...
        prox = s_parentpt;
        if(s_parentpt<0.35)s_parentpt=0.35;//1.17606;
        //ww = fpi->Eval(s_parentpt);
        int bin = pi0_weights->FindBin(s_parentpt);
        ww = pi0_weights->GetBinContent(bin);//gamma_weights->GetBinContent(bin);
        //write_pi0->Fill(prox,ww);
        if(0);//ratio->GetBinContent(bin)>0 && ratio->GetBinContent(bin)>1)ww2*= ratio->GetBinContent(bin)*0.01174;
        else{ww2*=0.01174;} // BR
      }
      if(s_head == 17 && s_gparentid==0){
        ...
        //ww2*= offset*0.2*num_gamma/num_eta*0.45* 0.0069/0.0174 * 1/2.64366201806170466e+02;// * 0.0069/0.0174

        //ww2*= num_pi0/num_eta*0.45; //本来的ratio 就是0.45，需要乘上一个数目的比值
        //int bin = ratio->FindBin(s_parentpt);
        if(0);//ratio->GetBinContent(bin)>0 && ratio->GetBinContent(bin)>1)ww2*= ratio->GetBinContent(bin)*0.0069;
        else{ww2*=0.0069;}
      }
      if(s_head == 1 && s_gparentid==0){
        ...
        //if(ww<1e-9)ww=0;
        //if(ww>5)ww=1;
        //write_gamma->Fill(prox,ww);
        //ww2*= num_pi0/num_gamma;
        //if(ratio->GetBinContent(bin)>0 && ratio->GetBinContent(bin)>1)ww2*= 1/ratio->GetBinContent(bin);
        //else{ww2*=1;}
      }
      //if(ww>5)ww=1;
  ...
  //============================================================================
    ...
    for(int i =0;i<ch11->GetEntries();i++){
      ...
      if(s_head == 7){
        ...
        if(s_parentpt<0.35)s_parentpt=0.35;//1.17606;
        ww = fpi->Eval(s_parentpt);//???? pi0_weights??
        //ww2 *= offset*0.0174;//offset*0.8*num_gamma/num_pi0 * 1/2.64366201806170466e+02;//num_gamma/num_pi0;
      ...
      if(s_head == 17){
        ...
        if(s_parentpt<0.35)s_parentpt=0.35;//1.17606;
        ww = fpi->Eval(s_parentpt);
        //ww2*= offset*0.2*num_gamma/num_eta*0.45* 0.0069/0.0174 * 1/2.64366201806170466e+02;
  ...
  mass_gamma->GetYaxis()->SetRangeUser(5e-5, 5e-1);
  ...
  lat.DrawLatex(12,10e-2,"2.0<#it{p}_{T} [GeV]<8.5"); // 没显示出来，估计是位置没放对
