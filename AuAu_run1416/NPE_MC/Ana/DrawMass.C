...
void DrawMass(){
    ...
    auto dr =(TH2F*)f->Get("mass_eh_all");
    auto h11 = (TH2F*) f->Get("h111");
    dr->SetLineColor(kRed);
    dr->SetMarkerColor(kRed);


    dr->GetXaxis()->SetRangeUser(1,10);
    h11->GetXaxis()->SetRangeUser(1,10);
...
void norm5(TH1F *h){
    h->GetXaxis()->SetTitle("m(ee)");
    h->GetYaxis()->SetTitle("A. U.");
    h->GetYaxis()->SetRangeUser(0.00001,0.3);
    h->Rebin(5);
