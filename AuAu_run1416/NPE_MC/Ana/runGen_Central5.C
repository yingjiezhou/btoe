...
void runGen_Central5(int save = 0){
  ...
  //auto *gpi = new TMultiGraph();
  //data_C5M(gpi);
  auto *gpi = new TGraphErrors();
  data_12C5(gpi);
  ...
  fpi->SetParameter(0,15000);
  fpi->SetParameter(1,1.);
  fpi->SetParameter(2,0.1);
  ...
  sprintf(dFile,"/Volumes/YJMAC/Matt_production/MCv2.root");// zyj
  ...
  Pi0GammaRatio->Draw();
  double int_gamma = fgamma->Integral(2,20);
  double int_pi0 = fpi->Integral(2,20);
  ...
  */
  ...
  //return ;
    ...
    TFile ff1("root_yj/NewWeights_yj_central5_v3.root","RECREATE");// with Xin's pion pt, v2 version is from phenix pion v2
