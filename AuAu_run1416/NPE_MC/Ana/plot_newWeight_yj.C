...
void plot_newWeight_yj(int save = 1, int weight_data=1){
  ...
  //=================================================================================
  TFile *fi = new TFile("root_yj/NewWeights_yj.root");
  ...
  sprintf(dFile1,"/Volumes/YJMAC/Matt_production/MC.root");
  ...
  TH2F *dcapt_fit = new TH2F("dcapt_fit","dcapt_fit",220,-0.11,0.11,85,0,8.5);dcapt_fit->Sumw2();
  TH2F *dcapt_fit_b = new TH2F("dcapt_fit_b","dcapt_fit_b",220,-0.11,0.11,85,0,8.5);dcapt_fit->Sumw2();
  ...
  for(int i =1e8;i<ch2->GetEntries();i++){
    ...
    if(s_id==7 && s_head==0){
      ...
      write_pi0->Fill(s_pt,pi0_new_weights->GetBinContent(bin));
    }
    if(s_id==1 && s_head==0){
      gen_gamma->Fill(s_pt);
      int bin = gamma_new_weights->FindBin(s_pt);
      write_gamma->Fill(s_pt,gamma_new_weights->GetBinContent(bin));
    }
  }
  ...
  for(int i =0;i<ch1->GetEntries();i++){
    ...
    if(fabs(s_dcaxy)>1)continue; //0827
    if(s_pt<0)continue;
    if(s_hft<1)continue;
    double ww = 1;
    double ww2 =1;
    ...
    if(s_head == 17){
      int bin = pi0_new_weights->FindBin(s_pt);
      ww = pi0_new_weights->GetBinContent(bin)*0.0199728*5*0.45*0.0069/0.0174;//
    }
    if(s_head == 1){
      int bin = gamma_new_weights->FindBin(s_pt);
    ...
    if(s_head==0){
      ...
      if(s_iso==1)dcapt_iso->Fill(s_cen+0.3,s_pt);// 加不加weight最后都会抵消，自己保持一致即可
      dcapt_noiso->Fill(s_cen+0.3,s_pt);
    }
    
    if(s_head==7 && s_parentid==7 && s_gparentid==0 && s_iso==1)dcapt_pi0->Fill(s_dcaxy,s_pt,ww2*ww);
    phi_pi0->Fill((s_phi-s_parentphi),ww*ww2); //0826???ww*ww??
    
    if(s_head==17 && s_parentid==17 && s_gparentid==0 && s_iso==1)dcapt_eta->Fill(s_dcaxy,s_pt,ww2*ww);
    if((s_head==7|| s_head==17) && s_gparentid==0){ //0826
      if(s_iso==1)dcapt_all->Fill(s_dcaxy,s_pt,ww2*ww);
      if(s_iso==1)dcapt_fit->Fill(s_dcaxy,s_pt,ww2);
      if(s_iso==1)dcapt_all_iso->Fill(s_cen+0.3,s_pt,ww*ww2);
      dcapt_all_noiso->Fill(s_cen+0.3,s_pt,ww*ww2);
    }
    if(s_head==1 && s_gparentid==0){
      if(s_iso==1)dcapt_gamma->Fill(s_dcaxy,s_pt,ww2*ww);
      if(s_iso==1)dcapt_eta_gamma->Fill(s_dcaxy,s_pt,ww2*ww*0.2*1/(1.21361-1));
      if(s_iso==1)dcapt_pi0_gamma->Fill(s_dcaxy,s_pt,ww2*ww*(1-0.2)*1/(1.21361-1));
      if(s_iso==1)dcapt_all->Fill(s_dcaxy,s_pt,ww2*ww*(1+1/(1.21361-1)));
      if(s_iso==1)dcapt_fit->Fill(s_dcaxy,s_pt,ww2*(1+1/(1.21361-1)));
  ...
  pair_dca->GetYaxis()->SetRangeUser(2e-7, 3e-1);
  pair_dca->Draw();
  pair_dca1->Draw("same hist");
  pair_dca2->Draw("same hist");
  cdca->SaveAs("plot_yj_v2/Pair_DCA_TrackType.pdf");
  //============================================================================
  
  TFile f("root_yj/DCA_Shapes.root","RECREATE");
  ...
  mass_gamma->GetYaxis()->SetRangeUser(5e-6, 5e-1);
  ...
  leg111->Draw("same");
  TLatex lat333;
  lat333.DrawLatex(0.1,1e-1,"0.6<#it{p}_{T} [GeV]<8.5"); // 没显示出来，估计是位置没放对
  hmass->SaveAs("plot_yj_v2/PhoE_Mass_All.pdf");
  //============================================================================
  
  TCanvas *hmass1 = new TCanvas("hmass1","PhoE Mass");
  mass_all->GetYaxis()->SetRangeUser(0.6,8.5);//(mass, pt)
  h111->GetYaxis()->SetRangeUser(0.6,8.5);
  ...
  leg3->Draw("same");
  TLatex lat444;
  lat444.SetTextAlign(13);
  lat444.DrawLatex(0.005,0.12,"0.6<#it{p}_{T} [GeV]<8.5");
  hmass1->SaveAs("plot_yj_v2/PhoE_Mass.pdf");
  //============================================================================
  ...
  dr3->Rebin(10);
  hdr->Rebin(10);
  dr3->GetYaxis()->SetRangeUser(2e-4,2e-1);
  dr3->GetXaxis()->SetRangeUser(1,20);
  norm(dr3);
  norm(hdr);
  ...
  fline->Draw("same");
  leg2->Draw("same");
  c232->SaveAs("plot_yj_v2/Iso_Eff.pdf");
  //============================================================================
  ...
  comp_gamma->Draw("same hist");
  comp_pi0->Draw("same hist");
  comp_eta->Draw("same hist");
  comp_pi0_gamma->Draw("same hist");
  comp_eta_gamma->Draw("same hist");
  leg11->Draw("same");
  fline->Draw("same");
  c22->SaveAs("plot_yj_v2/PhoE_Composition.pdf");
  //============================================================================
  ...
  ptrat->Draw("PE");
  fline->Draw("same");
  c5->SaveAs("plot_yj_v2/PhoE_Pt_Comp.pdf"); // zyj
  //=========================================================================
  ...
  c11->cd(10);
  leg22->Draw();
  c11->SaveAs("plot_yj_v2/PhoE_DCA3d_notLog.pdf");
