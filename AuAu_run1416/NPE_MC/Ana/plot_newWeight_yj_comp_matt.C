...
void plot_newWeight_yj(int save = 1, int weight_data=1){
  ...
  mass_all->GetYaxis()->SetRangeUser(0.6,8.5);// pt range0-2
  ...
  leg111->Draw("same");
  TLatex lat333;
  lat333.SetTextAlign(13); // zyj add
  lat333.DrawLatex(12,10e-2,"0.6<#it{p}_{T} [GeV]<8.5"); // 没显示出来，估计是位置没放对
  ...
  lat444.DrawLatex(12,10e-2,"0.6<#it{p}_{T} [GeV]<8.5");
  ...
  sprintf(label,"#it{p}_{T}#in [%1.1f,%1.1f] GeV",0.6,8.5);
  TLatex lat232;
  lat232.DrawLatex(-2.6,1.18,label);
