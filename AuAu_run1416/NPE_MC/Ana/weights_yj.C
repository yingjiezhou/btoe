...
void weights_yj(){
  ...
  TFile * f= new TFile("root/NewWeights.root");
 ...
 // norm(gen_pi0);
  ...
  for(int i = 1; i< write_gamma->GetNbinsX()+1;i++){
    ...
    write_gamma->SetBinContent(i,1./2./3.14*temp1/temp2/1.4/write_gamma->GetBinWidth(i));
    write_gamma->SetBinError(i,0);//1./2./3.14*temp11/temp2/127442/1.4/write_gamma->GetBinWidth(i));
    write_pi0->SetBinContent(i,1./2./3.14*temp3/temp4/1.4/write_pi0->GetBinWidth(i));
  ...
  cout<<fpi->Integral(0,20)<<": is the integral of xxxxxx"<<endl;
  write_pi0->Scale(fpi->Integral(0,20)*2*3.14*1.4);
  ...
  ggamma->Draw("APE"); // fit from data point zyj
  fgamma->Draw("l same"); // fit func
  write_gamma->Scale(fgamma->Integral(0,20)*2*3.14*1.4); // 0825, fgamma 要norm 就是等价于write_gamma 乘上积分再乘以这些个常数，你自己想想就知道了
  write_gamma->Draw("same PE"); // weighted hist, get from, non-sense here, inconsistent
  pad22->cd();
  for(int i = 0; i<24;i++){
  ...
  fratio->Draw("L");
  ratio->Draw("same"); // will be Nan, nonsense here
  //return ;
  TCanvas *c4w =  new TCanvas ("c4w","gamma_ratio");
  grat->GetYaxis()->SetRangeUser(0,2);
  grat->GetYaxis()->SetTitle("#gamma^{inclusive}/#gamma^{hadron}");
  grat->Fit("pol0");
  grat->Draw("APE");
  
  // gen and fit gen fun draw same
  TCanvas *c4 = new TCanvas("c4","embeding gen gamma");
  gen_gamma->GetXaxis()->SetTitle("p_{T} [GeV]");
  gen_gamma->Fit(fgammamc);
  gen_gamma->Draw("PE");
  fgammamc->Draw("l same");
  
  TCanvas *c44 = new TCanvas("c44","embeding gen pi0");
  gen_pi0->GetXaxis()->SetTitle("p_{T} [GeV]");
  gen_pi0->Fit(fpimc);
  ...
  // gen fit and weight fit draw same
  // 这里是转换成1/pTdpT之后再做weight，我得到的是没有norm 的，因为我是从new_weights哪里得到的。 但是matt 从gen_full 里面读取，说不定已经norm 了. 或者不norm，到时候weight 的时候乘上两者之间的比值即可
  ...
  TFile fi("root_yj/Pi0_Gamma_Ratio.root","RECREATE");
  ratio->Write("ratio");
  pi0_weights->Write();
  D0_weights->Write();
  gamma_weights->Write();
