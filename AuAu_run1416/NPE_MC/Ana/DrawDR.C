...
void DrawDR(){
    ...
    TFile *f = new TFile("./root_yj/DCA_plots_yj.root");
    ...
    sprintf(label,"%1.1f<#it{p}_{T}<%1.1f GeV/#it{c}",low_pt,high_pt);
    TLatex lat2;

    TLegend *leg = new TLegend(0.2,0.22,0.5,0.3);
    leg->AddEntry(h11_0,"Data","lp");
    leg->AddEntry(dr_0,"Embedding","lp");

    TCanvas *c5 = new TCanvas("c5","c5");
    c5->SetLogy();
    dr_0->GetYaxis()->SetRangeUser(1e-3, 1e0);
    //dr_0->GetXaxis()->SetTitle("e^{+}e^{-} vertex transverse position");
    dr_0->GetXaxis()->SetTitle("Decay Radius (cm)");
    dr_0->GetYaxis()->SetTitle("Arb. Units");
    dr_0->Draw("PE");
    h11_0->Draw("same PE");
    lat2.DrawLatex(5, 0.2,label);
    //leg->Draw("same");
    return ;
