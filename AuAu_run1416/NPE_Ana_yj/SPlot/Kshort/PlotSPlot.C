...
//////////////////////////////////////////////////////////////////
    ...
    auto *ch1 = (TChain *)f_D->Get("Ks_tree_sw");
    ...
    TH2F *hBKG = new TH2F("hBKG","hBKG",200,-1,1,400,0.45,0.55); hBKG->Sumw2();
  ...
  //===================================================================================
    ...
    ch1->SetBranchAddress("tag_p",&p_tag);
	...
	if(fabs(nsigpi)<2)isPion=1;
	if(isPion)hpass->Fill(pt,sw);
	if(!isPion)hfail->Fill(pt,sw);
	hBKG->Fill((p-p_tag)/(p+p_tag),mass);
	if(beta!=0){
	...
	if(fabs(mass-0.4976)<0.01){ // zyj mass peak within 3sigma??
	    hnsige_s->Fill(nsige);
	    hnsigpi_s->Fill(nsigpi);
	    hnsige_p_s->Fill(p,nsige);
	    hpt_s->Fill(pt);
	    if(emc0>0){
		hemc0_s->Fill(p/emc0);
		hzdist_s->Fill(zdist);
		hphidist_s->Fill(phidist);
	    }
	    if(nphi>0||neta>0){
		hneta_s->Fill(neta);
		hnphi_s->Fill(nphi);
	    ...
	    if(beta!=0){
		hbeta_s->Fill((1/beta - 1/(p / TMath::Sqrt(p*p+0.13957018*0.13957018)))/ (1/beta));
		hbetae_s->Fill((1/beta - 1/(p / TMath::Sqrt(p*p+0.00051099907*0.00051099907)))/ (1/beta));
		hbeta_p_s->Fill(p,1/beta);
	    }
	}
      
	if((mass<0.4976-0.02 && mass>0.4976-0.03) || (mass>0.4976+0.02 && mass<0.4976+0.03)){ // zyj side-bind range
            hnsige_sb->Fill(nsige);
            hnsigpi_sb->Fill(nsigpi);
            hnsige_p_sb->Fill(p,nsige);
            hpt_sb->Fill(pt);
            if(emc0>0){
                hemc0_sb->Fill(p/emc0);
                hzdist_sb->Fill(zdist);
                hphidist_sb->Fill(phidist);
            }
            if(nphi>0||neta>0){
                hneta_sb->Fill(neta);
                hnphi_sb->Fill(nphi);
            ...
            if(beta!=0){
                hbeta_sb->Fill((1/beta - 1/(p / TMath::Sqrt(p*p+0.13957018*0.13957018)))/ (1/beta));
                hbetae_sb->Fill((1/beta - 1/(p / TMath::Sqrt(p*p+0.00051099907*0.00051099907)))/ (1/beta));
                hbeta_p_sb->Fill(p,1/beta);
            }
        }
    }
    ...
    hbetaeff->Draw();
    TCanvas *c999 = new TCanvas("c999","beta var");
    hBKG->GetZaxis()->SetRangeUser(1,800);
    hBKG->Draw("COLZ");
