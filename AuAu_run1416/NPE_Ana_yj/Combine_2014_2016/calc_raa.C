...
//proper asym method  
    ...
    for(int i= 5 ; i<9; i++){
        ...
        if(tmp1>0 && tmp2>0){
            ...
            ratio->SetBinContent(i+1,val);
	    ratio->SetBinError(i+1,error_st);
	    ratio_sys->SetPoint(cnt,fb->GetBinCenter(i+1),val);
            ratio_sys->SetPointError(cnt,0,0,error_sy1,error_sy2);
            cnt++;
	    double null = null_hyp_bin->GetBinContent(i);
	    double nuller = null_hyp_biner->GetBinContent(i);
	    double null_er = fabs(null-nuller);
	    summ+=(val-null)*(val-null)/sqrt(error_st*error_st+error_sy2*error_sy2+null_er*null_er);
...
*/   
    ...
    null_hyp->SetFillColor(kBlue);
    null_hyp->SetFillStyle(3004);
    ...
    nulll->SetLineColor(kBlue+1);
    nulll->SetFillColor(kBlue);
    nulll->SetFillStyle(3144);
...
//    fit->Draw("same l");
    ...
    TLegend *leg111 = new TLegend(0.47,0.8,0.65,0.93);
    
    leg111->AddEntry(null_hyp,"Null hyp.","l");
    leg111->Draw("same");
    cout << "chi2 for ratio " << summ << " " << summ/4 << endl;
}
