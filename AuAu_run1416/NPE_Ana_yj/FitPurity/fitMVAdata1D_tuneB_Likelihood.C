...
{// int daughter,int mag are not used here
  gSystem->Load("libRooFit.so"); // zyj moved here
  RooMsgService::instance().setGlobalKillBelow(RooFit::ERROR);
  /////Minuit Options
  bool hesse = true;
  bool minos = false;
  
  double _start = -7,_end = 7;
  if(pt>0){
    _start = -5;
    _end = 5;//13;
  }
  ...
  RooRealVar mean11("mean11","mean11",0.9,-1.,1);
  RooRealVar sigma1("sigma1","sigma1",8.35581e-01,0.1,1.2);
  RooRealVar sigma11("sigma11","sigma11",1,0.1,1.2);
  RooRealVar sigma111("sigma111","sigma111",1,0.1,1.2);
  RooBifurGauss g1_2("g1_2","g1_2",*PID,mean11,sigma11,sigma111);
  RooGaussian g1_1("g1_1","g1_1",*PID,mean1,sigma1);
  RooRealVar frac1("frac1","frac1",0.3,0.0,1);
  
  
  RooAbsPdf *g1;
  if(pt==1)g1 = new RooAddPdf("g1","g1",g1_1,g1_2,frac1); // zyj made this, for root 6
  else  g1 = new RooBifurGauss("g1","g1",*PID,mean1,sigma1,sigma11);//,sigma11

  
  //if(pt==1)RooAddPdf g1("g1","g1",g1_1,g1_2,frac1);
  //else RooBifurGauss g1("g1","g1",*PID,mean1,sigma1,sigma11);//,sigma11
  
  RooRealVar mean2("mean2","mean2",-2.5,-2,-1);
  RooRealVar sigma2("sigma2","sigma2",0.1,0.001,5.);
  RooRealVar exp2("exp2","exp2",-0.00001,-100,100);
  RooRealVar exp22("exp22","exp22",-0.00001,-10,10);
  RooRealVar exp222("exp222","exp222",0.00001,-1,1);
  RooRealVar mean2_2("mean2_2","mean2_2",-2.,-7.,0.);
  RooRealVar sigma2_2("sigma2_2","sigma_2",0.4,0.01,1.);
  ...
  RooGaussian g2_2("g2_2","g2_2",*PID,mean2_2,sigma2_2); // zyj
  //if(pt<3)RooBifurGauss g2_2("g2_2","g2_2",*PID,mean2_2,sigma2_2,sigma2_22);
  RooGaussian g2("g2","g2",*PID,mean2,sigma2);
  //RooExponential g2("g2","g2",*PID,exp2);
  //RooChebychev g2("g2","g2",*PID,RooArgList(exp2,exp22,exp222));
  ...
  RooRealVar pi_signal_yield("pi_signal_yield","pi_signal_yield",0,0,1000000000);//data_pi->Integral()/0.00201634,data_pi->Integral()/0.00201634*0.0025
  ...
  sigma111.setRange(sigma111.getVal()-sigma111.getError(),sigma111.getVal()+sigma111.getError());
  ...
  exp2.setRange(exp2.getVal()-exp2.getError(),exp2.getVal()+exp2.getError());
  exp22.setRange(exp22.getVal()-exp22.getError(),exp22.getVal()+exp22.getError());
  exp222.setRange(exp222.getVal()-exp222.getError(),exp222.getVal()+exp222.getError());
  PID->setRange("range_2pi",2.,6);
  ...
  RooAbsReal *nllData = completePDF.createNLL(*dataHist,Extended(kTRUE),Range("range"));// zyj contruct likelihood function
  ...
  RooPlot *PIDFrame = PID->frame(); // zyj here is the final hist will draw and save
  dataHist->plotOn(PIDFrame,Name("HistM")); // zyj inc e
  ...
  if(1)dataHist1->plotOn(PIDFrame,Name("HistM1"),MarkerColor(kGray),LineColor(kGray)); // zyj just draw, nothing to do
  dataHist->plotOn(PIDFrame,Name("HistM")); // zyj draw again??
  //=================================================
  ...
  cout << "====================" << endl;
  for(double i=0; i<30; i++){ // zyj here, we can use diff purity calculation range!!!
  ...
  if(pt!=0)prettycanvasAll2(c1,PIDFrame,pullPlot,0);
