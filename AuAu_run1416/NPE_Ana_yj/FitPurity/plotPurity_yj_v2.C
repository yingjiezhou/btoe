void plotPurity_yj_v2(int pass=5){
  ...
  for(int i =1;i<10;i++){
    //double pe = (hp1->GetBinContent(i))/hp_k->GetBinContent(i);
  ...
  hpe->GetYaxis()->SetTitle("Hadron Contamination");
  hpe->GetYaxis()->SetRangeUser(0,1);
  hpe->Draw("PE");
  /*
  ...
  leg->AddEntry(hp1,"Rectangular Cuts","PLE"); // this is from Matt
  ...
  pad2->cd();
  hpe->GetXaxis()->SetTitle("#it{p}_{T} GeV/#it{c}");
  hpe->GetYaxis()->SetTitle("#frac{Rectangular}{Likelihood}");
  hpe->SetTitle(0);
  hpe->GetYaxis()->SetTitleSize(0.15);
  hpe->GetYaxis()->SetLabelSize(0.1);
  hpe->GetYaxis()->CenterTitle();
  hpe->GetYaxis()->SetTitleOffset(0.3);
  hpe->GetYaxis()->SetNdivisions(505);
  hpe->GetYaxis()->SetRangeUser(0, 10);
  hpe->GetXaxis()->SetTitleSize(0.3);
  hpe->GetXaxis()->SetLabelSize(0.15);
  ...
  //TF1 *fline = new TF1("fline","5",0,10);
  //fline->SetLineStyle(7);
  //fline->Draw("same");
  */
  c2->SaveAs("purity_comparsion_QM_v2.pdf");
  return ;
  //=============================================================================
