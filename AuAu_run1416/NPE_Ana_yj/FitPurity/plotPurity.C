void plotPurity(int pass=5){
  ...
  for(int i =1;i<10;i++){
    he->SetBinContent(i,hy11->GetBinContent(i)/hy1->GetBinContent(i)); // zyj likelihood effi
  ...
  hy11->GetYaxis()->SetTitle("Electron Yield"); //zyj after likelihood cut
  //hy1->Draw();
  hy11->Draw("same");
  //=============================================================================
  ...
  TLegend *leg = new TLegend(0.2,0.65,0.4,0.90);
  hp1->SetMarkerStyle(25);
  leg->AddEntry(hp1,"Rectangular Cuts","PLE"); // this is from Matt
  leg->AddEntry(hp_k,"Additional Likelihood Decision","PEL"); // this is from Kunsu????, should be calculated?? from formular??
  
  hp11->Draw("same PE");// purity after likelihood cut???
  hp1->GetYaxis()->SetRangeUser(0,1);
  hp1->GetYaxis()->SetTitle("Hadron Fraction"); //zyj  purity before likelihood cut, from Matt
