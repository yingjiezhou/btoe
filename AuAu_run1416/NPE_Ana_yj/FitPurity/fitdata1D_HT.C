...
//Signals
    ...
    RooRealVar sigma1("sigma1","sigma1",1,0,3.2);  
    ...
    RooRealVar sigma2("sigma2","sigma2",1,0.1,5.5);    
    RooRealVar sigma22("sigma22","sigma22",1.,0.1,5.);
    RooRealVar sigma222("sigma222","sigma222",1.,0.1,5.);
    RooRealVar frac2("frac2","frac2",0.5,0.1,1);
    ...
    if(pt>2 && pt<6)RooAddPdf g2("g2","g2",g2_1,g2_2,frac2);
    else RooBifurGauss g2("g2","g2",*PID,mean2,sigma2,sigma22);

    RooRealVar mean3("mean3","mean3",-5,-7,2);
    RooRealVar mean33("mean33","mean33",-5,-7,2);
    RooRealVar sigma3("sigma3","sigma3",1,0.0.1,5.5);    
    RooRealVar sigma33("sigma33","sigma33",1.,0.1,5.);
    ...
    if(pt>4 && pt<8)RooAddPdf g3("g3","g3",g3_1,g3_2,frac3);
    else RooBifurGauss g3("g3","g3",*PID,mean3,sigma3,sigma33);

    RooRealVar mean4("mean4","mean4",-5,-9,2);
    RooRealVar mean44("mean44","mean44",-5,-9,2);
    ...
    RooRealVar frac4("frac4","frac4",0.8,0.,1);
    RooBifurGauss g4_1("g4_1","g4_1",*PID,mean4,sigma4,sigma44);
    RooGaussian g4_2("g4_2","g4_2",*PID,mean44,sigma444);
    if(pt<8)RooAddPdf g4("g4","g4",g4_1,g4_2,frac4);
    else RooBifurGauss g4("g4","g4",*PID,mean4,sigma4,sigma44);
...
//Background
    ...
    if(pt>3||pt==0)shapeList.add(k_signal);
    ...
    if(pt>3||pt==0)yieldList.add(k_signal_yield);
...
//Now fit Data
    ...
    if(pt==1) PID->setRange("range_pi",-10,-1);
    ...
    if(pt>5) PID->setRange("range_pi",-7,3);
    if(pt>7) PID->setRange("range_pi",-4.5,3);
    //if(pt==6) PID->setRange("range_pi",-8,3);
    ...
    if(pt!=2) sigma2.setRange(sigma2.getVal()-sigma2.getError(),sigma2.getVal()+sigma2.getError());
    if(pt!=2)sigma22.setRange(sigma22.getVal()-sigma22.getError(),sigma22.getVal()+sigma22.getError());
    sigma222.setRange(sigma222.getVal()-sigma222.getError(),sigma222.getVal()+sigma222.getError());
    if(pt!=2)mean2.setRange(mean2.getVal()-mean2.getError(),mean2.getVal()+mean2.getError());
...
//Now fit All Data
    ...
    completePDF.plotOn(PIDFrame,Components(RooArgSet(e_signal)),LineStyle(7),LineColor(kGreen-2),FillColor(kGreen-2));
    ...
    completePDF.plotOn(PIDFrame,Components(RooArgSet(k_signal)),LineStyle(6),LineColor(kYellow-2),FillColor(kGreen-2));
...
//Objects for saving yields and so forth
    ...
    if (save==1){
        ...
        if(output1.is_open()){
            output1 <<  pt <<" " <<  yields[0] <<" "<< yields_errors[0] << " " << pur1  << " " << eff << " " << yields[1] << " " << yields_errors[1] << "\n";
            output1.close();
