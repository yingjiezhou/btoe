...
void run2DMVAFit(int rs=1,int save=0, int doAllFits=0)
{
  ...
  for(int pt=ptStart;pt<ptEnd+1;pt++){
    ...
    TFile *f_D1 = new TFile(dFile1);// zyj not used here, can comment
    
    h01_2D = (TH2F*) f_D->Get("hFit_3"); // MB, MVA>mvaCut
    h01_2D1 = (TH2F*) f_D->Get("hFit_0"); // MB, no MVA cut
    ...
    h01_2D_mva = (TH2F*) f_D->Get("hFit2D_0"); // MB, no MVA cut, (pt,MVADec)
    ...
    if(pt>0){
      ...
      h01_2D_p_mva->GetXaxis()->SetRangeUser(binning[pt-1],binning[pt]);// I think this two are same
