...
void runFit_sWeight_yj(int save=1){
  ...
  if(pt>0){
    ...
    sprintf(cut_bkg,"probe_beta>0 && probe_nsige<0 && probe_emc0>0 && probe_bkgcat==1 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]); // probe_bkgcat
    sprintf(cut_bkg1,"probe_beta>0 && probe_nsige<0 && probe_emc0>0 && probe_bkgcat==1 && probe_pt>%1.1f && probe_pt<%1.1f  && abs(probe_nsigpi)<2 ",binning[pt-1],binning[pt]);
  }
  ...
  char *file = "root_yj/weights_electron.root"; // pt ==0
  char *file_pion = "root_yj/weights_pion.root";
  
  if(pt == 2) {
    file = "root_yj/weights_electron_highPt_B10.root";
    file_pion = "root_yj/weights_pion_highPt_B10.root";
  }
  
  if(pt == 1) {
    file = "root_yj/weights_electron_lowPt.root";
    file_pion = "root_yj/weights_pion_lowPt.root";
  }
  ...
  //sprintf(dFile,"/Volumes/YJMAC/Matt_production/NPE_Tuples2.root"); // not ok, since is already disrupted
  ...
  sprintf(dFile1,"../SPlot/Kshort/root_yj/out.root"); //这里相当于是同时对pi 和 kaon 同时进行并行的得到likelihood
  //sprintf(dFile1,"../SPlot/Kshort_lowPT/out.root"); //这里相当于是同时对pi 和 kaon 同时进行并行的得到likelihood
  //sprintf(dFile1,"../SPlot/Kshort/all.root"); //这里相当于是同时对pi 和 kaon 同时进行并行的得到likelihood
...
void doLikelihood(){
  ...
  TFile file("root_yj/Ks_pion.root","RECREATE");
