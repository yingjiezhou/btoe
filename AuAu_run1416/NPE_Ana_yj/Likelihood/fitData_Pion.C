...
void fitdata1D_pion(TH1F* hpovere,TH1F* hnsige,TH1F* hdeltabeta,TH1F* hzdist, TH1F* hphidist,int save,char *outputfile,int pt,double ptLow ,double ptHigh)
{
  ...
  //Signals var1  ////////////////
  RooRealVar mean1_povere("mean1_povere","mean1_povere",2.5,1,8);
  RooRealVar mean2_povere("mean2_povere","mean2_povere",2.5,1,8);
  RooRealVar mean3_povere("mean3_povere","mean3_povere",2.5,1,8);
  RooRealVar sigma1_povere("sigma1_povere","sigma1_povere",1,0.1,3.2); //2.2
  RooRealVar sigma11_povere("sigma11_povere","sigma11_povere",1,0.1,3.2); //4.2
  
  RooRealVar sigma2_povere("sigma2_povere","sigma2_povere",1,0.1,3.2);
  RooRealVar sigma22_povere("sigma22_povere","sigma22_povere",1,0.1,3.2);
  RooRealVar sigma3_povere("sigma3_povere","sigma3_povere",1,0.1,3.2);
  RooRealVar sigma33_povere("sigma33_povere","sigma33_povere",1,0.1,3.2);
  /*
  RooRealVar alpha1("alpha1","alpha1",-1,-10,10);
  RooRealVar n1("n1","n1",1,0,2);
  RooRealVar frac1("frac1","frac1",0.7,0,1);
  RooRealVar frac2("frac2","frac2",0.7,0,1);
  if(0)RooGaussian g1_g("g1_g","g1_g",*povere,mean2_povere,sigma2_povere);
  //RooCBShape g1_cb("g1_cb","g1_cb",*povere,mean1_povere,sigma1_povere,alpha1,n1);
  RooBifurGauss g1_cb("g1_cb","g1_cb",*povere,mean1_povere,sigma1_povere,sigma11_povere); // zyj modified
  
  RooBifurGauss g1_g("g1_g","g1_g",*povere,mean2_povere,sigma2_povere,sigma22_povere);
  RooBifurGauss g2_g("g2_g","g2_g",*povere,mean3_povere,sigma3_povere,sigma33_povere); //g1_g= g2_g
  
  RooAddPdf g1_povere("g1_povere","g1_povere",g1_cb,g1_g,frac1);
  //RooAddPdf g1_povere("g1_povere","g1_povere",gp_povere,g2_g,frac2);
  */
  RooRealVar alpha1("alpha1","alpha1",-1,-10,10);
  RooRealVar n1("n1","n1",1,0,2);
  RooRealVar frac1("frac1","frac1",0.7,0,1);
  ...
  RooArgSet *freeparam_povere = completePDF_povere.getParameters(dataHist_povere);
  int numfreeparam_povere = (freeparam_povere->selectByAttrib("Constant",kFALSE))->getSize();
  ...
  RooArgSet *freeparam_nsige = completePDF_nsige.getParameters(dataHist_nsige);
  int numfreeparam_nsige = (freeparam_nsige->selectByAttrib("Constant",kFALSE))->getSize();
  ...
  RooRealVar *sigma1_deltabeta;
  RooRealVar *sigma2_deltabeta;
  RooRealVar *sigma22_deltabeta;
  
  RooAbsPdf *g1_deltabeta;
  RooAbsPdf *g1_1_deltabeta;

  RooAbsPdf *g1_2_deltabeta;
  RooRealVar *frac1_deltabeta;
  
  if(pt==2){
    sigma1_deltabeta = new RooRealVar("sigma1_deltabeta","sigma1_deltabeta",0.01,0.000001,1);
    sigma22_deltabeta = new RooRealVar("sigma22_deltabeta","sigma22_deltabeta",0.01,0.000001,1); //0.01
    g1_1_deltabeta = new RooBifurGauss("g1_1_deltabeta","g1_1_deltabeta",*deltabeta,mean1_deltabeta,*sigma1_deltabeta,*sigma22_deltabeta);

    sigma2_deltabeta = new RooRealVar("sigma2_deltabeta","sigma2_deltabeta",0.01,0.000001,1);
    g1_2_deltabeta = new RooGaussian("g1_2_deltabeta","g1_2_deltabeta",*deltabeta,mean1_deltabeta,*sigma2_deltabeta); // g1_2_deltabeta 虽然只在这里面用到，但是不能只在里面定义，否则会break，只能都在外面定义
    frac1_deltabeta = new RooRealVar("frac1_deltabeta","frac1_deltabeta",0.5,0,1); //0.6
    g1_deltabeta = new RooAddPdf("g1_deltabeta","g1_deltabeta",*g1_1_deltabeta,*g1_2_deltabeta,*frac1_deltabeta);
  }
  if(pt!=2){
    sigma1_deltabeta = new RooRealVar("sigma1_deltabeta","sigma1_deltabeta",0.001,0.0001,1); //0.001
    sigma2_deltabeta = new RooRealVar("sigma2_deltabeta","sigma2_deltabeta",0.001,0.0001,1);
    g1_deltabeta = new RooBifurGauss("g1_deltabeta","g1_deltabeta",*deltabeta,mean1_deltabeta,*sigma1_deltabeta,*sigma2_deltabeta);
  }

  /*
  if(pt!=2)sigma1_deltabeta = new RooRealVar("sigma1_deltabeta","sigma1_deltabeta",0.001,0.0001,1);
  if(pt!=2)sigma2_deltabeta = new RooRealVar("sigma2_deltabeta","sigma2_deltabeta",0.001,0.0001,1);
  if(pt==2) sigma1_deltabeta = new RooRealVar("sigma1_deltabeta","sigma1_deltabeta",0.01,0.000001,1);
  if(pt==2) sigma2_deltabeta = new RooRealVar("sigma2_deltabeta","sigma2_deltabeta",0.001,0.0001,1);

  if(pt==2)sigma22_deltabeta = new RooRealVar("sigma22_deltabeta","sigma22_deltabeta",0.01,0.000001,1);
  if(pt!=2)g1_deltabeta = new RooBifurGauss("g1_deltabeta","g1_deltabeta",*deltabeta,mean1_deltabeta,*sigma1_deltabeta,*sigma2_deltabeta);
  if(pt==2) g1_1_deltabeta = new RooBifurGauss("g1_1_deltabeta","g1_1_deltabeta",*deltabeta,mean1_deltabeta,*sigma1_deltabeta,*sigma22_deltabeta);
  RooGaussian g1_2_deltabeta("g1_2_deltabeta","g1_2_deltabeta",*deltabeta,mean1_deltabeta,*sigma2_deltabeta);
  RooRealVar frac1_deltabeta("frac1_deltabeta","frac1_deltabeta",0.6,0,1);
  if(pt==2)g1_deltabeta = new RooAddPdf ("g1_deltabeta","g1_deltabeta",*g1_1_deltabeta,g1_2_deltabeta,frac1_deltabeta);
  */
  ...
  deltabeta->setRange("range_deltabeta",-0.05,0.05); // zyj beta range changed, notice that x axis is 1e-3
  ...
  m_deltabeta.migrad();
  //RooPlot *Frame_deltabeta = deltabeta->frame(-0.05,0.05);
  ...
  RooArgSet *freeparam_deltabeta = completePDF_deltabeta.getParameters(dataHist_deltabeta);
  int numfreeparam_deltabeta = (freeparam_deltabeta->selectByAttrib("Constant",kFALSE))->getSize();
  ...
  RooRealVar frac1_zdist("frac1_zdist","frac1_zdist",0.5,0,1);
  RooRealVar frac2_zdist("frac2_zdist","frac2_zdist",0.5,0,1);
  ...
  RooArgSet *freeparam_zdist = completePDF_zdist.getParameters(dataHist_zdist);
  int numfreeparam_zdist = (freeparam_zdist->selectByAttrib("Constant",kFALSE))->getSize();
  ...
  RooRealVar frac1_phidist("frac1_phidist","frac1_phidist",0.2,0,1); //0.5 这个变小之后，对sigma 值有影响，一开始sigma 太小，导致拟合偏向左边，想要提高mean 对应的y 值就减小sigma
  RooRealVar frac2_phidist("frac2_phidist","frac2_phidist",0.9,0,1); //0.5
  ...
  RooArgSet *freeparam_phidist = completePDF_phidist.getParameters(dataHist_phidist);
  int numfreeparam_phidist = (freeparam_phidist->selectByAttrib("Constant",kFALSE))->getSize();
  ...
  if(save && pt==0){
    c_povere->SaveAs("plots_yj/Pion_POverE_Fit.pdf");
    c_nsige->SaveAs("plots_yj/Pion_NSigmapi_Fit.pdf");
    c_deltabeta->SaveAs("plots_yj/Pion_DeltaBeta_Fit.pdf");
    c_zdist->SaveAs("plots_yj/Pion_ZDist_Fit.pdf");
    c_phidist->SaveAs("plots_yj/Pion_PhiDist_Fit.pdf");
  ...
  if(save && pt==1){
    c_povere->SaveAs("plots_yj/Pion_POverE_Fit_lowPt.pdf");
    c_nsige->SaveAs("plots_yj/Pion_NSigmae_Fit_lowPt.pdf");
    c_deltabeta->SaveAs("plots_yj/Pion_DeltaBeta_Fit_lowPt.pdf");
    c_zdist->SaveAs("plots_yj/Pion_ZDist_Fit_lowPt.pdf");
    c_phidist->SaveAs("plots_yj/Pion_PhiDist_Fit_lowPt.pdf");
    TFile f(outputfile,"RECREATE");
    weight_povere->Write("pion_weight_povere");
    weight_nsige->Write("pion_weight_nsigpi");
    weight_deltabeta->Write("pion_weight_deltabeta");
  ...
  if(save && pt==2){
    c_povere->SaveAs("plots_yj/Pion_POverE_Fit_highPt_B10.pdf");
    c_nsige->SaveAs("plots_yj/Pion_NSigmae_Fit_highPt_B10.pdf");
    c_deltabeta->SaveAs("plots_yj/Pion_DeltaBeta_Fit_highPt_B10.pdf");
    c_zdist->SaveAs("plots_yj/Pion_ZDist_Fit_highPt_B10.pdf");
    c_phidist->SaveAs("plots_yj/Pion_PhiDist_Fit_highPt_B10.pdf");
    TFile f(outputfile,"RECREATE");
    weight_povere->Write("pion_weight_povere");
    weight_nsige->Write("pion_weight_nsigpi");
    weight_deltabeta->Write("pion_weight_deltabeta");
