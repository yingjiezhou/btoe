...
void fitdata1D(TH1F* hpovere,TH1F* hnsige,TH1F* hdeltabeta,TH1F* hzdist, TH1F* hphidist,int save,char *outputfile,int pt,double ptLow ,double ptHigh)
{
  ...
  RooRealVar frac1("frac1","frac1",0.5,0,1); //0.5 0813 fraction越小，sigma 越大，越平坦
  ...
  RooAddPdf g1_povere("g1_povere","g1_povere",g1_cb,g1_g,frac1);// zyj final function, 概率密度函数
  ...
  RooRealVar sigma1_deltabeta("sigma1_deltabeta","sigma1_deltabeta",0.01,0.0001,2);
  RooRealVar sigma11_deltabeta("sigma11_deltabeta","sigma1_1deltabeta",0.01,0.0001,2);
  RooRealVar sigma2_deltabeta("sigma2_deltabeta","sigma2_deltabeta",0.01,0.0001,2);
  RooBifurGauss g1_1_deltabeta("g1_1_deltabeta","g1_1_deltabeta",*deltabeta,mean1_deltabeta,sigma1_deltabeta,sigma11_deltabeta);
  RooGaussian g1_2_deltabeta("g1_2_deltabeta","g1_2_deltabeta",*deltabeta,mean2_deltabeta,sigma2_deltabeta);
  RooRealVar frac1_deltabeta("frac1_deltabeta","frac1_deltabeta",0.9,0,1); //0.8 0813
  ...
  RooRealVar sigma1_zdist("sigma1_zdist","sigma1_zdist",5,0.2,15);
  RooRealVar sigma2_zdist("sigma2_zdist","sigma2_zdist",5,0.2,15);
  RooRealVar sigma3_zdist("sigma3_zdist","sigma3_zdist",5,0.2,15);
  ...
  RooAddPdf *g2_1_zdist;
  RooAddPdf *g1_zdist;
  if(0){
    g2_1_zdist = new RooAddPdf("g2_1_zdist","g2_1_zdist",g1_1_zdist,g1_2_zdist,frac1_zdist);
    g1_zdist = new RooAddPdf("g1_zdist","g1_zdist",*g2_1_zdist,g1_3_zdist,frac2_zdist);
  }
  if(1) g1_zdist = new RooAddPdf("g1_zdist","g1_zdist",g1_1_zdist,g1_2_zdist,frac1_zdist);
  ...
  RooRealVar sigma3_phidist("sigma3_phidist","sigma3_phidist",0.01,0.0001,0.1);
  ...
  RooRealVar frac2_phidist("frac2_phidist","frac2_phidist",0.8,0,1); //0814
  RooAbsPdf *g1_phidist; //zyj
  RooAbsPdf *g2_1_phidist;
  if(1){
    g2_1_phidist = new RooAddPdf("g2_1_phidist","g2_1_phidist",g1_1_phidist,g1_2_phidist,frac1_phidist);
    g1_phidist = new RooAddPdf("g1_phidist","g1_phidist",*g2_1_phidist,g1_3_phidist,frac2_phidist);
  }
  if(0)g1_phidist = new RooAddPdf("g1_phidist","g1_phidist",g1_1_phidist,g1_2_phidist,frac1_phidist);
  ...
  RooMinuit m_phidist (*nll_phidist);// zyj 根据给定dataset， 以及带参数的函数，求最大似然函数的极值，求导最小， Minuiti就是求函数的极小值，你只需要给定函数即可
  m_phidist.setVerbose(kFALSE);
  m_phidist.migrad();
  RooPlot *Frame_phidist = phidist->frame();
  dataHist_phidist->plotOn(Frame_phidist,Name("Hist_phidist"));
  completePDF_phidist.plotOn(Frame_phidist,Name("Curve_phidist")); // 这个是你觉得的函数分布，可以因此而求导得到概率密度函数，从而log 得到似然函数，再求导得到最大似然函数，再求极值
  ...
  weight_phidist->GetYaxis()->SetTitle("Likelihood"); //表示从likelihood 得到的变量分布么？
  ...
  if(save && pt==0){
    c_povere->SaveAs("plots_yj/Electron_POverE_Fit.pdf");
    c_nsige->SaveAs("plots_yj/Electron_NSigmae_Fit.pdf");
    c_deltabeta->SaveAs("plots_yj/Electron_DeltaBeta_Fit.pdf");
    c_zdist->SaveAs("plots_yj/Electron_ZDist_Fit.pdf");
    c_phidist->SaveAs("plots_yj/Electron_PhiDist_Fit.pdf");
  ...
  if(save && pt==1){
    c_povere->SaveAs("plots_yj/Electron_POverE_Fit_lowPt.pdf");
    c_nsige->SaveAs("plots_yj/Electron_NSigmae_Fit_lowPt.pdf");
    c_deltabeta->SaveAs("plots_yj/Electron_DeltaBeta_Fit_lowPt.pdf");
    c_zdist->SaveAs("plots_yj/Electron_ZDist_Fit_lowPt.pdf");
    c_phidist->SaveAs("plots_yj/Electron_PhiDist_Fit_lowPt.pdf");
    TFile f(outputfile,"RECREATE");
    weight_povere->Write("electron_weight_povere");
    weight_nsige->Write("electron_weight_nsige");
    weight_deltabeta->Write("electron_weight_deltabeta");
  ...
  if(save && pt==2){
    c_povere->SaveAs("plots_yj/Electron_POverE_Fit_highPt_B10_TuneCut.pdf"); // should be named in _highpT???? zyj
    c_nsige->SaveAs("plots_yj/Electron_NSigmae_Fit_highPt_B10_TuneCut.pdf");
    c_deltabeta->SaveAs("plots_yj/Electron_DeltaBeta_Fit_highPt_B10_TuneCut.pdf");
    c_zdist->SaveAs("plots_yj/Electron_ZDist_Fit_highPt_B10_TuneCut.pdf");
    c_phidist->SaveAs("plots_yj/Electron_PhiDist_Fit_highPt_B10_TuneCut.pdf");
    TFile f(outputfile,"RECREATE");
    weight_povere->Write("electron_weight_povere");
    weight_nsige->Write("electron_weight_nsige");
    weight_deltabeta->Write("electron_weight_deltabeta");
