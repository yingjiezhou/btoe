...
void runFit_sWeight(int save=1){
  ...
  if(pt>0)sprintf(cut_s,"probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]); // zyj unlike sign, for pHe, is signal
  
  //if(pt>0)sprintf(cut_s,"probe_emc0>0",binning[pt-1],binning[pt]); // zyj unlike sign, for pHe, is signal
  ...
  if(pt>0)sprintf(cut_s_beta,"probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]); // zyj add beta cut
  ...
  // pt ==0, 就是说whole pt range
  if(pt==0)sprintf(cut_s,"probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",0.6,8.5); // zyj same with pt>0, cut_s_beta
  ...
  if(pt==0)sprintf(cut_bkg,"probe_beta>0 && probe_nsige<0 && probe_emc0>0 && probe_bkgcat==1 && probe_pt>%1.1f && probe_pt<%1.1f ",0.6,8.5); // zyj same with cut_bkg
  ...
  char file[100] = "root_yj/weights_electron.root";
  char file_pion[100] = "root_yj/weights_pion.root";
  //////////////////////////////////////////////////////////////////
  ...
  char dFile1[500];
  sprintf(dFile1,"../SPlot/Kshort/out.root"); //这里相当于是同时对pi 和 kaon 同时进行并行的得到likelihood
  ...
  TH1F* hSig_deltabeta = new TH1F("hSig_deltabeta","hSig_deltabeta",300,-0.05,0.05);
  ...
  //TTree* PhoE = (TTree*)f_D->Get("PhoE");
  //TTree* Ks_tree_sw = (TTree*)f_D1->Get("Ks_tree_sw");
  ...
  /*
   ...
   ch1->Add(dFile);
   TChain* ch2 = new TChain("Ks_tree_sw");
   ch2->Add(dFile1);
   */
  
  ch1->Project("hSig_povere","probe_p/probe_emc0",cut_s); // zyj test
  //ch1->Project("hSig_povere","probe_emc0",cut_s);
  ...
  //ch1->SetBranchAddress("probe_emc0",&emc0); // zyj
  ...
  for(int i =0;i<ch2->GetEntries();i++){
    ...
    //cout<<emc0<<"zyj jjjjjjjjjjjjjjjjjjj"<<endl; // zyj test
    
    if(emc0>0){
      //cout<<emc0<<"zyj cutuuuuuuuu jjjjjjjjjjjjjjjjjjj"<<endl;
