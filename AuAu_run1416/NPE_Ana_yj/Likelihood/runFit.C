...
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    char cut_bkg[500];
    char cut_bkg1[500];
    if(pt>0)sprintf(cut_s,"probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);
    if(pt>0)sprintf(cut_b,"probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge>0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);
...
//////////////////////////////////////////////////////////////////
    ...
    TH1F* hSig_deltabeta = new TH1F("hSig_deltabeta","hSig_deltabeta",50,-0.03,0.03);
    ...
    TH1F* hSig_deltabeta_ws = new TH1F("hSig_deltabeta_ws","hSig_deltabeta_ws",50,-0.03,0.03);
    ...
    if(pt==0||pt==1)TH1F* hBkg_deltabeta = new TH1F("hBkg_deltabeta","hBkg_deltabeta",50,-0.005,0.005);
...
void doLikelihood(){
    ...
    ch1 = (TChain*)f_D->Get("PhoE");
    ch2 = (TChain*)f_D->Get("BKG");
    //ch2 = (TChain*)f_D1->Get("BKG_highPT");
    ...
    TH1F* DLL_e = new TH1F("DLL_e","DLL_e",250,-5,5);
    TH1F* DLL_e_ws = new TH1F("DLL_e_ws","DLL_e_ws",250,-5,5);
    ...
    TH2F* DLL_e_pt = new TH2F("DLL_e_pt","DLL_e_pt",200,0,20,200,-4,4);
    TH2F* DLL_e_pt_ws = new TH2F("DLL_e_pt_ws","DLL_e_pt_ws",200,0,20,200,-4,4);
    TH2F* DLL_p_pt = new TH2F("DLL_p_pt","DLL_p_pt",200,0,20,200,-4,4);
    ...
    TH1F* DLL_p = new TH1F("DLL_p","DLL_p",250,-5,5);
    TH1F* DLL_e_nw = new TH1F("DLL_e_nw","DLL_e_nw",250,-5,5);
    TH1F* DLL_e_ws_nw = new TH1F("DLL_e_ws_nw","DLL_e_ws_nw",250,-5,5);
    TH1F* DLL_p_nw = new TH1F("DLL_p_nw","DLL_p_nw",250,-5,5);

    TH1F* DLL_e_post = new TH1F("DLL_e_post","DLL_e_post",250,-5,5);
    TH1F* DLL_e_ws_post = new TH1F("DLL_e_ws_post","DLL_e_ws_post",250,-5,5);
    TH1F* DLL_p_post = new TH1F("DLL_p_post","DLL_p_post",250,-5,5);
    TH1F* DLL_pr_post = new TH1F("DLL_pr_post","DLL_pr_post",250,-5,5);
    TH1F* DLL_k_post = new TH1F("DLL_k_post","DLL_k_post",250,-5,5);
    TH2F* DLL_e_2D = new TH2F("DLL_e_2D","DLL_e_2D",200,0,20,400,-4,4); // what we will use
    ...
    TH2F* DLL_pr_2D = new TH2F("DLL_pr_2D","DLL_pr_2D",200,0,20,400,-4,4);
    TH1F* DLL_2p = new TH1F("DLL_2p","DLL_2p",1000,-4,4);
    TH2F* DLL_2p_2D = new TH2F("DLL_2p_2D","DLL_2p_2D",200,0,20,400,-4,4);
    TH1F* DLL_k = new TH1F("DLL_k","DLL_k",1000,-4,4);
    TH2F* DLL_k_2D = new TH2F("DLL_k_2D","DLL_k_2D",200,0,20,400,-4,4);
    ...
    TH1F* eff_e = new TH1F("eff_e","eff_e",250,-5,5);
    TH1F* sign = new TH1F("sign ","sign ",250,-5,5);
    TH1F* line = new TH1F("line","line",50,-13,40);
    TH1F* eff_p = new TH1F("eff_p","eff_p",250,-5,5);

    TH1F* ROC = new TH1F("ROC","ROC",100,0.99,1);
    ...
    for(int i=0; i<ch1->GetEntries();i++){ // zyj for sig
	if((i%10000==0)){
	    double inte = i;
	...
	double dll_nw = TMath::Log(weight1+weight2+weight3+weight4+weight5) - TMath::Log(weight11+weight22+weight33+weight44+weight55); //zyj  _nw means non wieght
	if(dll>300 || dll<-300)dll=2; //zyj  随便取都可以？ 取成3呢，是不是在3的地方有个峰
	////////////////////DLL
	...
	if(probe_charge*tag_charge<0){
	    ...
	    DLL_e->Fill(dll);
	    if(fabs(1/beta-1)<0.025 && p/emc0>0.3 && p/emc0<1.5)DLL_e_post->Fill(dll); //经过cut 之后，？？
	    DLL_e_nsig->Fill(nsige,dll);
	    DLL_e_pt->Fill(pt,dll);
	    DLL_e_pt->Fill(nsige,dll);
	...
	if(probe_charge*tag_charge>0){
	    ...
	    DLL_e_ws->Fill(dll);
	    if(fabs(1/beta-1)<0.025 && p/emc0>0.3 && p/emc0<1.5)DLL_e_ws_post->Fill(dll);
    ...
    int iLoop = 1000000;
    for(int i=1; i<iLoop;i++){// zyj bkg corr and Dell
	if((i%100000==0)){
	    double inte = i;
	...
	if(dll>300 || dll<-300)dll=2;
	if(bkgcat==1){
	    DLL_p->Fill(dll);
	    if(fabs(1/beta-1)<0.025 && p/emc0>0.3 && p/emc0<1.5)DLL_p_post->Fill(dll);
	    DLL_p_nsig->Fill(nsige,dll);
	    DLL_p_pt->Fill(pt,dll);
	    DLL_p_nw->Fill(dll_nw);
  ...
  // zyj for overlap regin
	...
	Float_t zdist=0;
	Float_t nsige=0;
      Float_t nsigpi=0; // zyj why define again??? even if in {} no need to define again
	Float_t beta=0;
	Float_t emc0=0;
	Float_t probe_charge=0;
	Float_t tag_charge=0;
  ...
  //=====================================================================================
    ...
    canv2->cd(1);
    electron_weight_povere->Draw("hist"); // zyj likelihood dist
  ...
  //=====================================================================================
	...
	if(temp1>0 && temp2>0){
	    double temp = temp1/DLL_e->Integral(1,max)*1/sqrt(temp1) / sqrt(temp1/DLL_e->Integral(1,max)*1/sqrt(temp1)+67.8*temp2/DLL_p->Integral(1,max)*1/sqrt(temp2)); // zyj ???
    ...
    leg->Draw("same");
  //=====================================================================================
   // zyj save as DLL_All.pdf I think,
    ...
    DLL_e_post->GetXaxis()->SetTitle("#Delta#it{LL}(e-#pi)");
  ...
  //=====================================================================================
  // zyj eff as a function of nsigma_e

    for(int i = 1;i<DLL_e_nsig->GetNbinsX();i++){
	double low = -10 + (i-1)*15/DLL_e_nsig->GetNbinsX(); // （15， -10， 5）就是为了从（1，15） 转换成（-10， 5）， 因为i 只能从正数开始
	...
	htemp1 = (TH1F*) DLL_e_nsig->ProjectionY(); // zyj 二维直方图投影，也就是取不同的nsigmae 区间
	htemp2 = (TH1F*) DLL_p_nsig->ProjectionY();
	ZERO(htemp1);
	int bin1 = htemp1->FindBin(0.0);
	
	if(htemp1->Integral()>0 && i>5){// 因为
	    nsig_eff_e->SetBinContent(i,htemp1->Integral(bin1,-1)/htemp1->Integral());// 这里取的是DLL 大于0 的eff， 下面的canvas 是分了多种情况的cut
  ...
  //=====================================================================================
   // zyj eff as a function of pt， also consider diff MVA cut
	...
	bin1 = htemp1->FindBin(0.8); // zyj 选取不同的response cut，对应的PHE 的 cut eff 不一样
  ...
  //=====================================================================================
    ...
    line->Draw("same hist");
    leg11->Draw("same");
    lat.DrawLatex(2,0.5,"#Delta#it{LL}(e-#pi)>0");
  ...
  //=====================================================================================
    ...
    nsig_eff_e->GetYaxis()->SetTitle("#Delta#it{LL}(e-#pi) Efficiency");
    ...
    lat.DrawLatex(2,0.5,"#Delta#it{LL}(e-#pi)>0");
    leg11->Draw("same");
    TCanvas *canv7 = new TCanvas ("canv7","corr",800,400);
    canv7->Divide(2,1);
    canv7->cd(1);
    DLL_e_nsige->Draw("COLZ");
    canv7->cd(2);
    DLL_p_nsige->Draw("COLZ");
    TCanvas *canv77 = new TCanvas ("canv77","corr",800,400);
    canv77->Divide(2,1);
    canv77->cd(1);
    DLL_e_pt->Draw("COLZ");
    canv77->cd(2);
...
void doLikelihood_highPT(){
    gROOT->ProcessLine(".x ~/myStyle.C");
    gStyle->SetPalette(56);
    TGaxis::SetMaxDigits(4);
    int const numPtBins=4;
    double binning[numPtBins+1]={2.5,3.5,4.5,5.5,8.5};
    TFile *file_e = new TFile("./root/weights_electron_highpt.root");
    TFile *file_p = new TFile("./root/weights_pion_highpt.root");
    electron_weight_povere=(TH1F*)file_e->Get("electron_weight_povere");
    electron_weight_nsige=(TH1F*)file_e->Get("electron_weight_nsige");
    electron_weight_deltabeta=(TH1F*)file_e->Get("electron_weight_deltabeta");
    electron_weight_zdist=(TH1F*)file_e->Get("electron_weight_zdist");
    ...
    TH1F* eff_e = new TH1F("eff_e","eff_e",250,-5,5);
    TH1F* sign = new TH1F("sign ","sign ",250,-5,5);
    TH1F* line = new TH1F("line","line",100,-5,40);
    TH1F* eff_p = new TH1F("eff_p","eff_p",250,-5,5);
	...
	if(probe_charge*tag_charge<0){
	    ...
	    e_corr_beta_povere->Fill(dbeta,p/emc0);
	    DLL_e->Fill(dll);
	    if(fabs(1/beta-1)<0.025 && p/emc0>0.3 && p/emc0<1.5)DLL_e_post->Fill(dll);
	    DLL_e_nsige->Fill(nsige,dll);
	    DLL_e_pt->Fill(pt,dll);
	    DLL_e_nw->Fill(dll_nw);
	    DLL_e_2D->Fill(pt,dll);
	    LL_ee->Fill((weight1*weight2*weight3*weight4*weight5));// ？？ 这个是干嘛的，
	    LL_ep->Fill((weight11*weight22*weight33*weight44*weight55));
	    electron_povere->Fill(p/emc0);
	    electron_nsige->Fill(nsige);
	    electron_nsigpi->Fill(nsigpi);
    ...
    Float_t bkgcat=0;
    ch2 -> SetBranchAddress( "probe_bkgcat" , &bkgcat );
    int iLoop = ch2->GetEntries();//10000000;
	...
	if(!(pt>2.5))continue; 
	//if(!(bkgcat==1))continue;
	if(!(nsige<1))continue; // ？？？ 这么要求，不是成了e 么，对于真正的inc 来说，本来的背景就是被误判为e 的pi，用pure pion 只是得到pion_weight_povere， 现在这一步骤的目的是为了将inc e 和被误判的e 区分开，所以要求是这个条件？
	double dbeta = (1/beta - 1/(p / TMath::Sqrt(p*p+0.13957018*0.13957018)))/ (1/beta);
	double dbeta_e = (1/beta - 1/(p / TMath::Sqrt(p*p+0.00051099907*0.00051099907)))/ (1/beta);
	/////////////////////DLL
	int bin1 = electron_weight_povere->FindBin(p/emc0);
 ...
 //=====================================================================
  // 人为的取不同的weight，对每个成分
    ...
    double binLow2  = 0.;
    double binHigh2 = 1.;
    double binStep2 = (binHigh2 - binLow2)/numBins2;
    TH2F* hOverlap = new TH2F("hOverlap","hOverlap",numBins,binLow+binStep/2.,binHigh+binStep/2.,numBins2,binLow2+binStep2/2.,binHigh2+binStep2/2.);
    //hOverlap->GetYaxis()->SetTitle("e-#pi overlap");
    //hOverlap->GetXaxis()->SetTitle("w_{#it{p}/E}");
    //hOverlap->GetYaxis()->SetTitle("w_{n#sigma_{e}}");
    //hOverlap->GetXaxis()->SetTitle("w_{#Delta(1/#beta)/(1/#beta)}");
    hOverlap->GetXaxis()->SetTitle("w_{#Delta#it{z}}");
    hOverlap->GetYaxis()->SetTitle("w_{#Delta#phi}");
    w1=2.;
    w2=3.1;
    w3=3.7;
    w4=0.4;
    ...
    cout << "Scanning from " << binLow+binStep << " to " << binHigh << " in steps of " << binStep << endl;
    /*for(int loop=1;loop<numBins+1;loop++){
	for(int loop2=1;loop2<numBins2+1;loop2++){
	    w4 = binLow+binStep*(loop);
	    w5 = binLow2+binStep2*(loop2);
	    cout << "On loop number " << loop  << " " << loop2 <<  " :Scanning weight " << binLow+binStep*(loop)<< " " << binLow2+binStep2*(loop2) << endl;
	    Float_t pt=0;
	    Float_t p=0;
	    Float_t nphi=0;
	    Float_t neta=0;
	    ...
	    hOverlap->SetBinContent(loop,loop2,overlap);
	    //hOverlap->SetBinError(loop,loop2,overlap*sqrt(1/norm1+1/norm2));	
	    tDLL_e->Delete();
	    tDLL_p->Delete();
	    tDLL_e_ws->Delete();
	}
  ...
  //=====================================================================
    ...
    DLL_e_nsige->Add(DLL_e_nsige_ws,-1);
    DLL_e_pt->Add(DLL_e_pt_ws,-1);
    e_corr_nsige_beta->Add(e_corr_nsige_beta_ws,-1);
    e_corr_nsige_povere->Add(e_corr_nsige_povere_ws,-1);
    e_corr_povere_zdist->Add(e_corr_povere_zdist_ws,-1);
    ...
    leg->AddEntry(DLL_p,"Pions","L");
  //=====================================================================
  // Comparing electrons and pions for likelihood dist
    TCanvas *canv2 = new TCanvas ("canv2","weights",1600,800);
    canv2->Divide(4,2);
    pion_weight_povere->SetLineColor(kRed);
    pion_weight_nsigpi->SetLineColor(kRed);
  ...
  //=====================================================================
    ...
    line->Draw("same hist");
    //leg11->Draw("same");
    lat.DrawLatex(3.5,0.5,"#Delta#it{LL}(e-#pi)>0");
    // zyj add
  canv5->SaveAs("DLLEff_0.pdf"); // I guess is this pdf
  
    TCanvas *canv55 = new TCanvas ("canv55","DLL");
    pt_eff_e->GetYaxis()->SetTitle("#Delta#it{LL}(e-#pi) Efficiency");
    pt_eff_e->GetYaxis()->SetRangeUser(0,1.1); //  zyj attention this range
    pt_eff_e->GetXaxis()->SetTitle("#it{p}_{T}");
    pt_eff_e->Draw("PE");
    pt_eff_p->Draw("same PE");
    line->Draw("same hist");
  ...
  canv55->SaveAs("DLLEff_All_Opt");// zyj add, some pdf like this
  
  // zyj below are the corr histo, 2D
    e_corr_nsige_beta->GetYaxis()->SetTitle("#Delta(1/#beta)/(1/#beta)");
    e_corr_nsige_beta->GetXaxis()->SetTitle("n#sigma_{e}");
    e_corr_nsige_povere->GetYaxis()->SetTitle("#it{p}/E");
    e_corr_nsige_povere->GetXaxis()->SetTitle("n#sigma_{e}"); 
