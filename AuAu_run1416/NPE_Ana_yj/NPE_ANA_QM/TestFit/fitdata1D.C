...
//Now fit All Data
   ...
   TPaveText* txt1 = new TPaveText(0.56,0.55,0.9,0.71,"BRNDC");
    ...
    if(1){  
        sprintf(ptbin,"#it{p}_{T} #in [%1.1f,%1.1f] GeV/#it{c}",ptLow,ptHigh);
...
/*    for(int i = 0; i<10;i++){
	double inti = -2.8;
	double endi = -2.2;
	double width =fabs(inti-endi)/100.;
	DCA->setRange("range1",-2.4,-2);
	RooAbsReal* e1_1 = g1.createIntegral(*DCA,*DCA,"range1");
	RooAbsReal* had1_1 = g2.createIntegral(*DCA,*DCA,"range1");
	RooAbsReal* b_1 = gb.createIntegral(*DCA,*DCA,"range1");
	RooAbsReal* c_1 = gc.createIntegral(*DCA,*DCA,"range1");
	...
	cout << "Range " << inti+width*i << " " << endi << " " << c_int*yields[3] /(e_int*yields[0]+ h_int*yields[1] + b_int*yields[2] + c_int*yields[3]) << endl;
