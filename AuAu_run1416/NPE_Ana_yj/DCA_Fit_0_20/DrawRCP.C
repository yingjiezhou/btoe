...
///////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(temp1>0 && temp3>0){
	    hrat->SetBinContent(i,(1/temp3-1)/(1/temp1-1));
	    hrat->SetBinError(i, (1/temp3-1)/(1/temp1-1)*sqrt(temp11*temp11/temp1/temp1/(temp1-1)/(temp1-1)+temp33*temp33/temp3/temp3/(temp3-1)/(temp3-1)));
	    
	}
	if(temp2>0 && temp3>0){
	    hrat1->SetBinContent(i,(1/temp3-1)/(1/temp2-1));
	    hrat1->SetBinError(i,(1/temp3-1)/(1/temp2-1)*sqrt(temp22*temp22/temp2/temp2/(temp2-1)/(temp2-1)+temp33*temp33/temp3/temp3/(temp3-1)/(temp3-1)));	 	    
	}
	if(temp1>0 && temp1>0){
	    hrat2->SetBinContent(i,(1/temp2-1)/(1/temp1-1));
	    hrat2->SetBinError(i,(1/temp2-1)/(1/temp1-1)*sqrt(temp22*temp22/temp2/temp2/(temp2-1)/(temp2-1)+temp11*temp11/temp1/temp1/(temp1-1)/(temp1-1)));	 	    
	}
    }
    ...
    TLegend *legg = new TLegend(0.17,0.6,0.52,0.8);
    hrat->Fit("fit");
    hrat1->Fit("fit1");
    hrat2->Fit("fit2");
    TLegend *legg1 = new TLegend(0.17,0.7,0.52,0.8);
    ...
    sprintf(label,"0-20%: %1.2f #pm% 1.2f",vall,errr);
    sprintf(label1,"20-40%: %1.2f #pm% 1.2f",vall1,errr1);
    sprintf(label2,"0-20%: %1.2f #pm% 1.2f",vall2,errr2);
    legg->AddEntry(fit,label,"l");
    legg->AddEntry(fit1,label1,"l");
    legg1->AddEntry(fit2,label2,"l");
    ...
    hrat2->GetYaxis()->SetTitle("R_{CP}^{#it{b}#rightarrow#it{e}}/R_{CP}^{#it{c}#rightarrow#it{e}} (/20-40%)");    
    hrat2->GetXaxis()->SetRangeUser(0,8.5);
    hrat2->GetYaxis()->SetRangeUser(0,4);  
    ...
    fb1->GetXaxis()->SetTitle("#it{p}_{T} (GeV/#it{c})");
