               ...
               int pt,int eta, int rs,float cutLow,float cutHigh,double ptLow ,double ptHigh,double eff_b, int pur_dir, int HT, double e_const)
{
  ...
  //cout<<"zyjjjjjjjjjjjjjjjjjjkkkkkkkkkkkkkkk"<<dir1<<endl;
  //return ;
  ...
  if(purData.is_open()){
    while(!purData.eof()){
      ...
      int b=y_e;
      if(pT!=0){
        hp1->SetBinContent(pT,p1-0.15); // zyj purity - 15% j/psi
  ...
  cout<<b_int <<c_int<<e_int<<h_int<<endl;
