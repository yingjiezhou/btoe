void plot_npe(){
    TFile *d0_res = new TFile("/project/projectdirs/starprod/rnc/mkelsey/NPE_MC/Ana/v2_hists/D_v2.root");
    ...
    double binning1[numPtBins+1]={0.6,1+0.1,1.2+0.1,1.5+0.1,2.0+0.1,2.5+0.1,3.5+0.1,4.5+0.1,5.5+0.1,8.5+0.1};
    double binning2[numPtBins+1]={0.6,1-0.1,1.2-0.1,1.5-0.1,2.0-0.1,2.5-0.1,3.5-0.1,4.5-0.1,5.5-0.1,8.5-0.1};
    double b;
    double b_e;
    double c;
    double c_e;
    ...
    getData(hb,hb_sys,"./98/yields.dat");
    getData(hc,hc_sys,"./99/yields.dat");
    getData(hb1,hb_sys1,"./7/yields.dat");//tight dca
    getData(hc1,hc_sys1,"./8/yields.dat");//tight dca
    ...
    hc->SetLineColor(kGray+1);
    hc->SetMarkerColor(kGray+1);
    ...
    hc->GetXaxis()->SetTitle("#it{p}_{T} [GeV]");
    hc->GetYaxis()->SetTitle("#it{v}_{2}");//(#it{b}#rightarrow#it{e})");
    hc->Draw("E1 X0");
    hc_sys->SetFillColor(kGray);
    //g3_sys->Draw("same E2");
    //gp_sys->Draw("same E2");    
    ...
    gs4->Draw("same pe");
    D0V2->Draw("same pe");
    TLegend *leg2 = new TLegend(0.25,0.6,0.4,0.8);
    ...
    leg2->AddEntry(hb,"This Analysis v_{2}{EP} Min. Bias","PLE");
    leg2->AddEntry(hc,"This Analysis v_{2}{EP} High Tower","PLE");
    leg2->Draw("same");
    gPad->RedrawAxis();
