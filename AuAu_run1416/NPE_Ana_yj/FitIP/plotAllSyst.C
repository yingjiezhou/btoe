...
void plotAllSyst(int savefrac = 1){
...
0.0992};
 ...
 // DUKE
    ...
    hb_t->Scale(2*3e+08/(41./152.)/hc_t->Integral());//*9.5711E-03);//hb_t->Integral()/hc_t->Integral());
    hb_t->Scale(0.7*0.6*0.8*0.1);
    hb_t1->Scale(0.7*0.6*0.8*43.9/1000*291);
...
//============================================================================================
    ...
    StatError = (TGraphAsymmErrors*)run12->Get("StatError");
    systError =(TGraphAsymmErrors*)run12->Get("systError");
    for(int i = 0;i<6;i++){
	double _x;double _y;
    ...
    TH1F *raa_b = new TH1F("raa_b","raa_b",numPtBins,binning);
    TH1F *raa_b_sys = new TH1F("raa_b_sys","raa_b_sys",numPtBins,binning);
    TH1F *raa_c = new TH1F("raa_c","raa_c",numPtBins,binning);
    TH1F *raa_c_sys = new TH1F("raa_c_sys","raa_c_sys",numPtBins,binning);
    ...
    /*getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,1,"./400/yields.dat",1,y_sys_mb);//Central 11
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,2,"./402/yields.dat",0,y_sys_mb);//phoe 1-pion
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,3,"./404/yields.dat",0,y_sys_mb);//phoe 1-gamma
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,4,"./406/yields.dat",0,y_sys_mb);// phoe 2
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,5,"./408/yields.dat",0,y_sys_mb);// pt spectra
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,6,"./414/yields.dat",0,y_sys_mb);// pt spectra
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,7,"./410/yields.dat",0,y_sys_mb);// more dp fragmetation
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,8,"./412/yields.dat",2,y_sys_mb);// less dp fragmetation
    */
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,1,"./500/yields.dat",1,y_sys_mb);//Central 11
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,2,"./502/yields.dat",0,y_sys_mb);//phoe 1-pion
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,3,"./504/yields.dat",0,y_sys_mb);//phoe 1-gamma
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,4,"./506/yields.dat",0,y_sys_mb);// phoe 2
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,5,"./508/yields.dat",0,y_sys_mb);// pt spectra
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,6,"./510/yields.dat",0,y_sys_mb);// pt spectra
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,7,"./512/yields.dat",0,y_sys_mb);// more dp fragmetation
    getData(h_yield_mb,h_yield_mb_sys,h1,frac,frac1,frac2,frac3,8,"./514/yields.dat",2,y_sys_mb);// less dp fragmetation
    ...
    leg4->AddEntry(hb_t,"[N(D^{0})/f_{D^{0}}]#times[#sigma(#it{pp}#rightarrow#it{b})/#sigma(#it{pp}#rightarrow#it{c})]#times#it{B}_{SL}#times#varepsilon(#it{e})","L"); 
    leg4->AddEntry(hb_t1,"FONLL#times#it{B}_{SL}#times#it{L}#times<N_{coll}>#times#varepsilon(#it{e})","L");   
...
//y_sys_ht->Draw("hist same");
   ...
   if(chol2.Decompose()){
    ...
    if(savefrac){
	TFile save("root/Fractions_Split_Low_sys2.root","RECREATE");
	frb->Write("bfrac");
	frc->Write("cfrac");
	frh->Write("hfrac");   
	fre->Write("efrac");
    ...
    if(1){
        TFile save("root/FracB.root","RECREATE");
        h_ave->Write("fb");
	...
	raa->Write("ratio");
	raa_sys->Write("ratio_sys");
	gDuke->Write("duke_ratio");
    }
