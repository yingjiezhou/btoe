               ...
               int pt,int eta, int rs,float cutLow,float cutHigh,double ptLow ,double ptHigh,double eff_b, int pur_dir, int HT)
{
  ...
  if(purData.is_open()){
    while(!purData.eof()){
      ...
      int b=y_e;
      if(pT!=0){
        if(pur_dir==3)p1*=corr[int(pT)-1]; // HT no likelihood, why need corr??
  ...
  fit->Constrain(0,0.0,1);  // leave lower at 0.05 for MB
  fit->Constrain(1,0.0,1);
  //fit->Constrain(2,1-purity-1e-3+0.1,1-purity+1e-3+0.1);
  fit->Constrain(2,0.0,1);
  fit->Constrain(3,0.0,1);
  ...
  if (status == 0) {
    ...
    TH1F* mc4 = (TH1F*)fit->GetMCPrediction(3);
    ...
    pad2->cd();
    TH1F *Pull = new TH1F("Pull","Pull",mc1->GetNbinsX(),-4,-1);
    Pull->GetXaxis()->SetTitle("log_{10}(|DCA|/cm)");
    for(int i = 1; i<mc1->GetNbinsX()+1;i++){
      int push = data->FindBin(-4);
      double check1 = data->GetBinCenter(i); // zyj not used here
