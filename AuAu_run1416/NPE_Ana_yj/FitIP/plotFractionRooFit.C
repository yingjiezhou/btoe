...
void plotFractionRooFit(int pass=1){
  ...
  char file[50];
  sprintf(file,"./%i/yields.dat",1111133); //mb 11
  ifstream data(file);
  char file1[50];
  sprintf(file1,"./%i/yields.dat",1); // ht
  ifstream data1(file1);
  
  TFile * f= new TFile("../../NPE_MC/Ana/root/root/HF_plots.root");
  ...
  leg1->Draw("same");
  c1->SaveAs("plots_yj/b_fraction_comp_11111333.pdf");
  ...
  */
  ...
  c2->SaveAs("plots_yj/NPE_PE_ratio_11111333.pdf");
  //h2_ht->Draw("same");
  ...
  h4->Draw("histe same");
  leg->Draw("same");
  c3->SaveAs("plots_yj/bfraction_11111333.pdf");
  ...
  */
  ...
  hc_t->Draw(" hist");
  hb_t->Draw("same hist");
  c4->SaveAs("plots_yj/pt_b_d_11111333.pdf");
