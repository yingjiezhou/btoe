void prettycanvasAllleg(TCanvas *c1,RooPlot *Frame, RooPlot *Pull, int log, double max)
{
    ...
    TPaveText *Name_pre = new TPaveText( 0.56,0.45, 0.9, 0.61,"BRNDC");
    Name_pre->AddText("STAR preliminary");
    Name_pre->SetFillColorAlpha(0, 0);
    Name_pre->SetTextAlign(12);
    Name_pre->SetBorderSize(0);
    Name_pre->Draw("same");
    ...
    Frame->Draw();
    auto leg = new TLegend(0.2,0.4,0.4,0.7);
