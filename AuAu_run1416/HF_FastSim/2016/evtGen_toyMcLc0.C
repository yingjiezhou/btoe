...
//int pidcode =511; //B0
//double mass = 5.27929;//
//std::string outFileName = "B0.toyMc0";

//int pidcode =521; //B+
//double mass = 5.2796;
//std::string outFileName = "Bp.toyMc0";

int pidcode =411; //D+
double mass = 1.86962;
std::string outFileName = "Dp.toyMc0";

//int pidcode =421; //D0
//double mass = 1.86484;
//std::string outFileName = "D0.toyMc0";
...
int jobindx;
void evtGen_toyMcLc0(int npart=10e6, int jobId=0) {
  ...
  for (int ipart = 0; ipart < npart; ipart++)
  {
    ...
    decayAndFill(pidcode,  b_d, fWeightFunction->Eval(b_d->Perp()), ptl);
    decayAndFill(-1*pidcode, b_d, fWeightFunction->Eval(b_d->Perp()), ptl);
...
void fill(int const kf, TLorentzVector* b, double weight,TLorentzVector const& eMom, TVector3 v00)
{
  ...
  // smear position
  int kch,pch,pich;
  if (kf > 0) {kch = 3; pch = 4; pich = 0;}
  else        {kch = 2; pch = 5; pich = 1;}
  TVector3 const eRPos = smearPosData(pich, vertex.z(), eRMom, v00);
  float const eDca = dcaSigned(eMom.Vect(), v00, vertex); // not used,
  float const eRDca = dcaSigned(eRMom.Vect(), eRPos, vertex); // 先对vertex 根据DCA进行
  float const eRSDca = dcaSigned(eRMom.Vect(), eRPos, vertex); // ??? not used,and same with eRDca
...
void bookObjects(std::string outFileName)
{
  ...
  TFile f("Momentum_resolution_FromHijing.root");
  fPionMomResolution = (TF1*)f.Get("fPionPlus_0")->Clone("fPion");
  fKaonMomResolution = (TF1*)f.Get("fKaonMinus_0")->Clone("fKaon");
  fProtonMomResolution = (TF1*)f.Get("fProton_0")->Clone("fProton");
  f.Close();
  ...
  TFile fHftRatio1("Out_hftRatio_AuAu2016_st_all_vz6_kpip_wch_global_hybrid223.root");
  TFile fDca1("Out_distsforfastsimu_2016_vz6_kpip_wch_global_hybrid223_all.root");
  
  char name[500];
  for (int ii = 0; ii < nCentHftRatio; ++ii)
  {
    sprintf(name,"hvz_c%d",ii);
    TH2F* hVzTemp = (TH2F*)fDca1.Get(name); sprintf(name,"hvz_zdcx_c%d",ii);
    h1Vz[ii]      = (TH1F*)hVzTemp->Clone(name);
  ...
  for (int iParticle = 0; iParticle < nParticles; ++iParticle)
  {
    for (int iEta = 0; iEta < nEtasHftRatio; ++iEta)
    {
      for (int iVz = 0; iVz < nVzsHftRatio; ++iVz)
      {
        ...
        {
          hHftRatio1[iParticle][iEta][iVz][iPhi] = (TH1D*)(fHftRatio1.Get(Form("hTrkRat_c%d_p%d_vz%d_eta%d_phi%d", centralitySelect, iParticle, iVz, iEta, iPhi)));
          hHftRatio1[iParticle][iEta][iVz][iPhi]->SetDirectory(0); // 只有不属于任何目录，在file close 的时候，才能不crash，否则在file close 之后，这个直方图也就不能用了
        }
      }
    }
    ...
    for (int iEta = 0; iEta < nEtasDca; ++iEta)
    {
      for (int iVz = 0; iVz < nVzsDca; ++iVz)
      {
        ...
        {
          h2Dca[iParticle][iEta][iVz][iPt] = (TH2D*)((fDca1.Get(Form("h_dcaXYvsZ_c%d_p%d_vz%d_eta%d_pt%d", centralitySelect, iParticle, iVz, iEta, iPt))));
  ...
  TFile fTpcEff("EffTpcEmbedd2016_pikp.root");
  
  for (int iCent = 0; iCent < nCentHftRatio; ++iCent)
  {
    hTpcPiPlus[iCent] = (TH1D*)fTpcEff.Get(Form("htpceff_c%d_p0", iCent));
    hTpcPiPlus[iCent]->SetDirectory(0);
    hTpcPiMinus[iCent] = (TH1D*)fTpcEff.Get(Form("htpceff_c%d_p1", iCent));
    hTpcPiMinus[iCent] ->SetDirectory(0);
    hTpcKPlus[iCent] = (TH1D*)fTpcEff.Get(Form("htpceff_c%d_p2", iCent));
    hTpcKPlus[iCent]->SetDirectory(0);
    hTpcKMinus[iCent] = (TH1D*)fTpcEff.Get(Form("htpceff_c%d_p3", iCent));
    hTpcKMinus[iCent]->SetDirectory(0);
    hTpcProton[iCent] = (TH1D*)fTpcEff.Get(Form("htpceff_c%d_p4", iCent));
    hTpcProton[iCent]->SetDirectory(0);
    hTpcAntiProton[iCent] = (TH1D*)fTpcEff.Get(Form("htpceff_c%d_p5", iCent));
  ...
  nt = new TNtuple("nt", "", "cent:vx:vy:vz:"
                   ...
                   "ePt:eEta:" // e Pion
                   "eRPt:eREta:eRDca:eRDcaXY:eRDcaZ:eTpc:eHft" // e Pion
                   //"pPt:pEta:" // MC Proton
...
void initEvtGen()
{
  ...
  starEvtGenDecayer->SetDecayTable("HFSemileptonicDecay.DEC");
