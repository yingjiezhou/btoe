...
void StarEvtGenDecayer::Decay(TString EvtGenParticleName, TLorentzVector* _p)
{
    LOG_INFO << "Decay EvtGen Particle=" << EvtGenParticleName << endm;

    // Clear the event from the last run
    ClearEvent();

    // Add the particle to the pythia stack
    AppendParticle(EvtGenParticleName, _p );
...
void StarEvtGenDecayer::AppendParticle(Int_t pdg, TLorentzVector* _p)
{
    ...
    if(mParticle->getSpinStates()==3) mParticle->setVectorSpinDensity();
    else mParticle->setDiagonalSpinDensity();
}

void StarEvtGenDecayer::AppendParticle(TString EvtGenParticleName, TLorentzVector* _p)
{
    // Append a particle to the stack to be decayed
    EvtVector4R p_init(_p->E(), _p->Px(), _p->Py(), _p->Pz());
    EvtId parentID = EvtPDL::getId(EvtGenParticleName.Data());
    mParticle = EvtParticleFactory::particleFactory(parentID, p_init);
    if(mParticle->getSpinStates()==3) mParticle->setVectorSpinDensity();
    else mParticle->setDiagonalSpinDensity();
}
