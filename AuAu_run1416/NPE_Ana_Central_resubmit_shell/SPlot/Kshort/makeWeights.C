...
void makeWeights()
{
  ...
  sprintf(dFile,"/Volumes/YJMAC/Matt_production/NPE_Tuples_TuneB10.root");//B4

  //sprintf(dFile,"../Ks.root");
  TFile *f_D = new TFile(dFile);
  TTree *t = (TTree *)f_D->Get("Ks");
...
void Write(RooWorkspace* ws, TTree* tree){
  TFile out("root_yj/out.root","RECREATE");
  RooDataSet* data = (RooDataSet*) ws->data("dataWithSWeights");
