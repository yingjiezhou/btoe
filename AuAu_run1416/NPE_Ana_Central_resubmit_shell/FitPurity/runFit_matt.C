...
void runFit_matt(int rs=1,int save=1, int doAllFits=1)
{
  /////////////////////////////////////////////////////////////////////////////////////////
  ...
  int ptStart=2;//Here, start from 1
  int etaEnd=etaStart;
  int ptEnd=ptStart;
  if(doAllFits){
    etaStart=0;
  ...
  for(int pt=ptStart;pt<ptEnd+1;pt++){ // with diff pt bin
    ...
    char dFile1[500];
    char dFile2[500];
    sprintf(dFile,"/Volumes/YJMAC/Matt_production/NPE_Tuples_Central5.root"); // I don't have this root file
    //sprintf(dFile,"production/NPE_Tuples_TuneCentral5.root"); // so I changed to use this

    // for pion
    //sprintf(dFile1,"../Likelihood/root/Ks_pion.root"); // sWeighted pion, all is from zyj 
    sprintf(dFile1,"../Likelihood/root_yj/Ks_pion_matt.root"); // sWeighted pion, all is from Matt, zyj 111/
