...
void runFit_sWeight_test(int save=1){
  /////////////////////////////////////////////////////////////////////////////////////////
  ...
  double binning[numPtBins+1]={0.6,2.0,8.5};
  
  char cut_s[500]; // s and b means unlike and like sign(wrong sign)
  char cut_b[500];
  
  char cut_s_beta[500]; //_beta means with beta cut
  char cut_b_beta[500];
  
  char cut_bkg[500];
  char cut_bkg1[500]; //zyj abs(probe_nsigpi)<2
  
  // zyj 看这样子，下面应该都是拿的PHE， 从root file 中取出来
  if(pt>0){
    //sprintf(cut_s,"probe_nsige<13 && probe_nsige>-4 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]); // 0814
    //sprintf(cut_b,"probe_nsige<13 && probe_nsige>-4 && probe_emc0>0 && probe_charge*tag_charge>0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);
    //sprintf(cut_s_beta,"probe_beta>0 && probe_nsige<13 && probe_nsige>-4 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);
    //sprintf(cut_b_beta,"probe_beta>0 && probe_nsige<13 && probe_nsige>-4 && probe_emc0>0 && probe_charge*tag_charge>0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);

    sprintf(cut_s,"probe_nsige<13  && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]); // 0814
    sprintf(cut_b,"probe_nsige<13  && probe_emc0>0 && probe_charge*tag_charge>0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);

    sprintf(cut_s_beta,"probe_beta>0 && probe_nsige<13  && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);
    sprintf(cut_b_beta,"probe_beta>0 && probe_nsige<13  && probe_emc0>0 && probe_charge*tag_charge>0 && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]);
    
    sprintf(cut_bkg,"probe_beta>0 && probe_nsige<0 && probe_emc0>0  && probe_pt>%1.1f && probe_pt<%1.1f ",binning[pt-1],binning[pt]); // probe_bkgcat
    sprintf(cut_bkg1,"probe_beta>0 && probe_nsige<0 && probe_emc0>0 && probe_pt>%1.1f && probe_pt<%1.1f  && abs(probe_nsigpi)<2 ",binning[pt-1],binning[pt]);
  }
  
  // pt ==0, whole pt range
  if(pt==0){ // always has beta cut???
    sprintf(cut_s,"probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",0.6,8.5);
    sprintf(cut_b,"probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge>0 && probe_pt>%1.1f && probe_pt<%1.1f ",0.6,8.5);
    sprintf(cut_s_beta,"probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge<0 && probe_pt>%1.1f && probe_pt<%1.1f ",0.6,8.5); // zyj same with pt>0, cut_s_beta
    sprintf(cut_b_beta,"probe_beta>0 && probe_nsige<13 && probe_emc0>0 && probe_charge*tag_charge>0 && probe_pt>%1.1f && probe_pt<%1.1f ",0.6,8.5); // should be beta???
    sprintf(cut_bkg,"probe_beta>0 && probe_nsige<0 && probe_emc0>0 && probe_bkgcat==1 && probe_pt>%1.1f && probe_pt<%1.1f ",0.6,8.5);
    sprintf(cut_bkg1,"probe_beta>0 && probe_nsige<0 && probe_emc0>0 && probe_bkgcat==1 && probe_pt>%1.1f && probe_pt<%1.1f && abs(probe_nsigpi)<2 ",0.6,8.5);
  }
  
  cout << "Using cuts " << cut_s << endl;
  char *file = "root/weights_electron.root"; // pt ==0
  char *file_pion = "root/weights_pion.root";
  
  if(pt == 2) {
    file = "root/weights_electron_highPt_B10.root";
    file_pion = "root/weights_pion_highPt_B10.root";
  }
  
  if(pt == 1) {
    file = "root/weights_electron_lowPt.root";
    file_pion = "root/weights_pion_lowPt.root";
  }
  ...
  sprintf(dFile1,"../SPlot/Kshort/root/out.root"); //这里相当于是同时对pi 和 kaon 同时进行并行的得到likelihood
  ...
  auto ch1 = (TChain*)f_D->Get("PhoE");
  auto ch2 = (TChain*)f_D1->Get("Ks_tree_sw");

  /*
  ch1->Project("hSig_povere","probe_p/probe_emc0",cut_s); // zyj test
  ...
  //unlike - like, ws mean like sign, which is wrong sign
  ...
  */
  ...
  TH1F* hBkg_mass = new TH1F("hBkg_mass","hBkg_mass",600, 0.4, 0.6);
  TH1F* hBkg_nsigpi = new TH1F("hBkg_nsigpi","hBkg_nsigpi",70,-13,13);
  TH1F* hBkg_deltabeta;
  //if(1)hBkg_deltabeta = new TH1F("hBkg_deltabeta","hBkg_deltabeta",30,-0.05,0.05);
  if(1)hBkg_deltabeta = new TH1F("hBkg_deltabeta","hBkg_deltabeta",50,0.0005,0.006);
  ...
  for(int i =0;i<ch2->GetEntries();i++){
    ...
    cout<<beta<<endl;
    hBkg_deltabeta->Fill(beta);
    //if(beta!=0) jjjy++;
    jjjy++;
    /*
    if(beta!=0){
      //hBkg_deltabeta->Fill((1/beta - 1/(p / TMath::Sqrt(p*p+0.00051099907*0.00051099907)))/ (1/beta),sw);
      hBkg_deltabeta->Fill((1/beta - 1/(p / TMath::Sqrt(p*p+0.00051099907*0.00051099907)))/ (1/beta));
    }
    */
    hBkg_mass->Fill(mass);
  }
  ...
  /*
  TCanvas *canv2 = new TCanvas ("canv2","weights",1500,800); // 这个图之前就应该画过
  canv2->Divide(3,2);
  canv2->cd(1);
  hBkg_povere->Draw();
  canv2->cd(2);
...
void doLikelihood(){
  ...
  sprintf(dFile,"/Volumes/YJMAC/Matt_production/NPE_Tuples_TuneA10.root");
  
  //sprintf(dFile1,"../SPlot/BKG.root");
  sprintf(dFile1,"../SPlot/Kshort/all.root"); // zyj ???
  TFile *f_D = new TFile(dFile);
  TFile *f_D1 = new TFile(dFile1);
  auto ch1 = (TChain*)f_D->Get("PhoE");
  auto ch2 = (TChain*)f_D1->Get("Ks_tree_sw");
  ...
  hFit_e_3->Add(hFit_e_WS_3,-1); // zyj add dll>0.45 cut
  hFit_e_3->Write();
  hFit_pi_3->Write();
  hFit_pi_4->Write();
  hFit_pi_5->Write();
  ...
  for(int loop=2;loop<numBins+1;loop++){
    ...
    ch2 -> SetBranchAddress( "probe_emc0" , &emc0 );
    Float_t bkgcat=0;
    ch2 -> SetBranchAddress( "probe_bkgcat" , &bkgcat ); // zyj not know
  ...
  DLL_e_2D->Add(DLL_e_ws_2D,-1); // zyj like-unlike
  ...
  //leg->Draw("same");
  canv2->SaveAs("plots_yj_try/Elec_pion_Probability.pdf");
  //========================================================================
  ...
  LL_pp->Draw(" PE same"); // zyj no need ???
  //========================================================================
  // eff as a function of dll cut
  
  TCanvas *canv4 = new TCanvas ("canv4","DLL cut eff");
  for(int i=1;i<DLL_e->GetNbinsX();i++){
    ...
    int bin = ROC->FindBin(temp1/DLL_e->Integral(1,max)); //eff
    double ttemp =  ROC->GetBinContent(bin);
    if(ttemp==0)ROC->SetBinContent(bin,1-temp2/DLL_p->Integral(1,max)); // rej
    if((1-temp2/DLL_p->Integral(1,max))<ttemp)ROC->SetBinContent(bin,1-temp2/DLL_p->Integral(1,max));// 取最小的rej
  ...
  for(int i=1;i<DLL_e->GetNbinsX();i++){
    ...
    if(temp1>0 && temp2>0){
      ...
      sign->SetBinContent(i,temp); // zyj sig
    }
  }
  ...
  line->Draw("same hist");
  leg->Draw("same");
  canv4->SaveAs("plots_yj_try/Likelihood_or_response_Cut_Effi.pdf");
  //========================================================================
  ...
  ROC->Draw("P");
  cpp->SaveAs("plots_yj_try/ROC.pdf");
  //========================================================================
  
  TCanvas *canvsig = new TCanvas ("canvsig","significance");
  sign->GetXaxis()->SetTitle("Likelihood Cut");
  sign->GetYaxis()->SetTitle("Significance");
  sign->Draw("P");
  canvsig->SaveAs("plots_yj_try/Signi.pdf");
  //========================================================================
  ...
  TCanvas *canv1 = new TCanvas ("canv1","DLL_comparsion");
  ...
  leg2->Draw("same");
  canv1->SaveAs("plots_yj_try/Likelihood_Response_Comparision.pdf");
  //========================================================================
  
  TCanvas *cp = new TCanvas("cp","post_DLL_comparsion");
  DLL_e_post->Draw(); // with DLL_e_post cut
  //    DLL_k_post->Draw("same");
  //    DLL_pr_post->Draw("same");
  DLL_p_post->Draw("same");
  leg2->Draw("same");
  cp->SaveAs("plots_yj_try/Likelihood_Response_Comparision_post.pdf");
  //========================================================================
  // sigma_e eff
  for(int i = 1;i<DLL_e_nsig->GetNbinsX();i++){
    ...
    auto htemp1 = (TH1F*) DLL_e_nsig->ProjectionY();
    auto htemp2 = (TH1F*) DLL_p_nsig->ProjectionY();
    ZERO(htemp1);
  ...
  //=========================================================================
  // likelihood eff as a func of pt, 1 for e, e for pion
  
  cout << "Electron eff for pt bins here " << endl;
  for(int i = 1;i<numPtBins+1;i++){ // zyj eff for different pt bin
    ...
    auto htemp1 = (TH1F*) DLL_e_2D->ProjectionY(); // 1 for e, e for pion
    auto htemp11 = (TH1F*) DLL_e_2D1->ProjectionY();
    auto htemp2 = (TH1F*) DLL_p_2D->ProjectionY();
    ZERO(htemp1);
    int bin1 = htemp1->FindBin(0.45);
    ...
    pt_eff_e->SetBinContent(i,htemp1->Integral(bin1,max)/htemp1->Integral()); // likelihood eff as a func of pt
    cout<<htemp1->Integral(bin1,max)/htemp1->Integral()<<" is the likelihood eff zyjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj"<<endl;
    ...
    pt_eff_r->SetBinContent(i,htemp1->Integral(bin1,max)/htemp1->Integral(bin2,max)); //    int bin2 = htemp1->FindBin(0.4); // ??? shoule be 0??
  ...
  /*
   ...
   TCanvas *canv5 = new TCanvas ("canv5","e_p Likelihood Efficiency");
   
   pt_eff_p->SetMarkerStyle(21);
   pt_eff_p->SetLineColor(kRed);
   pt_eff_p->SetMarkerColor(kRed);
   pt_eff_e->GetYaxis()->SetTitle("Likelihood Efficiency"); // zyj got for purity
   ...
   canv5->SaveAs("plots_yj_try/Likelihood_Efficiency_pt.pdf");
   //=========================================================================
   
   TCanvas *canv51 = new TCanvas ("canv51","DLL Rat");
   pt_eff_r->Draw("PE");
   canv51->SaveAs("plots_yj_try/DLL_Rat.pdf");
   //=========================================================================
   
   TCanvas *canv55 = new TCanvas ("canv55","e Likelihood Efficiency comp");
   ...
   leg1->AddEntry(pt_eff_e,"#Delta#it{LL}(e-#pi)>0.45","PEL");
   ...
   leg1->Draw("same");
   canv55->SaveAs("plots_yj_try/Likelihood_Efficiency_diff_MVACut.pdf");
   //=========================================================================
   ...
   TCanvas *canv6 = new TCanvas ("canv6","e_var corr",1200,800);
   canv6->Divide(3,2);
   canv6->cd(1);
   ...
   canv6->SaveAs("plots_yj_try/corr_electron.pdf");
   //=========================================================================
   ...
   hOverlap->Draw();
   canv9->SaveAs("plots_yj_try/overlap.pdf");
   //=========================================================================
   
   TCanvas *canv25 = new TCanvas ("canv52","DLL nsige effi");
   TLegend *leg12 = new TLegend(0.3,0.5,0.5,0.5);
   leg11->AddEntry(nsig_eff_e,"Electrons","PEL");
   leg11->AddEntry(nsig_eff_p,"Pions ","PEL");
   ...
   line->Draw("same hist");
   lat.DrawLatex(3,0.5,"Likelihood>0.45");
   leg12->Draw("same");
   canv25->SaveAs("plots_yj_try/Likelihood_Efficiency_Nsigma_e.pdf");
   //=========================================================================
   
   TCanvas *canv7 = new TCanvas ("canv7","nsige_dist",800,400);
   canv7->Divide(2,1);
   canv7->cd(1);
   DLL_e_nsig->Draw("COLZ");
   canv7->cd(2);
   DLL_p_nsig->Draw("COLZ");
   canv7->SaveAs("plots_yj_try/nsige_dll_dist.pdf");
   //=========================================================================
   TCanvas *canv77 = new TCanvas ("canv77","pt_dist",800,400);
   canv77->Divide(2,1);
   canv77->cd(1);
   DLL_e_pt->Draw("COLZ");
   canv77->cd(2);
   DLL_p_pt->Draw("COLZ");
   canv77->SaveAs("plots_yj_try/pt_dll_dist.pdf");
...
} // zyj 这个运行要用到前一个步骤产生的weight root file，产生之后就可以单独运行了
