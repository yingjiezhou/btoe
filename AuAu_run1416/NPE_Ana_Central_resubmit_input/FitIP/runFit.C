...
void runFit(int rs=1,int save=1, int doAllFits=1, int HT=0)
{
  ...
  //double binning[numPtBins]={0.6,1,1.2,1.5,2.0,2.5,3.5,4.5,8.5};
  int etaStart=0;//Here
  int etaEnd=etaStart;
  int ptStart=9;//Here
  int ptEnd=ptStart;
  ...
  //for(int pt=ptStart;pt<ptStart+1;pt++){ //zyj test
    ...
    sprintf(dFile5,"/Volumes/YJMAC/Matt_production/NPE_Tuples_Central5.root"); 

    //sprintf(dFile,"../TestArea/test.root");
    TFile *f_D = new TFile(dFile);
    TFile *f_D5 = new TFile(dFile5);
    ...
    sprintf(dFile2,"../../NPE_MC/Ana/root_yj/HF_plots_C5_yj_Qing.root"); // updated D and Lc/D0 ratio
    //sprintf(dFile2,"../../NPE_MC/Ana/root_yj/HF_plots_C5_yj_lowDp_Qing.root"); // updated D and Lc/D0 ratio
    ...
    if(HT){
      ...
      h01_2D_pi = (TH2F*)f_D5->Get("hDcaLog3D_pi_1");//isPion 4, no mva!!!
      h01_2D_pi_resubmit = (TH2F*)f_D->Get("hDcaLog3D_pi_1");
    }
    ...
    h01_2D_pi_resubmit->GetYaxis()->SetRangeUser(-4,-1);
    ...
    if(pt>0){
      ...
      h01_2D_pi_resubmit->GetXaxis()->SetRangeUser(binning[pt-1],binning[pt]);
