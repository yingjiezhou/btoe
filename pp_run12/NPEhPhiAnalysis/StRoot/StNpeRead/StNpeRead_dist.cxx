...
void StNpeRead::bookObjects()
{
  ...
  for(Int_t trg=0; trg<4; trg+=2)
  {
    ...
    hRcptMeeSMD2[trg]        = new TH2F(Form("RcptMeeSMD2_%i", trg),"",1000,0.,20., 1000,0.,20.); // zyj
    hRcptMeeSMD2US[trg]        = new TH2F(Form("hRcptMeeSMD2US_%i", trg),"",1000,0.,20., 1000,0.,20.);
...
void StNpeRead::writeObjects()
{
  ...
  for(Int_t trg=0; trg<4; trg+=2)
  {
    ...
    hRcptMeeSMD2[trg]->Write();
    hRcptMeeSMD2US[trg]->Write();
...
void StNpeRead::zFill_Photonic (Int_t bTrg,StDmesonEvent * mNpeEvent ,Double_t ps )
{
  ...
  for(Int_t ip=0;ip<mNpeEvent->nElectronPair();ip++)
  {
    ...
    StDmesonTrack*  ptrk = (StDmesonTrack*)aTracks->At(pair->partnerId());
    if(ptrk->gMom().perp()< cuts::partnerElecGlobalPt) continue;
    if(pair->pairDca() < cuts::pairDCA && pair->m() < 0.24) // Pair Conditions
    {
...
//         pass_cut_Match_EMC_Dz(etrk) && pass_cut_Match_EMC_Dphi(etrk) && pass_cut_poe(etrk) && pass_cut_EMC(etrk)) // Is track an electron and the ADC0 big enough to be the trigger?
        if(pass_cut_GoodTrack(etrk) && pass_cut_nsigmaE(etrk) && pass_cut_Pt_Eta(etrk) && pass_cut_ADC(bTrg,etrk) && pass_cut_poe(etrk))
      {
        ...
        Float_t pp = ptrk->gMom().perp(); //partner e
        ...
        if(eq == pq)
        {
          ...
          hRcptMeeSMD2[bTrg] -> Fill(epT, pp);
          cout<<"check       "<<pp<<etrk->pMom().mag()<<endl;
        ...
        if(eq != pq)
        {
          ...
          hRcptMeeSMD2US[bTrg] -> Fill(epT, pp);
...
//  if(nSigmaE>cuts::nsigmae_low && nSigmaE<cuts::nsigmae_high)
  if(nSigmaE>-0.5 && nSigmaE<cuts::nsigmae_high)
...
Int_t StNpeRead::readEff()
{
  ...
  mtrigEffi = new TFile("/star/data01/pwg/yjzhou19/Local/zamiller/NPEhPhiAnalysis2015/StRoot/StNpeRead/TrigEffGraph.root", "READ");
  if(!mtrigEffi->IsOpen()){
