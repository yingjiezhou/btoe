...
void StNpeRead::bookObjects()
{
  ...
  for(Int_t trg=0; trg<4; trg+=2)
  {
    ...
    mh3DelPhiPhotLS[trg]     = new TH3F(Form("mh3DelPhiPhotLS_%i",trg),"",100,-TMath::Pi(),TMath::Pi(),80,-0.1,0.1,200,0,20);
    mh3DelPhiPhotUS[trg]     = new TH3F(Form("mh3DelPhiPhotUS_%i",trg),"",100,-TMath::Pi(),TMath::Pi(),80,-0.1,0.1,200,0,20);
    mh3DelPhiPhotUSNP[trg]   = new TH3F(Form("mh3DelPhiPhotUSNP_%i",trg),"",100,-TMath::Pi(),TMath::Pi(),80,-0.1,0.1,200,0,20);
    mh3DelPhiPhotLSNP[trg]   = new TH3F(Form("mh3DelPhiPhotLSNP_%i",trg),"",100,-TMath::Pi(),TMath::Pi(),80,-0.1,0.1,200,0,20);
  ...
  /// Mixed Events
  mh3MixedDelPhi          = new TH3F("mh3MixedDelPhi","",100,-0.1,0.1,200,0,20,100,0.,10.);
  mh3MixedDelEta          = new TH3F("mh3MixedDelEta","",100,-0.1,0.1,200,0,20,100,0.,10.);
  mh3MixedEtaPhi          = new TH3F("mh3MixedEtaPhi","",100,-0.1,0.1,200,-5,5,100,0.,10.);
  mh3MixedDelPhiWt        = new TH3F("mh3MixedDelPhiWt","",100,-0.1,0.1,200,0,20,100,0.,10.);
  mh3MixedDelEtaWt        = new TH3F("mh3MixedDelEtaWt","",100,-0.1,0.1,200,0,20,100,0.,10.);
  mh3MixedEtaPhiWt        = new TH3F("mh3MixedEtaPhiWt","",100,-0.1,0.1,200,-5,5,100,0.,10.);
  
  // 1D histos for 2.5 to 3.5 GeV epT bin, HT0, hPt>0.3 GeV, For comparison with previous analysis
  mh1delPhiIncl         = new TH1F("mh1delPhiIncl","",100,-0.1,0.1);  // TrigCount bin 1
  mh1delPhiUS           = new TH1F("mh1delPhiUS","",100,-0.1,0.1);    // TrigCount bin 2
  mh1delPhiLS           = new TH1F("mh1delPhiLS","",100,-0.1,0.1);    // TrigCount bin 3
  mh1delPhiHad          = new TH1F("mh1delPhiHad","",100,-0.1,0.1);   // TrigCount bin 4
...
Bool_t StNpeRead::pass_cut_hTrack(StDmesonTrack * trk)
{
  ...
  Float_t dca = trk->dca();
  //cout<<""
  if(Pt > cuts::hadPtMin && fabs(eta) < cuts::hadEta && nhitDedx >= cuts::hadDedx && nhitsFit >= cuts::hadHitsFit && (Float_t) nhitsFit/nhitsMax>cuts::nhit_Possmax && dca < cuts::hadDCA && dca > 0)
    return kTRUE;
  else return kFALSE;
}
