  ...
  mdEdx(0.), mPt(0.), mEta(0.), mPhi(0.), mIndex2Cov(-1), mIdTruth(0), mQuality(0), mIdParentVx(0)
{
  ...
  if ( track->geometry() && track->geometry()->charge()) {
	...
	cout<<vtx_id<<", is vtx_id =====zyj ====="<<endl;
      if (vtx_id >= 0) {
        mVertexIndex=vtx_id;
	cout<<"mVertexIndex   "<<mVertexIndex<<"zbbbbbbbb"<<endl;
      }
      else {
...
StThreeVectorF StMuTrack::dcaGlobal(Int_t vtx_id) const {
  if (vtx_id == -1)
    vtx_id = StMuDst::currentVertexIndex();
  if (vtx_id==mVertexIndex) {
    cout<<mDCAGlobal<<"dcaGlobal=========================================="<<endl;
    return mDCAGlobal;
  }
  else if (mVertexIndex == -1)  { // should not happen
    return gDummy;
...
cout<<"index "<<mVertexIndex<<", vtx_id "<<vtx_id<<"    " <<gTrack->dca(((StMuPrimaryVertex*)StMuDst::array(muPrimaryVertex)->UncheckedAt(vtx_id))->position()).mag()<<"final dca "<<endl;
...
void StMuTrack::Print(Option_t *option) const {
  ...
  cout << "DCA  " << mDCA.mag() << endl;
  //cout << "\t radial " << dcaD()  << ", z " << dcaZ() << endl;
  cout << "global DCA " << mDCAGlobal.mag() << endl;
  cout<< dca(-1).mag() <<"=========================================done======================="<<endl;
/*
