...
Int_t StDmesonMaker::Make()
{
  ...
  if (isGoodEvent())
  {
    ...
    //mPicoEvent->Print();
    //      mPicoDst->Print();
    //cout << "******DEBUG 6: event Id = "<<mPicoEvent->eventId()<<"********************"<<endl;
    //      cout << "******DEBUG 8: tracks = " << nTracks  << endl;
    
    for (UInt_t iTrack = 0; iTrack < nTracks; iTrack++)
    {
      StPicoTrack* trk = mPicoDst->track(iTrack);
      //if(trk->dca() < 5 )trk->Print();
      
      
      if (!trk || !isGoodTrack(trk)) continue;
      
      //	  trk->Print();
      //if(trk->dca() < 5)cout << "******DEBUG 7: dca = " << trk->dca() << endl;
      //         cout << "******DEBUG 7: dca = " << trk->dca() << endl;
      mh2Pion_nSigmaElec[0]->Fill(trk->gMom().perp(),trk->nSigmaElectron()-trk->nSigmaPion());
      mEta_ptUnlike[0]->Fill(trk->gMom().perp(),trk->gMom().pseudoRapidity());
      mgDcalike[0]->Fill(trk->gMom().perp(),trk->dca());
      mHitFit_ptlike[0]->Fill(trk->gMom().perp(),trk->nHitsFit());
      ...
      if(trk->pMom().perp()>1)  getTrgBTOW(trk, trg_tow_id,dsm_adc, adc);
          ...
          || bKaon || bPion || bElectron || bGlobalElectron)
      {
        ...
        mh2Pion_nSigmaElec[1]->Fill(trk->gMom().perp(),trk->nSigmaElectron()-trk->nSigmaPion());
        mEta_ptUnlike[1]->Fill(trk->gMom().perp(),trk->gMom().pseudoRapidity());
        mgDcalike[1]->Fill(trk->gMom().perp(),trk->dca());
        mHitFit_ptlike[1]->Fill(trk->gMom().perp(),trk->nHitsFit());
        
      }
    } // .. end tracks loop
    
    // make photonic electron
    //std::shuffle(idPicoPrimaryElectrons.begin(), idPicoPrimaryElectrons.end(), e);
...
{
  if (trk->pMom().perp() >= cuts::electronPt  && trk->dca()<cuts::dca &&
      trk->nSigmaElectron()>cuts::nSigmaElectronLow && trk->nSigmaElectron()<cuts::nSigmaElectronHigh
      && fabs(trk->pMom().pseudoRapidity())< cuts::electronEta
      && (trk->pMom().perp()>1.0 || (trk->btofBeta()>0 && fabs(1/trk->btofBeta()-1)<cuts::electronsBeta))) return kTRUE;
