...
int main(int argc, char **argv)
{
  ...
  fincCheck = new TNtuple("incCheck", "single e for gpt sys", "ElectronpT:ElectronEta:ElectronPhi:HadronpT:HadronEta:HadronPhi:trig:dphi:deta:hpid:mcpt:hmcpt:mceta:hmceta:mcphi:hmcphi:mcdeta:mcdphi:mcy:hmcy:nTrk");
  ...
  for(int i=0;i<nevents;i++){
    ...
    //cout<<"number of electrons "<<nTracks<<endl;
    bool checkE = kFALSE;
    for(int j=0;j<nTracks;j++) {
      ...
      for(int cc = j+1; cc<nTracks; cc++){
        if(cc == j) continue;
        if(t->parentGeantId[cc]!=0)continue;
        if(isHotTower(t->E_rctowerId[cc])) continue;
        if(t->E_rcNCommonHits[cc]<10) continue;
        float ceta = t->E_rcEta[cc];
        float cphi = t->E_rcPhi[cc];
        float cpt = t->E_rcPt[cc];
        if((eta == ceta) && (phi == cphi) && (cpt == t->E_rcPt[j])) {
          //cout<<ceta<<"   "<<eta<<"  "<<phi<<"   "<<cphi<<endl;
          checkE = kTRUE;
          break;
        }
      }
    ...
    for(int j=0;j<nTracks;j++) {
      ...
      fabs(eta)<0.7 &&
      nMaxPts>0   &&
      fabs(1.0*nHitsFit)/(1.0*nMaxPts)>0.52;// &&
      ...
      if(cutOK && nHitsFit>20 && nDedxPts>15 && dca<1.5 && FirstTpcR<73 && e>0)
      {
        if(pe>0.3&&pe<1.5 && nEta>1&&nPhi>1&&fabs(zDist)<3&&fabs(phiDist)<0.015){
          if(dsmAdc>11&&adc0>180){
            for(int g=j+1;g<nTracks;g++){
              if(g==j) continue;
//              if(t->parentGeantId[g]!=0)continue;
              if(isHotTower(t->E_rctowerId[g])) continue;
              if(t->E_rcNCommonHits[g]<10) continue;
              ...
              //              bool htag = kFALSE;
              //              for(int gg=0;gg<nTracks;gg++){
              //                if(gg == g) continue;
              //                if(t->parentGeantId[gg]!=0)continue;
              //                float hmcptk  = t->E_mcPt[gg];
              //                float hmcetak = t->E_mcEta[gg];
              //                float hmcphik = t->E_mcPhi[gg];
              //                TLorentzVector hMck(0.,0.,0.,0.);
              //                hMck.SetPtEtaPhiM(hmcptk,hmcetak,hmcphik,MassE);
              //
              //                Double_t hdeta = Mck.Rapidity() - hMck.Rapidity();
              //                Double_t hdphi = mcphik - hmcphik;
              //
              //                while(hdphi>TMath::Pi()) hdphi -= 2.*TMath::Pi();
              //                while(hdphi<-TMath::Pi()) hdphi = hdphi + 2.*TMath::Pi();
              //
              ////                while(hdphi>2*TMath::Pi()) hdphi -= 2.*TMath::Pi();
              ////                while(hdphi<0) hdphi += 2.*TMath::Pi();
              ////                while(hdphi>TMath::Pi()) hdphi = hdphi -2*TMath::Pi();
              //                if(TMath::Abs(hdeta)<0.1 && TMath::Abs(hdphi)<0.5) htag = kTRUE;
              //              }
              //              if(htag) continue;
              ...
              while(mcdphi>TMath::Pi()) mcdphi -= 2.*TMath::Pi();
              while(mcdphi<-TMath::Pi()) mcdphi = mcdphi + 2.*TMath::Pi();
              ...
              hnMaxPts>0   &&
              nHitsFit>=15 &&
              fabs(1.0*hnHitsFit)/hnMaxPts*1.0 >0.52 &&
              hdca < 1. && hdca > 0;
