  ...
  for(Int_t ipt=0;ipt<NpT_bins_run12_HT;ipt++)
    {
      ...
      mh1nSigmaE_unlike[ipt]=(TH1F *) mh3nSigmaE_pT_Mass_unlike[0]->ProjectionX(buf,mh3nSigmaE_pT_Mass_unlike[0]->GetYaxis()->FindBin(pt_run12_HT[ipt]),mh3nSigmaE_pT_Mass_unlike[0]->GetYaxis()->FindBin(pt_run12_HT[ipt+1]),1,20);
      ...
      mh1NisgmaE_like[ipt]=(TH1F *) mh3nSigmaE_pT_Mass_like[0]->ProjectionX(buf,mh3nSigmaE_pT_Mass_unlike[0]->GetYaxis()->FindBin(pt_run12_HT[ipt]),mh3nSigmaE_pT_Mass_unlike[0]->GetYaxis()->FindBin(pt_run12_HT[ipt+1]),1,20);
...
void Fit_electron_nsigma_Mean(TH1F *mh1NisgmaE_unlike[NpT_bins_run12_HT],TH1F *mh1NisgmaE_like[NpT_bins_run12_HT],TH1F *mh1NisgmaE_unlike_like[NpT_bins_run12_HT])
{
  ...
  Mean->Fit(fmean,"R","same",2.5,8.5); 
  Mean_u->Fit(fmean_u,"R0","same",2.5,8.5); 
  Mean_d->Fit(fmean_d,"R0","same",2.5,8.5); 
  
  Sigma->Fit(fsigma,"R","same",2.5,8.5);
  Sigma_u->Fit(fsigma_u,"R0","same",2.5,8.5);
  Sigma_d->Fit(fsigma_d,"R0","same",2.5,8.5);
  ...
  c5->SaveAs("Nsigma_mean_sigma_primary.pdf");
  ...
  TH1F * nsigma_e_kaon=(TH1F *) nsigma_E_kaon->Rebin(NpT_bins_run12_HT,"nsigma_E_kaon",pt_run12_HT); 
   ...
   c7->SaveAs("hadron_nsigmaE_calibration_primary.pdf");
