void run_StPheRecoEff(const char* file, const char* outfile) 
{
    ...
    string SL_version = "SL12d_embed"; // DEBUG
    ...
    // FILES
    char* infiles = "@test_file.list";
    StFile* files = new StFile();
    files->AddFile(infiles);

    // I/O maker
    // StIOMaker* ioMaker = new StIOMaker;
    // ioMaker->SetFile(file);
    // ioMaker->SetIOMode("r");
    StIOMaker* ioMaker = new StIOMaker("ioMaker","r",files);
    // ioMaker->SetFile(file);
    // ioMaker->SetIOMode("r");
    ...
    StPheRecoEff* analysis = new StPheRecoEff(outfile,149,col_energy,triggers);
