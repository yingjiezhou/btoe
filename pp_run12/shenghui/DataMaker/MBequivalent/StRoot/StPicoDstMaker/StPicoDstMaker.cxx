  ...
  mChain(0), mTTree(0), mSplit(99), mCompression(9), mBufferSize(65536*4)
{
  ...
  if(mIoMode==ioWrite) {
    ...
    mOutputFileName.ReplaceAll("MuDst.root","FemtoDst.root");
  }
  if(mIoMode==ioRead) {
...
Int_t StPicoDstMaker::openRead() {
  ...
  if (dirFile.find(".list")!=string::npos)  {
    ...
    while (inputStream.good()) {
      ...
      string aFile = line;      
      if (inputStream.good() && aFile.find(".FemtoDst.root")!=string::npos) {
//        TFile *ftmp = new TFile(line);
  ...
  } else if (dirFile.find(".FemtoDst.root")!=string::npos)  {
