...
void plotReceff()
{
	...
	TH2F *htmp;
	TLegend *lg;
	TFile *f = new TFile("../Efficiency/embedding/DataMaker/Pi0Dalitz_hist.root");
	//TFile *f = new TFile("DataMaker/Pi0Dalitz_hist_Sys.root");
	//TFile *f = new TFile("DataMaker/Pi0Dalitz_hist_BRSys.root");
	TH1F *InputNumberPi0Dalitz=(TH1F *)f->Get("hnTotalyCut");
	TH1F *EleNumberPi0Dalitz=(TH1F *)f->Get("RcptPosEle");
	TH1F *RcptPi0Dalitz = (TH1F *)f->Get("Rcpt");
	TH1F *RcptMeePi0Dalitz = (TH1F *)f->Get("RcptMee");
	//TH1F *RcptMeePi0Dalitz = (TH1F *)f->Get("RcptMeeSys");
	//TH1F *RcptMeePi0Dalitz = (TH1F *)f->Get("RcptMeeGptP");
	TFile *f = new TFile("../Efficiency/embedding/DataMaker/EtaDalitz_hist.root");
	//TFile *f = new TFile("DataMaker/EtaDalitz_hist_Sys.root");
	//TFile *f = new TFile("DataMaker/EtaDalitz_hist_BRSys.root");
	TH1F *InputNumberEtaDalitz=(TH1F *)f->Get("hnTotalyCut");
	TH1F *EleNumberEtaDalitz=(TH1F *)f->Get("RcptPosEle");
	TH1F *RcptEtaDalitz = (TH1F *)f->Get("Rcpt");
	TH1F *RcptMeeEtaDalitz = (TH1F *)f->Get("RcptMee");
	//TH1F *RcptMeeEtaDalitz = (TH1F *)f->Get("RcptMeeSys");
	//TH1F *RcptMeeEtaDalitz = (TH1F *)f->Get("RcptMeeGptP");
	TFile *f = new TFile("../Efficiency/embedding/DataMaker/Gamma_hist.root");
	//TFile *f = new TFile("DataMaker/Gamma_hist_Sys.root");
	TH1F *InputNumberGamma=(TH1F *)f->Get("hnTotalyCut");
	TH1F *EleNumberGamma=(TH1F *)f->Get("RcptPosEle");
	TH1F *RcptGamma = (TH1F *)f->Get("Rcpt");
	TH1F *RcptMeeGamma = (TH1F *)f->Get("RcptMee");
	//TH1F *RcptMeeGamma = (TH1F *)f->Get("RcptMeeSys");
	...
	cc=Canvas("cc",0.1,0.05,0.15,0.06,0,1,0);
	cc->SetGrid(1,1);
	float ymin,ymax;
	///////////////////////////////////////////////////////////////////////////////////////////////////////
	//Pi0Dalitz Rec. eff.
	...
	htmp=histo2D("htmp",20,PtMinhtmp[0],PtMaxhtmp[0],20,0,1.4,"p_{T} (GeV/c)","Reconstruction effciency",0.075,0.85,0.07,0.83);
	//htmp->SetNdivisions(505,"y");
	...
	TF1 *fun = new TF1("feff","[0]/(exp(-(x-[1])/[2])+1)+[3]",PtMin[0],PtMax[0]);
	fun->SetParameters(1.004,-0.01253,2.158,-0.4732);
	heff->Fit(fun,"N0RIS","",PtMin[0],PtMax[0]);
	...
	heff->Fit(fun,"N0RIS","",PtMin[0],PtMax[0]);
	//heff->Fit(fun,"N0RIS","",4,PtMax[0]);
	TFitResultPtr fitr = heff->Fit(fun,"N0RIS","",PtMin[0],PtMax[0]);
	TH1D *Funcerr = new TH1D("Funcerr","",750,PtMin[0],PtMax[0]);
	for(int i=0;i<750;i++){
		double PTerr=0.005;
	...
	TPaveStats *ptstats;
	ptstats= new TPaveStats(0.67,0.17,0.94,0.4,"brNDC");
	...
	lg= Legend(0.41,0.58,0.71,0.83,0.05);
	...
	lg->AddEntry(Funcerr,"Fitting uncertainty","f");
	lg= Legend(0.32,0.2,0.67,0.35,0.045);
	lg->AddEntry(fun,"Fit to Combined","l");
	lg->AddEntry(fun,"p_{0}/(e^{-(p_{T}-p_{1})/p_{2}}+1)+p_{3}","");
	tx.SetTextSize(0.065);
	tx.DrawLatex(0.15,0.86,"(b)");
	tx.SetTextSize(0.07);
	tx.DrawLatex(0.47,0.86,"p+p @ #sqrt{s} = 200GeV");
	c->SaveAs("pic/Receff.png");
	c->SaveAs("pic/Receff.eps");
	TH1F *hefffun=(TH1F *)heff->Clone("hefffun");
	TH1F *hefffunSysUnc=(TH1F *)heff->Clone("hefffunSysUnc");
	for(int i=0;i<nPtBins;i++){
		hefffun->SetBinContent(i+1,fun->Eval(0.5*(ptbin[i+1]+ptbin[i])));
		hefffun->SetBinError(i+1,0.0);//heff->GetBinError(i+1));
		hefffunSysUnc->SetBinContent(i+1,fun->Eval(0.5*(ptbin[i+1]+ptbin[i]))+Funcerr->GetBinError(Funcerr->FindBin(0.5*(ptbin[i+1]+ptbin[i])+1e-6)));
