#include "StSingleElectronHadron.h"
...
ClassImp(StSingleElectronHadron)

//_________________________________________________
StSingleElectronHadron::StSingleElectronHadron(const char *name, const char *file):StMaker(name)
{
	//  StSingleElectronHadron Constructor
  ...
  mTree->Branch("h_geantId",mParticle.h_geantId,"h_geantId[nTracks]/I");
...
//_________________________________________________
StSingleElectronHadron::~StSingleElectronHadron()
{
}

//_________________________________________________

void StSingleElectronHadron::Clear(const char*)
{
	StMaker::Clear();
}

//_________________________________________________
Int_t StSingleElectronHadron::Finish()
{
	mOutputFile->Write();
	mOutputFile->Close();
...
//_________________________________________________
Bool_t StSingleElectronHadron::projTrack(StThreeVectorD *atFinal, StThreeVectorD *momentumAtFinal, StTrack* track, double magField, double radius, int option) {
...
Int_t StSingleElectronHadron::Init()
{
	Clear("C");
	///////
...
//_________________________________________________
Int_t StSingleElectronHadron::Make()
{
	Clear("C");
	...
	for(int i=0;i<(int) mctracks.size();i++) {
		...
		mParticle.h_geantId[nTracks] = mcElectron->geantId();
    mcE = mcElectron;
...
StTrackPairInfo* StSingleElectronHadron::findBestMatchedGlobal(StMcTrack* mcTrack)
{
