...
{
	TF1 *myGausP = new TF1("myGausP","gaus",-10,10);
	myGausP->SetParameters(1,-0.4161,0.8977);
	TF1 *myGausE = new TF1("myGausE","gaus",-10,10);
	myGausE->SetParameters(1,-0.4161,0.8977);
	if(argc != 3) exit(1);
	FileOutPut = "out/Eff_";
	FileOutPut += argv[1];
	FileOutPut += ".root";
	FileInPut = "DataMaker/out/";
	FileInPut += argv[1];
	FileInPut += ".root";
	...
	char style[3];
	strncpy(style,argv[1],3);
	style[3]='\0';

	weightFile=new TFile("DataMaker/GetGammaSpectrum/macros/rootfile/Pi0EtadNdyDirectGammaSpectrumpp.root","READ");
	if(strcmp(style, Style[0]) == 0){
		hdNdpTdy=(TH1F *)weightFile->Get("hFunPi0err");
		mass=0.1349766;
		BR=1.174e-2;//BR Unc 0.035e-2
		//BR=1.174e-2+0.035e-2;
	}
	if(strcmp(style, Style[1]) == 0){
		hdNdpTdy=(TH1F *)weightFile->Get("hFunEtaerr");
		mass=0.547853;
		BR=6.9e-3;//BR Unc 0.4e-3
		//BR=6.9e-3+0.4e-3;
	}
	MassE = 0.000511;
	hnTotal            = new TH1F("hnTotal","",40,-2.,2.);
	hnTotalyCut        = new TH1F("hnTotalyCut","",40,-2.,2.);
	hRcpt              = new TH1F("Rcpt","",nPtBins-1,ptbinfyj);
	hRcptPosEle        = new TH1F("RcptPosEle","",nPtBins-1,ptbinfyj);
	hRcptHT0           = new TH1F("RcptHT0","",nPtBins-1,ptbinfyj);
	hRcptPosEleHT0     = new TH1F("RcptPosEleHT0","",nPtBins-1,ptbinfyj);
	hRcptHT2           = new TH1F("RcptHT2","",nPtBins-1,ptbinfyj);
	hRcptPosEleHT2     = new TH1F("RcptPosEleHT2","",nPtBins-1,ptbinfyj);
	hRcptSMD           = new TH1F("RcptSMD","",nPtBins-1,ptbinfyj);
//	hRcptPosEleSMD     = new TH1F("RcptPosEleSMD","",nPtBins-1,ptbinfyj);
  hRcptPosEleSMD     = new TH2F("RcptPosEleSMD","",1000, 0, 20 ,1000, 0, 20);

	hRcptHT0SMD        = new TH1F("RcptHT0SMD","",nPtBins-1,ptbinfyj);
	hRcptPosEleHT0SMD  = new TH1F("RcptPosEleHT0SMD","",nPtBins-1,ptbinfyj);
	hRcptHT2SMD        = new TH1F("RcptHT2SMD","",nPtBins-1,ptbinfyj);
	hRcptPosEleHT2SMD  = new TH1F("RcptPosEleHT2SMD","",nPtBins-1,ptbinfyj);
	...
	hRcptMeeGptP       = new TH1F("RcptMeeGptP","",nPtBins-1,ptbinfyj);
	hRcptMee           = new TH1F("RcptMee","",nPtBins-1,ptbinfyj);
	hRcptMeeHT0        = new TH1F("RcptMeeHT0","",nPtBins-1,ptbinfyj);
	hRcptMeeHT2        = new TH1F("RcptMeeHT2","",nPtBins-1,ptbinfyj);
	hRcptMeeSys        = new TH1F("RcptMeeSys","",nPtBins-1,ptbinfyj);
	hRcptMeeSysHT0     = new TH1F("RcptMeeSysHT0","",nPtBins-1,ptbinfyj);
	hRcptMeeSysHT2     = new TH1F("RcptMeeSysHT2","",nPtBins-1,ptbinfyj);
	hRcptMeeSMD1        = new TH1F("RcptMeeSMD1","",nPtBins-1,ptbinfyj);
	hRcptMeePOESMD1     = new TH1F("RcptMeePOESMD1","",nPtBins-1,ptbinfyj);
	hRcptMeeSysSMD1     = new TH1F("RcptMeeSysSMD1","",nPtBins-1,ptbinfyj);
//	hRcptMeeSMD2        = new TH1F("RcptMeeSMD2","",nPtBins-1,ptbinfyj);
  hRcptMeeSMD2        = new TH2F("RcptMeeSMD2","",1000, 0, 20, 1000, 0, 20);
  hRcptMeeSMD3        = new TH1F("RcptMeeSMD3","",1000, 0, 20);

	hRcptMeePOESMD2     = new TH1F("RcptMeePOESMD2","",nPtBins-1,ptbinfyj);
	hRcptMeeSysSMD2     = new TH1F("RcptMeeSysSMD2","",nPtBins-1,ptbinfyj);
//	hRcptMeeSMD3        = new TH1F("RcptMeeSMD3","",nPtBins-1,ptbinfyj);
	hRcptMeePOESMD3     = new TH1F("RcptMeePOESMD3","",nPtBins-1,ptbinfyj);
	hRcptMeeSysSMD3     = new TH1F("RcptMeeSysSMD3","",nPtBins-1,ptbinfyj);
	hRcptMeeHT0SMD     = new TH1F("RcptMeeHT0SMD","",nPtBins-1,ptbinfyj);
	hRcptMeeHT2SMD     = new TH1F("RcptMeeHT2SMD","",nPtBins-1,ptbinfyj);
	hRcptMeeSysHT0SMD  = new TH1F("RcptMeeSysHT0SMD","",nPtBins-1,ptbinfyj);
	hRcptMeeSysHT2SMD  = new TH1F("RcptMeeSysHT2SMD","",nPtBins-1,ptbinfyj);

	//--------------------
	// loop the chain
	//--------------------
	TChain *chain = new TChain("mcTDalitz");
	chain->Add(FileInPut.Data());

	int nevents = (int)chain->GetEntries();
	cout << "== total entries : " << nevents << endl;

	mcTDalitz *t = new mcTDalitz(chain);
	...
	for(int i=0;i<nevents;i++){
		...
		for(int k=0;k<t->nTracksDalitz;k++) {
			TLorentzVector McInput(0.,0.,0.,0.);
			if(t->mcEtaInput[k]>2000.) continue;
			McInput.SetPtEtaPhiM(t->mcPtInput[k],t->mcEtaInput[k],t->mcPhiInput[k],mass);

			hnTotal->Fill(McInput.Rapidity());
			if(fabs(McInput.Rapidity())<1.0) hnTotalyCut->Fill(McInput.Rapidity());
		}
		/*for(int k=0;k<t->nTracks;k++) {
		  TLorentzVector McInput(0.,0.,0.,0.);
		  McInput.SetPtEtaPhiM(t->mcPt[k],t->mcEta[k],t->mcPhi[k],mass);
		...
		for(int j=0;j<t->nTracks;j++) {
			...
			Mc.SetPtEtaPhiM(pt,eta,phi,mass);
			if(fabs(Mc.Rapidity())>1.0) continue;
			...
			Double_t nsigmaP = myGausP->GetRandom();
			Double_t nsigmaE = myGausE->GetRandom();
			...
			bool positronParCutOK =
				...
				electronCutOK                    &&
				nsigmaE>-5.0     &&    nsigmaE<5.0    &&
				t->E_rcNCommonHits[j]>=10;//        &&
				//!(isHotTower(t->E_rctowerId[j]));

			//float wtPt = hdNdpTdy->GetBinContent(hdNdpTdy->FindBin(pt));
      
			float wtPt = hdNdpTdy->GetBinContent(hdNdpTdy->FindBin(pt))+hdNdpTdy->GetBinError(hdNdpTdy->FindBin(pt));
			float dydeta=pt*cosh(eta)/sqrt(mass*mass+pt*pt*cosh(eta)*cosh(eta));
			float wt = wtPt*dydeta;
			float wtEle = wt*2*BR/(2*TMath::Pi())/hRcptPosEle->GetBinWidth(hRcptPosEle->FindBin(E_rcPt))/1.4*20;
			float wtPos = wt*2*BR/(2*TMath::Pi())/hRcptPosEle->GetBinWidth(hRcptPosEle->FindBin(P_rcPt))/1.4*20;
			//===========================
			//denominator
			//===========================
			if(positronPriCutOK){
				...
				if(positronSMDPriCutOK){
					...
					hRcptPosEleSMD->Fill(P_rcPt, E_rcPt,wtPos/P_rcPt);//zyj
//          hRcptPosEleSMD->Fill(P_rcPt,wtPos/P_rcPt);
			...
			if(electronPriCutOK){
				...
				if(electronSMDPriCutOK){
					...
					hRcptPosEleSMD->Fill(E_rcPt, P_rcPt,wtEle/E_rcPt); //zyj, here fill with cross section
				...
				if(eem<0.24) {
					if(positronPriCutOK && electronParCutOK) { //tag: positron, partner: electron
						if(positronSMDPriCutOK) { //tag: positron, partner: electron
							hRcptMeeSMD1->Fill(P_rcPt,wt);
							if(pe_P>0.6) hRcptMeePOESMD1->Fill(P_rcPt,wt);
							if(eem<0.15) hRcptMeeSysSMD1->Fill(P_rcPt,wt);
							if(t->E_rcgPt[j]>0.1){
                hRcptMeeSMD2->Fill(P_rcPt, t->E_rcgPt[j],wt); // zyj, since we are get effi, only fill dNdpT will be enough
//								hRcptMeeSMD2->Fill(P_rcPt,wt);
								if(pe_P>0.6) hRcptMeePOESMD2->Fill(P_rcPt,wt);
								if(eem<0.15) hRcptMeeSysSMD2->Fill(P_rcPt,wt);
							}
							if(t->E_rcgPt[j]>0.4){
//								hRcptMeeSMD3->Fill(P_rcPt,wt);
								if(pe_P>0.6) hRcptMeePOESMD3->Fill(P_rcPt,wt);
								if(eem<0.15) hRcptMeeSysSMD3->Fill(P_rcPt,wt);
							}
						}
					}
					if(electronPriCutOK && positronParCutOK) { //tag: electron, partner: positron
						if(electronSMDPriCutOK) { //tag: positron, partner: electron
							hRcptMeeSMD1->Fill(E_rcPt,wt);
							if(pe_E>0.6) hRcptMeePOESMD1->Fill(E_rcPt,wt);
							if(eem<0.15) hRcptMeeSysSMD1->Fill(E_rcPt,wt);
							if(t->P_rcgPt[j]>0.1){
//								hRcptMeeSMD2->Fill(E_rcPt,wt);
                hRcptMeeSMD2->Fill(E_rcPt, t->P_rcgPt[j],wt); // zyj

								if(pe_E>0.6) hRcptMeePOESMD2->Fill(E_rcPt,wt);
								if(eem<0.15) hRcptMeeSysSMD2->Fill(E_rcPt,wt);
							}
							if(t->P_rcgPt[j]>0.4){
//								hRcptMeeSMD3->Fill(E_rcPt,wt);
								if(pe_E>0.6) hRcptMeePOESMD3->Fill(E_rcPt,wt);
								if(eem<0.15) hRcptMeeSysSMD3->Fill(E_rcPt,wt);
							}
						}
					}
				}
...
//	hRcptMeeSMD1->Write();
//	hRcptMeePOESMD1->Write();
//	hRcptMeeSysSMD1->Write();
	hRcptMeeSMD2->Write();
//	hRcptMeePOESMD2->Write();
//	hRcptMeeSysSMD2->Write();
//	hRcptMeeSMD3->Write();
//	hRcptMeePOESMD3->Write();
//	hRcptMeeSysSMD3->Write();
