...
//			if(fabs(geta)<1.0) {
        if(gpt>0.2&&fabs(geta)<1.0&&fabs(nHitsFit)>=15) {
			...
			//cout<<"dsmadc"<<dsmadc<<" "<<mHTth<<" "<<adc0<<" "<<mHTAdc0th<<endl;
			int  isTrackQ = 0;
			if(fabs(nHitsFit)>nFPtsCut&&nHitsDedx>ndEdxPtsCut&&dca<dcaCut) isTrackQ = 1;
			//cout<<nHitsFit<<" "<<nFPtsCut<<" "<<nHitsDedx<<" "<<ndEdxPtsCut<<" "<<dca<<" "<<dcaCut<<endl;
...
void makerealevent()
{
	...
	for(int i=0; i<CurrentEvent_nEMCE; i++) {
		...
		for(int j=0; j<CurrentEvent_nEs; j++) {
			...
			if(fabs(EMCENhitsFit)>=nFPtsCut&&EMCENhitsDedx>=ndEdxPtsCut&&EMCEDca<dcaCut) isTrackQ = 1;
			if(fabs(EtaPart)<0.7&&fabs(ENhitsFit)>=nFPtsCut&&ENhitsDedx>=ndEdxPtsCut&&EDca<dcaCut) isTrackQPart = 1;
			...
			if(charge1*charge2 == -1) {
				...
				if(isEMC&&EMCENSigE>-0.5&&EMCENSigE<3.){
					...
					if(isTriggered&&EMCENhitsDedx>=ndEdxPtsCut&&EMCEDca<dcaCut&&gPt>0.3)
						hnHitsFit->Fill(PtTag,fabs(EMCENhitsFit));
					/*if(isTriggered&&isTrackQ)
					  hnHitsFitPart->Fill(PtPart,fabs(ENhitsFit));*/
					if(isTriggered&&fabs(EMCENhitsFit)>=nFPtsCut&&EMCEDca<dcaCut&&gPt>0.3)
          ...
          if(isTriggered&&isTrackQ&&(fabs(ENSigE)<5.)) hGpTPart->Fill(PtTag,gPt);
					if(isSMD){
				...
				//nsigmae eff
				/////////////////////////////////////////////////////
				if(isTriggered&&isTrackQ&&isEMC&&gPt>0.3){
				...
				if(isEMC&&EMCENSigE>-0.5&&EMCENSigE<3.){
					...
					if(isTriggered&&isTrackQ&&(fabs(ENSigE)<5.))hGpTPartlike->Fill(PtTag,gPt);
					if(isSMD){
				...
				//nsigmae eff
				/////////////////////////////////////////////////////
				if(isTriggered&&isTrackQ&&gPt>0.3&&isEMC){
