void drawtext();
TGraph *getFonll();
TGraphAsymmErrors *getSysb(const char* sys = "gpt", const char* def = "default");
void makebFPlot(const char* FileName="Sep16"){
  gROOT->ProcessLine(".x ~/myStyle.C");
  //  gROOT->SetBatch(kTRUE);
  //  gStyle->SetOptFit(1111);
  char name[100];
  const char* outfileName = "gptVsDef";
  TGraphAsymmErrors*  ggpt = getSysb("gpt", FileName);

  auto fyjd = new TFile(Form("FFOutput/sys/sysChange_default_%s_FIT.root", FileName),"READ");
  auto hd = (TGraphAsymmErrors *)fyjd->Get("sys");
  auto hnorm = (TGraphAsymmErrors *)fyjd->Get("normsmall");
  //===========================================================================
  // preliminary, shift x axis
  float ypT[9], yRb[9], ydyU[9], ydyD[9], ydysU[9], ydysD[9];
  ...
  //========================================================================
  // run6
  ifstream infile2("input/Run6Result.txt");
  int k=0;
  ...
  TGraphAsymmErrors* hdf = new TGraphAsymmErrors(7,pT,yRb,dx,dx,ydyD,ydyU); // zyj
  TGraphAsymmErrors* stat = new TGraphAsymmErrors(9,ypT,Rb,dx,dx,dyD,dyU); // preliminary
  TGraphAsymmErrors* syst = new TGraphAsymmErrors(9,ypT,Rb,dx2,dx2,dysD,dysU);
  TGraphErrors* stat6 = new TGraphErrors(7,pT6,Rb6,dx,dy6); // run6
  ...
  hdf->GetXaxis()->SetTitle("NPE p_{T} (GeV/c)");
  hdf->GetYaxis()->SetTitle("B#rightarrowNPE / Inclusive#rightarrowNPE");
  ...
  hdf->GetXaxis()->SetLimits(0,12);
  hdf->GetYaxis()->SetLimits(-0.1,1.2);
  hdf->GetYaxis()->SetRangeUser(-0.1,1.2);
  
  hdf->SetMarkerSize(1.5);
  hdf->SetMarkerStyle(21);
  hdf->SetMarkerColor(kBlue);
  ...
  TLegend* leg = new TLegend(0.18,0.72,0.6,0.91);
  leg->SetBorderSize(0);
  leg->SetFillStyle (0);
  leg->SetFillColor (0);
  //  leg->AddEntry(stat6,"Run 5/6 p+p 200 GeV","pe");
  leg->AddEntry(stat,"STAR Run 12(Prelimary)","pe");
  leg->AddEntry(hdf,"STAR Run 12(Yingjie), gp_{T}>0.3","pe");
  leg->AddEntry(ggpt,"STAR Run 12(Yingjie), gp_{T}>0.2","pe");
  ...
  //  TCanvas* temp = new TCanvas("temp","",50,50,1200,1200);
  sprintf(name, "pic/%s_bFraction_%s.pdf[", FileName, outfileName);
  temp->Print(name);
  //  temp = cnorm;
  //  temp->Print(name);
  temp = c1;
...
TGraphAsymmErrors *getSysb(const char* sys = "gpt", const char* def = "default"){
  ...
  auto fyjd = new TFile(Form("FFOutput/sys/sysChange_%s_%s_FIT.root", sys, def),"READ");
  auto hd = (TGraphAsymmErrors *)fyjd->Get("sys");
  auto hnorm = (TGraphAsymmErrors *)fyjd->Get("normsmall");
  
  float dx[9],ypT[9], yRb[9], ydyU[9], ydyD[9];
  
  for(auto i = 0; i<7; i++){
    double ty, tx;
    hd->GetPoint(i, tx, ty);
    cout<<tx<<endl;
    ypT[i] = tx+0.3;  // for shift
    yRb[i] = ty;
    dx[i] = 0.15;
    ydyD[i] = hd->GetErrorYlow(i);
    ydyU[i] = hd->GetErrorYhigh(i);
  ...
  TGraphAsymmErrors* hdf = new TGraphAsymmErrors(7,ypT,yRb,dx,dx,ydyD,ydyU); // zyj
  hdf->SetMarkerSize(1.5);
  hdf->SetMarkerStyle(21+4);
  hdf->SetMarkerColor(kBlue);
  hdf->SetLineColor(kBlue);
