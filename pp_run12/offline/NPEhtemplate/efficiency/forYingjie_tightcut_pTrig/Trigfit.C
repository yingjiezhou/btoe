...
void Trigfit(){
  ...
  auto fght1 = new TF1("fght1", "TMath::Erf(x - 2.41589)", 2, 20);
  auto fght2 = new TF1("fght2", "TMath::Exp(-16.0543+3.3662*x)", 0, 4.5);
  auto fght3 = new TF1("fght3", "TMath::Erf(0.651713*x-2.6212)", 4.5, 20);
  fght1->SetLineColor(kBlue);
  fght2->SetLineColor(kBlue);
  fght3->SetLineColor(kBlue);
  ...
  //  auto fht2 = new TF1("fht2", "TMath::Exp([0]+[1]*x)-[2]", 2.5, 5.1);
  //  auto fht3 = new TF1("fht3", "TMath::Erf([0]*x-[1])-[2]", 5, 20);
  fht1->SetLineColor(kRed);
  fht2->SetLineColor(kRed);
  fht3->SetLineColor(kRed);
  ...
  leg->SetNColumns(2);
  leg->AddEntry(fght1,"HT0, gpt","l");
  leg->AddEntry(fght2,"HT2, gpt","l");
  ...
  hr0->Draw();
  hr2->SetLineColor(kRed);
  hr2->SetMarkerColor(kRed);
  
  hr2->Draw("same");
  c->SaveAs("trigg_eff_fit.pdf");
