...
//cout << "2222222222222222222222"<< endl;
    //  StDcaGeometry    dcaG  = t->dcaGeometry();
      StPhysicalHelixD helix = t->helix(B);
...
//thelix.Move(thelix.Path(0.2, 0.2));
  //    const Double_t *pos = thelix.Pos();
    //  const Double_t *dir = thelix.Dir();
     StThreeVectorF DcaVec = helix.origin();
      StThreeVectorF dcaGlobal = DcaVec - pVtx;
      StThreeVectorF p = t->gMom(pVtx, B);
...
void StMyAnalysisMaker::getRunInfo(int const runNumber)
{
  ...
  char fileName[256];
  sprintf(fileName, "%s/%i.qVector.root", EventPlaneConstants::qVectorRunDir.Data(), 17073001);
  TFile* fQVector = new TFile(fileName);
  if(fQVector->IsZombie())
    {
      int dayNumber = mRunNumber%1000000/1000;
      sprintf(fileName, "%s/%03d.qVector.root", EventPlaneConstants::qVectorDayDir.Data(), 117);
      delete fQVector;
