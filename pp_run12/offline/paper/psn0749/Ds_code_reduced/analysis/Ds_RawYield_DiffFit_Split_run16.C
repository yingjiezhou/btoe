...
TF1 *fMass_BKfromWS = new TF1("fMass_BKfromWS","([0])/sqrt(2*TMath::Pi())/[2]*exp(-(x-[1])*(x-[1])/2./[2]/[2])+[3]+[4]*x+([5])/sqrt(2*TMath::Pi())/[7]*exp(-(x-[6])*(x-[6])/2./[7]/[7])", 1.8, 2.1);
	...
	TFile *File_input_Large_run16 = TFile::Open("/star/u/fuchuan/data02/Ds/Ds_code_GPC/analysis/DataCut_0080_1_8_KPipT0.5_phyAndsst_run16.root");
	sig_largeh3 = (TH3D*)File_input_Large_run16->Get(Form("h3sg_%s_Lc_%i",mCent.Data(), Cut_num));
	bkg_largeh3 =(TH3D*)File_input_Large_run16->Get(Form("h3bkg_%s_Lc_%i",mCent.Data(), Cut_num));
	h3bkg_Ds =(TH3D*)File_input_Large_run16->Get(Form("h3bkg_%s_Ds",mCent.Data()));
	h3RS_Ds =(TH3D*)File_input_Large_run16->Get(Form("h3RS_%s_Ds",mCent.Data()));
	...
	//cout << " ************ sig_largeh1->GetMaximum()************** = " << sig_largeh1->GetBinContent(sig_largeh1->GetMaximumBin()) << endl;
	...
	fMass->SetParLimits(2,0.009,0.013);//old: 0.007-0.013
...
TF1 *f_BK_BKfromWS = new TF1("f_BK_BKfromWS","[0]+[1]*x", 1.8, 2.1);
	...
	if(mPt_Mini == 51 && mPt_Max == 80 && cent_low == 1 && cent_high == 4){ fMass_19_21->SetParLimits(2,0.009,0.013);}
	else	fMass_19_21->SetParLimits(2,0.009,0.013);
	...
	fMass_second->SetParLimits(5,-100,100);
	...
	sig_largeh1->SetAxisRange(Minimum_Bin*0.6-2,Maxmum_Bin*1.5,"Y");
	//	if(mPt_Mini == 51 && mPt_Max == 80 && cent_low == 1 && cent_high == 4) sig_largeh1->SetAxisRange(-1,12,"Y");
	...
	//  bkg_largeh1->SetFillStyle(3004);
	//bkg_largeh1->SetFillColor(15);
...
//	cout << Bin_siglarge_Start << Bin_siglarge_Stop << endl;
...
//cout << " Bkg_counts_Scaledhist = " <<  Bkg_counts_Scaledhist << endl;
...
if (Bkg_ploy_num>=0)	 Ds_counts_fit = RS_num - Bkg_ploy_num;
	...
	double Signi_sig_err = ((pow((RS_num+Ds_counts_fit),2))/(4*pow(RS_num,3)))*RS_err*RS_err;
...
TMatrixDSym mat_BKfromWS = Rpar_BKfromWS->GetCovarianceMatrix();
	...
	//cout << Form("Par_second[5]from %i-%i, %i-%i = ", cent_low, cent_high, mPt_Mini, mPt_Max) << Par_second[5] << endl;
	//for(int i = 0; i<=8; i++)
	//{
	...
	TString mCentrality;
	TString Lable = Form("Au+Au -> D_{s}^{#pm} + X, #sqrt{s_{NN}} = 200 GeV,2016");
 ...
 plotTopCent(Form("Linear fit (WS shape): #(D^{#pm}_{s})=%.i#pm%.i",Ds_counts_fit_BKfromWS , Ds_err_fit_BKfromWS),0.12,0.6+0.06,0.04,2,0.0,22,1);
	...
	TString mTitle = Form("/home/fuchuan/TMVA_My/Anlysis_Ds/Raw_Sig/run16/pic/DsRec_%s_%.1f_%.1f_defaultBDT_Combined_phyAndsst.pdf", mCentrality.Data(), (mPt_Mini-1)*0.1, mPt_Max*0.1); 
//		c1->SaveAs(mTitle.Data());
}

int main()
{
	TFile *output = new TFile("Ds_RawYield_run16_Combined_phyAndsst_defaultBDTcut.root","recreate");
	...
	//double Nev_16[5] = {1.25e8, 3.89e8, 5.28e8, 1.28e8, 2.61e8}; //st_physics
	double Nev_16[5] = {1.25e8+4.18e7, 3.89e8+1.46e8, 5.28e8+1.90e8, 1.28e8+4.71e7, 2.61e8+9.94e7}; //Combined_phyAndsst
	...
	for (int i = 0; i <= 4; i++)
	{
		if (i != 1)
		{
			for (int j = 0; j <= 3; j++)
			{
				...
				mT_BR_Nev_dmT_dy[i][j] = (12.56*0.0227*mT[j]*dmT[j]*dY*Nev_16[i]);
	...
	int BDT_cut[5][5] = { {113, 75, 81, 86, 86}, {102, 105, 61, 68, 72}, {40, 58, 71, 65, 65}, {113, 66, 72, 76, 76}, {98, 55, 63, 67, 67} };// combined st_sst and st_physics

//	int BDT_cut[5][5] = { {113+10, 75+10, 81+17, 86+26, 86+26}, {102+16, 105+19, 61+20, 68+20, 72+30}, {40+23, 58+27, 71+29, 65+30, 65+30}, {113+12, 66+18, 72+16, 76+30, 76+30}, {98+23, 55+22, 63+28, 67+30, 67+30} };// combined st_sst and st_physics

//	int BDT_cut[5][5] = { {113-10, 75-10, 81-17, 86-30, 86-30}, {102-14, 105-19, 61-23, 68-30, 72-30}, {40-30, 58-30, 71-15, 65-5, 65-5}, {113-12, 66-20, 72-26, 76-26, 76-26}, {98-12, 55-29, 63-30, 67-30, 67-30} };// combined st_sst and st_physics


int SigNum[5][5] = {{309, 320, 156, 31, 31}, {325, 457, 841, 469, 139}, {167, 191, 110, 44, 44}, {288, 364, 199, 55, 55}, {415, 462, 279, 86, 86}};// run16 phyAndsst SigNum
...
RawYield_6080_155_fit_BKfromWS = new TH1F("RawYield_6080_155_fit_BKfromWS","", 1, 1.5, 5);
	...
	for (int i = 0; i<=4; i++)
	{
		if (i != 1)
		{
			for (int j =0; j<=3; j++)
			{
				...
				RawYield_fit[i]->SetBinContent(j+2,num_fit/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_fit[i]->SetBinError(j+2,err_fit/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_fit_BKfromWS[i]->SetBinContent(j+2,num_fit_BKfromWS/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_fit_BKfromWS[i]->SetBinError(j+2,err_fit_BKfromWS/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_fit_183_201[i]->SetBinContent(j+2,num_fit_183_201/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_fit_183_201[i]->SetBinError(j+2,err_fit_183_201/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_fit_19_21[i]->SetBinContent(j+2,num_fit_19_21/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_fit_19_21[i]->SetBinError(j+2,err_fit_19_21/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_fit_second[i]->SetBinContent(j+2,num_fit_second/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_fit_second[i]->SetBinError(j+2,err_fit_second/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_hist[i]->SetBinContent(j+2,num_hist/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_hist[i]->SetBinError(j+2,err_hist/pT_BR_Nev_dpT_dy_run16[i][j]);
				RawYield_RSAll[i]->SetBinContent(j+2,num_RSAll/pT_BR_Nev_dpT_dy_run16[i][j]);
	...
	Ds_RawYield_DiffFit_Split_run16("1040", 101, 16, 25,  5, 7, 500, num_RSAll, num_hist, num_fit, num_fit_BKfromWS, num_fit_183_201, num_fit_19_21, num_fit_second, err_hist, err_fit, err_fit_BKfromWS, err_fit_183_201, err_fit_19_21, err_fit_second, width_fit, width_fit_err, mean_fit, mean_fit_err, signi_fit, signi_fit_err, BkgNum, BkgNum_err, RSNum, RSNum_err);//st_phy: BDT cut = 89
	...
	RawYield_1040_1525_hist->SetBinContent(1,num_hist/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_hist->SetBinError(1,err_hist/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit->SetBinContent(1,num_fit/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit->SetBinError(1,err_fit/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit_BKfromWS->SetBinContent(1,num_fit_BKfromWS/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit_BKfromWS->SetBinError(1,err_fit_BKfromWS/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit_183_201->SetBinContent(1,num_fit_183_201/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit_183_201->SetBinError(1,err_fit_183_201/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit_19_21->SetBinContent(1,num_fit_19_21/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit_19_21->SetBinError(1,err_fit_19_21/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit_second->SetBinContent(1,num_fit_second/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	RawYield_1040_1525_fit_second->SetBinError(1,err_fit_second/(12.56*0.0227*pT[0]*dpT[0]*dY*Nev_16[1]));
	Ds_RawYield_DiffFit_Split_run16("4080", 54, 16, 50,  3, 4, 396, num_RSAll, num_hist, num_fit, num_fit_BKfromWS, num_fit_183_201, num_fit_19_21, num_fit_second, err_hist, err_fit, err_fit_BKfromWS, err_fit_183_201, err_fit_19_21, err_fit_second, width_fit, width_fit_err, mean_fit, mean_fit_err, signi_fit, signi_fit_err, BkgNum, BkgNum_err, RSNum, RSNum_err);//54-30, 54+24
	...
	RawYield_4060_155_hist->SetBinContent(1,num_hist/(12.56*0.0227*2.61486*3.5*dY*3.64e8));//st_phy:2.64e8 + st_sst:1.0e8
	RawYield_4060_155_hist->SetBinError(1,err_hist/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit->SetBinContent(1,num_fit/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit->SetBinError(1,err_fit/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit_BKfromWS->SetBinContent(1,num_fit_BKfromWS/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit_BKfromWS->SetBinError(1,err_fit_BKfromWS/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit_183_201->SetBinContent(1,num_fit_183_201/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit_183_201->SetBinError(1,err_fit_183_201/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit_19_21->SetBinContent(1,num_fit_19_21/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit_19_21->SetBinError(1,err_fit_19_21/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit_second->SetBinContent(1,num_fit_second/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	RawYield_4060_155_fit_second->SetBinError(1,err_fit_second/(12.56*0.0227*2.61486*3.5*dY*3.64e8));
	...
	Ds_RawYield_DiffFit_Split_run16("4080", 28, 16, 50,  1, 2, 53, num_RSAll, num_hist, num_fit, num_fit_BKfromWS, num_fit_183_201, num_fit_19_21, num_fit_second, err_hist, err_fit, err_fit_BKfromWS, err_fit_183_201, err_fit_19_21, err_fit_second, width_fit, width_fit_err, mean_fit, mean_fit_err, signi_fit, signi_fit_err, BkgNum, BkgNum_err, RSNum, RSNum_err);//28-27, 28+30
	...
	RawYield_6080_155_hist->SetBinContent(1,num_hist/(12.56*0.0227*2.61486*3.5*dY*3.54e8));//st_phy:2.64e8 + st_sst:9e7
	RawYield_6080_155_hist->SetBinError(1,err_hist/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit->SetBinContent(1,num_fit/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit->SetBinError(1,err_fit/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit_BKfromWS->SetBinContent(1,num_fit_BKfromWS/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit_BKfromWS->SetBinError(1,err_fit_BKfromWS/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit_183_201->SetBinContent(1,num_fit_183_201/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit_183_201->SetBinError(1,err_fit_183_201/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit_19_21->SetBinContent(1,num_fit_19_21/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit_19_21->SetBinError(1,err_fit_19_21/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit_second->SetBinContent(1,num_fit_second/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
	RawYield_6080_155_fit_second->SetBinError(1,err_fit_second/(12.56*0.0227*2.61486*3.5*dY*3.54e8));
