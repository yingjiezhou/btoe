...
void offline(const char* FileName="Sep16", const char* FileNameR="default", Bool_t doPurity = kFALSE, Bool_t doRec = kFALSE)
{
  ...
  if(doPurityAdc0) sprintf(name,"output/%s_trig.root",FileName);
  if(doPuritySigmae) sprintf(name,"output/%s_nsigmae.root",FileName);
  if(doPurityPoe) sprintf(name,"output/%s_poe.root",FileName);
  if(doRecMass) sprintf(name,"output/%s_masscut.root",FileName);
  if(doRecGpt) sprintf(name,"output/%s_gpt.root",FileName);
