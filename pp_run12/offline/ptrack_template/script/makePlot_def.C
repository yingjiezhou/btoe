void drawtext();
TGraph *getFonll();
void makePlot_def(const char* FileName="Sep16"){
  
    gROOT->ProcessLine(".x ~/myStyle.C");
  //  gROOT->SetBatch(kTRUE);
  //  gStyle->SetOptFit(1111);
  char name[100];
  auto fyjd = new TFile(Form("FFOutput/sysChange_default_%s_FIT.root", FileName),"READ");
  auto hd = (TGraphAsymmErrors *)fyjd->Get("HT0");
...
//  leg->AddEntry(stat6,"Run 5/6 p+p 200 GeV","pe");
  ...
  leg->AddEntry(hdf,"STAR Run 12(Yingjie)","pe");
...
//  TCanvas* temp = new TCanvas("temp","",50,50,1200,1200);
  sprintf(name, "bFraction.pdf[", FileName);
  temp->Print(name);
//  temp = cnorm;
//  temp->Print(name);
  temp = c1;
