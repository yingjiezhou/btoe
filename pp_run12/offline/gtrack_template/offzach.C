...
void offzach(const char* FileName="test")
{
  ...
  // Open ROOT File
  char name[1000];
  sprintf(name,"../../production/npe_tree_Mar3_1.root");
  ...
  TFile* file;
  if(makeROOT){
    sprintf(fname,"%s_processed.root",FileName);
  ...
  double effPar[3];
  FILE* pFile;
  pFile = fopen("PHE_Plots/effParFits.txt","r");
 ...
 /* mixedC->cd(2);
  ...
  for(Int_t trig = 0; trig < numTrigs; trig++){
    ...
    for(Int_t ptbin=0; ptbin<1; ptbin++)// cut in to trigger pt slices
    {
      ...
      projZDCxHadHad[ptbin][trig]-> Scale(HHScale*hadPur); // should be some problem zyj
//      cout<<"scale hadron factor       "<<HHScale*hadPur<<endl;
