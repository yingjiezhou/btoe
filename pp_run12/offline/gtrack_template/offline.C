...
void offline(const char* FileName="Mar3_1", const char* FileNameR="default")
{
...
//  fs->SetMarkerSize(0.4);
  ...
  for(Int_t trig = 0; trig < numTrigs; trig++){
    ...
    for(Int_t ptbin=0; ptbin<1; ptbin++)// cut in to trigger pt slices
    {
      ...
      epsilon[ptbin] = epsCom;
//      Float_t pCom[3] = {0.9743, 0.02128, -0.00438}; // zach used
//      Float_t pCom[3] = {0.968187, 0.0236712, -0.00452398};
      Float_t pur = purPar[0] + (purPar[1]*xCom)+(purPar[2]*xCom*xCom);
//      Float_t hadPurCom = 1-pur;

//      USNorm   = projZDCxTrigUS[ptbin][trig]->Integral();
//      inclNorm = mh1PtETracks[trig]->Integral(mh1PtETracks[trig]->GetXaxis()->FindBin(pulowpt[ptbin]),mh1PtETracks[trig]->GetXaxis()->FindBin(puhighpt[ptbin])-1);
//
//      hhNorm   = mh1PtHadTracks[trig]->Integral(mh1PtHadTracks[trig]->GetXaxis()->FindBin(pulowpt[ptbin]),mh1PtHadTracks[trig]->GetXaxis()->FindBin(puhighpt[ptbin])-1);
//
...
//      projZDCxHadHad[ptbin][trig]-> Scale(HHScale*hadPurCom);
...
//      projZDCxTrigHad[ptbin][trig]-> Scale(HHScale*hadPurCom);
...
//      projZDCxTrig[ptbin][trig] -> Add(projZDCxTrigHad[ptbin][trig], -1.);
      ...
      projZDCxHadUS[ptbin][trig] -> Scale(1./epsilon[ptbin]);
      projZDCxHad[ptbin][trig] -> Add(projZDCxHadUS[ptbin][trig], -1.);
...
//      Float_t p[3] = {0.968187, 0.0236712, -0.00452398};
      ...
      projZDCxTrig[ptbin][trig] -> Add(projZDCxTrigUS[ptbin][trig], 1);
      projZDCxTrig[ptbin][trig] -> Scale(purity);
      projZDCxTrigUS[ptbin][trig] -> Add(projZDCxTrigLS[ptbin][trig], -1.);
      projZDCxTrigUS[ptbin][trig] -> Scale(1./epsilon[ptbin]);
      projZDCxTrig[ptbin][trig] -> Add(projZDCxTrigUS[ptbin][trig], -1.);
...
//      cout << "HT" << trig << " " << lowpt[ptbin] << " < pT < " << highpt[ptbin] << " hhevents: " << hhNorm << endl;
      ...
      Double_t dphiBins = (Double_t)projDelPhiInclWt[ptbin][trig]->FindBin(3.1415) - (Double_t)projDelPhiInclWt[ptbin][trig]->FindBin(-3.1414);
...
//      TH1F *USmLS  = (TH1F*)USIM[ptbin][trig]->Clone();
//      USmLS->Add(LIKE,-1);
