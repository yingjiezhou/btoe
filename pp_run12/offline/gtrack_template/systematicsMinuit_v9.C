...
//    cout << "!!!!!!! HTC ptbin: " <<  highpt[ptbin] << "-" << lowpt[ptbin] <<" !!!!!!!"<< endl;
  ...
  grA->GetYaxis()->SetRangeUser(0, 0.2);
  ...
  if(!oneParamFit){
    ...
    grAS->GetYaxis()->SetRangeUser(0, 0.2);
...
void doFit(TMinuit* gMinuit, Double_t& p0, Double_t& p1, Double_t& e0U, Double_t& e1U, Double_t& e0D, Double_t& e1D)
{
  ...
  //starting values
  double vstart[2]={0.3,0.}; //frac
  double step[2]={0.005,0.001}; //starting step
  gMinuit->mnparm(0,"BtoNPE frac",vstart[0],step[0],0.000,1,ierflg);
  gMinuit->mnparm(1,"Scale Factor",vstart[1],step[1],0.000,0.03,ierflg);
