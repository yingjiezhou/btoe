...
Double_t FITPARAS = 1.036; // From Fit of P1. Error is 0.007 on fit`
Bool_t doPileup = kTRUE;
Bool_t doPythia = kFALSE;

void systematicsMinuit_zach(const char* FileName="Mar3_1", const char* FileNameR="zach_NPE")
{
  ...
  if(doPythia)
    sprintf(name,"/Users/zhouyingjie/Local/PWG/NPE/1_pp_zamiller/minBiasTemplates111915/readTreeOutput/input/pythia_npe_tree_Feb24.root");
  TFile *fT = new TFile(name,"READ");
  
//  sprintf(name,"/Users/zhouyingjie/Local/PWG/NPE/1_pp_zamiller/minBiasTemplates111915/readTreeOutput/input/pythia_npe_tree_Feb24.root");
//  TFile *fTsys = new TFile(name,"READ");
  
//  sprintf(name,Form("FFOutput/npe_tree_%s_processed_default.root", FileName));
  sprintf(name,"test_processed.root");
  TFile *fD = new TFile(name,"READ");
  
  //  sprintf(name,"/Users/zach/Research/rootFiles/run12NPEhPhi/currentDataSystematics.root");
  if(doPileup) sprintf(name,"test_processed.root");
    //sprintf(name,Form("FFOutput/npe_tree_%s_processed_default.root", FileName));
  else if(oneParamFit) sprintf(name,Form("FFOutput/npe_tree_%s_processed_default.root", FileName));
  else if(doPythia)sprintf(name,Form("FFOutput/npe_tree_%s_processed_default.root", FileName));
  else   sprintf(name,"test_processed.root");

//    sprintf(name,Form("FFOutput/npe_tree_%s_processed_%s.root", FileName, FileNameR));
...
//  TCanvas* deltaPhiSys  = new TCanvas("deltaPhiSys","Pythia Delta Phi sys",150,0,1150,1000);
...
//      cout<<FileNameR<<"  "<<Rb0S[plotCountSys]<<"    "<<Rb0[plotCount0]<<endl;
      ...
      sysChange[plotCountSys] = 100.*(Rb0S[plotCountSys]-Rb0[plotCount0])/Rb0[plotCount0];
      cout<<FileNameR<<"  "<<plotCount0<<"   :"<<sysChange[plotCountSys] <<endl;
      fprintf(pFile,"%d %.3f %.3f %.3f \n",plotCount0,Rb0[plotCount0],Rb0S[plotCountSys],sysChange[plotCountSys]);
