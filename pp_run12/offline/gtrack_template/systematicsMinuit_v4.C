...
Double_t FITPARAS = 1.047; // From Fit of P1. Error is 0.007 on fit`
//1.034 ± 0.01067
...
void systematicsMinuit(const char* FileName="Mar3_1", const char* FileNameR="fitNormPlusLarge")
{
  ...
  TGraphAsymmErrors *psyst = getPrelimnarySys();sprintf(name,"/Users/zhouyingjie/Local/PWG/NPE/1_pp_zamiller/minBiasTemplates111915/readTreeOutput/input/pythia_npe_tree_Feb4.root");
  //  TGraphAsymmErrors *psyst = getPrelimnarySys();sprintf(name,"/Users/zhouyingjie/Local/PWG/NPE/1_pp_zamiller/minBiasTemplates111915/readTreeOutput/input/pythia_npe_tree_Feb24.root");
  ...
  if(doPileup) sprintf(name,Form("FFOutput/npe_tree_%s_processed_default.root", FileName));
  else if(oneParamFit) sprintf(name,Form("FFOutput/npe_tree_%s_processed_default.root", FileName));
  else if(PYT)sprintf(name,Form("FFOutput/npe_tree_%s_processed_default.root", FileName));
  ...
  for(Int_t ptbin=0; ptbin<9; ptbin++)
  {
    ...
    if(doPileup){
      ...
      for(int i = 0; i < 2; i++){
        ...
        //        beforeShift->GetYaxis()->SetRangeUser(0,.04);
        ...
        //        cout<<beforeShift->GetBinContent(100)<<"  HT"<<i<<"     pile up correction factor   "<<endl;
        //        if(ptbin == 3){
        //          pileU->cd(i+1);
        //          beforeShift->Draw("P");
