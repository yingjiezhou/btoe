...
Bool_t doFitRange = kFALSE;
Bool_t dofitMethod = kTRUE;
void systematicsMinuit(const char* FileName="Mar3_1", const char* FileNameR="FitMethod2")
{
//  bUp = kTRUE;
//  bDown = kTRUE;
  ...
  if(doPileup || doPythia || oneParamFit || doFitRange || dofitMethod) sprintf(name,Form("FFOutput/npe_tree_%s_processed_default.root", FileName));
...
//    cout << ptbin << "; 0: " << norm0 << " 2: " << norm2 << endl;
    ...
    TMinuit* gMinuitS = new TMinuit(1);
    ...
    if(highpt[ptbin] < 5)
    {
      ...
      if(doPythia || doFitRange || dofitMethod){
        Double_t x,y;
...
//    cout << "!!!!!!! HTC ptbin: " <<  highpt[ptbin] << "-" << lowpt[ptbin] <<" !!!!!!!"<< endl;
  ...
  grA->GetYaxis()->SetRangeUser(0, 0.06);
  ...
  if(!oneParamFit){
    ...
    grAS->GetYaxis()->SetRangeUser(0, 0.06);
...
void doFit(TMinuit* gMinuit, Double_t& p0, Double_t& p1, Double_t& e0U, Double_t& e1U, Double_t& e0D, Double_t& e1D)
{
  ...
  //starting values
  double vstart[2]={0.2,0.001}; //frac
  double step[2]={0.001,0.001}; //starting step
  gMinuit->mnparm(0,"BtoNPE frac",vstart[0],step[0],0.000,1,ierflg);
  gMinuit->mnparm(1,"Scale Factor",vstart[1],step[1],0.00,0.05,ierflg);
...
double getFitFunction(Double_t *par, double y1, double y2)
{
 ...
 ycomb = par[1]+ par[0]*y2 + y1*(1-par[0]); //A*rb*yb + A*(1-rb)*yc
...
//  ycomberr = sqrt(par[1]*par[0]*par[1]*par[0]*ey2*ey2 + ey1*ey1*(1-par[0])*par[1]*(1-par[0])*par[1]); //A*rb*yb + A*(1-rb)*yc
  ycomberr = sqrt(par[0]*par[0]*ey2*ey2 + ey1*ey1*(1-par[0])*(1-par[0])); //A*rb*yb + A*(1-rb)*yc
...
//    ycomberr = sqrt(par[1]*par[0]*par[1]*par[0]*ey2*ey2 + ey1*ey1*(1-par[0])*par[1]*(1-par[0])*par[1]); //A*rb*yb + A*(1-rb)*yc
  ...
  ycomberr = sqrt(par[0]*par[0]*ey2*ey2 + (1-par[0])*(1-par[0])*ey1*ey1);
...
//    l.PaintText(x,y+0.2,Form("%4.3f",y));
    l.PaintText(x,y+0.007,Form("%4.3f",y));
