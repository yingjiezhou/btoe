...
void systematicsMinuit()
{
  ...
  for(Int_t ptbin=0; ptbin<numPtBins; ptbin++)
  {
    ...
    if(kSymmetrize)
    {
      ...
      norm0S = 2.0*histoNorms->GetBinContent(histoNormsS->GetBin(1,ptbin+1));
      norm2S = 2.0*histoNorms->GetBinContent(histoNormsS->GetBin(3,ptbin+1));
    }
    else
    {
      norm0 = histoNorms->GetBinContent(histoNorms->GetBin(1,ptbin+1));
    ...
    cClone2->Scale((1.-p01S[ptbin])*FITPARA); bClone2->Scale(p01S[ptbin]*FITPARA);
    cClone->Add(bClone);
    ...
    if(highpt[ptbin] > 4.6)
    {
      ...
      Rb0S[plotCount2] = p21S[ptbin];///(p21[ptbin]+p20[ptbin]);
    ...
    cClone2->Scale((1.-p21S[ptbin])*FITPARA); bClone2->Scale(p21S[ptbin]*FITPARA); // scale by contribution param
    cClone->Add(bClone);
    // cClone->Scale(dClone->GetMaximum()/cClone->GetMaximum());
    dClone->GetXaxis()->SetRangeUser(anaConst::lowPhi,anaConst::highPhi);
    dClone->GetYaxis()->SetRangeUser(plotLow[ptbin],plotHigh[ptbin]);
...
//  grP->GetYaxis()->SetRangeUser(0,1);
  ...
  leg2->AddEntry(gr0S,"STAR Run 12, Sys","pe");
