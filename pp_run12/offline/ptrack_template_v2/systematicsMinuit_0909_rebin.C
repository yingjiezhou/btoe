...
void systematicsMinuit(const char* FileName="May5", const char* FileNameR="default", Bool_t doPileup = kFALSE, Bool_t doPythia = kFALSE, Bool_t doFitRange = kFALSE)
{
//  bUp = kTRUE;
//  bDown = kTRUE;
...
//  sprintf(name,"minBiasTemplates111915/readTreeOutput/input/pythia_npe_tree_May9.root"); // from yjzhou
  if(doPythia) sprintf(name,"output/sim/pythia_npe_tree_May26.root");
//    if(doPythia) sprintf(name,"/Users/zhouyingjie/Local/PWG/btoe/pp_run12/sim/offline/Jpsi_sys/rootfile/hf_newTune.root");

//  if(doPythia && doJpsi) sprintf(name,"output/sim/AfterSubScale_pythia_npe_tree_May18.root");// simu, just take minus, no need to consider fraction
  if(doPythia && doJpsi) sprintf(name,"output/sim/pythia_npe_tree_May18_jpsi_Octet.root");// new, bak
...
//  deltaPhiSys->Divide(3,3);
  ...
  for(Int_t ptbin=0; ptbin<7; ptbin++)
  {
    ...
    if(doJpsi){
      cbPtNorms[ptbin]   = (TH1F*)fT->Get(Form("ceEventTallyJpsi_%i",ptbin));
...
//  cout<<projData0[ptbin]->GetXaxis()->FindBin(3)<<"   xxxxx"<<projData0[ptbin]->GetXaxis()->FindBin(-3)<<"  "<<projData0[ptbin]->GetXaxis()->FindBin(1.5)<<"  "<<projData0[ptbin]->GetXaxis()->FindBin(1.75)<<projData0[ptbin]->GetBinCenter(125)<<endl;
  rangeLow  = projData0[ptbin]->GetXaxis()->FindBin(-1.5);
  rangeHigh  = projData0[ptbin]->GetXaxis()->FindBin(1.5);
//  cout<<rangeLow<<"       xxxxx         "<<rangeHigh<<endl;
...
void chi2_0S(Int_t &npar,Double_t *gin,Double_t &func,Double_t *par,Int_t iflag){
  ...
  rangeLow  = projData0S[ptbin]->GetXaxis()->FindBin(-1.5);
  rangeHigh  = projData0S[ptbin]->GetXaxis()->FindBin(1.5);
...
void chi2_2S(Int_t &npar,Double_t *gin,Double_t &func,Double_t *par,Int_t iflag){
  ...
  rangeLow  = projData2S[ptbin]->GetXaxis()->FindBin(-1.5);
  rangeHigh  = projData2S[ptbin]->GetXaxis()->FindBin(1.5);
