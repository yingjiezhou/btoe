...
void offline_addPartnerCorr_LoopisOK_addSplitCutOnLS_noTrigEff(const char* FileName="May5", const char* FileNameR="default", Bool_t doPurity = kFALSE, Bool_t doRec = kFALSE)
{
  ...
  else sprintf(name,"output/data/out_addPartnerCorr_default_LoopisOK_splitcutOnLS_noTrigEff.root"); // modified PHE loop
  if(doRecGpt) sprintf(name,"output/data/out_addPartnerCorr_gpt_LoopisOK_splitcutOnLS_noTrigEff.root");
