...
void offline(const char* FileName="May5", const char* FileNameR="default", Bool_t doPurity = kFALSE, Bool_t doRec = kFALSE)
{
...
//  else sprintf(name,"output/data/npe_tree_%s_10default_%s.root",FileName, FileNameR);
  else sprintf(name,"output/data/out_Modify_Loop_default.root"); // modified PHE loop
