...
//  else sprintf(name,"output/data/out_NoEPair.root"); // modified PHE loop
  ...
  for(Int_t ptbin = 0; ptbin < numPtBins; ptbin++)
  {
    ...
    if(ptbin == 0)
    {
      ...
      usmlsComp[ptbin][0]->SetTitle("Unlike - Like Sign Trigger DeltaPhi Comparison, no partner");
    ...
    //    //reset ranges
    inclTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001,2200);
    usTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001,1200);
    lsTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001,700);
    usmlsComp[ptbin][0]->GetYaxis()->SetRangeUser(.001,700);
