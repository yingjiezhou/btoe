...
void offline_addPartnerCorr_LoopisOK(const char* FileName="May5", const char* FileNameR="default", Bool_t doPurity = kFALSE, Bool_t doRec = kFALSE)
{
...
//  gROOT->ProcessLine(".x ~/myStyle.C");
  ...
  else sprintf(name,"output/data/out_addPartnerCorr_default_LoopisOK.root"); // modified PHE loop
  if(doRecGpt) sprintf(name,"output/data/out_addPartnerCorr_gpt_LoopisOK.root");
  ...
  const int num=10;
  const char *slopeName[num] = {"NPE HT0", "SemiInc HT0", "US HT0", "LS HT0", "Had HT0", "NPE HT2", "SemiInc HT2", "US HT2", "LS HT2", "Had HT2"};
  ...
  for(Int_t trig = 0; trig < numTrigs; trig++){
    ...
    for(Int_t ptbin=0; ptbin<1; ptbin++)// cut in to trigger pt slices
    {
      ...
      pulbl[ptbin] = new TPaveText(.4,.15,.60,.23,Form("NB NDC%i",ptbin));
...
//      gStyle->SetOptFit(1111);
      ...
      projZDCxHad[ptbin][trig]->SetNdivisions(505,"x");
      ...
      TLegend* pileupLeg = new TLegend(0.33,0.74,0.55,0.87);
    ...
    for(Int_t ptbin = 0; ptbin < numPtBins; ptbin++){
      ...
      //      Float_t Norm = (Float_t)inclNorm - 1./epsilon[ptbin] *(USNorm -LSNorm) - HHScale*hadPur*hhNorm; // Use the number of "signal" counts
      ...
      TH1F *USmLS  = (TH1F*)LIKENP->Clone();
      USmLS->Add(UNLIKENP,-1);
...
//        pileupCorrection[ptbin][trig]->SetNdivisions(505,"y");
//        pileupCorrection[ptbin][trig]->SetNdivisions(505,"x");
...
//    usmlsTrig[ptbin][0]->GetYaxis()->SetRangeUser(-0.11,0.01);
//
...
//    usmlsTrig[ptbin][2]->GetYaxis()->SetRangeUser(-0.11,0.01);
    ...
    //    inclTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001,0.3);
    //    usTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001,0.3);
    //    usTrigNP[ptbin][0]->GetYaxis()->SetRangeUser(.001,0.3);
    //    lsTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001,0.3);
    //    lsTrigNP[ptbin][0]->GetYaxis()->SetRangeUser(.001,0.3);
    //    usmlsTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001,0.3);
