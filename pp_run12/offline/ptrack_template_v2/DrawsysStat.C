...
void DrawsysStat(const char* FileName="result_allSys_10defaultd_stat_04"){
  ...
  auto fyj = new TFile("FFOutput_addPartnerCorr_LoopisOK/sysChange_default_May5_FIT.root","READ");
...
//    "FFOutput/sysChange_pythia_May5_FIT.root",
    "FFOutput_addPartnerCorr_LoopisOK/sysChange_gpt_May5_FIT.root",
    "FFOutput_addPartnerCorr_LoopisOK_SplitCut/sysChange_gpt_May5_FIT.root",
    "FFOutput/sysChange_gpt_May5_FIT.root",
//    "FFOutput_addPartnerCorr_addSplitCut_usePartner/sysChange_gpt_May5_FIT.root"
  };
...
//  TString filename[20] = { "pileup", "purity", "rec", "gpt", "masscut", "nsigmae", "poe",  "trigEff", "fitrange", "pythia", "jpsi"};
  ...
  TString filename[3] = {"gpt", "gpt_withSplitCut", "gpt_woTrigEff"};
    ...
    for(int ii = 0; ii < nSys; ii++)
    {
      ...
      if(i>4){
        if(y[i][ii] > -0.5 && y[i][ii] < 0.5){
          ...
          cout << "pT: " << pTSys[ii] << " Sys%(+/-): " << sqrt(y[i][ii]*y[i][ii]) << endl;
        }
      }
      else {
        cout<<y[i][ii]<<endl;
        if(y[i][ii] > -0.5 && y[i][ii] < 0.5) // Add up the squares of all positive contrib in each pt
        {
          if(y[i][ii]>sysEP[ii] || y[i][ii]>sysEM[ii]){
            ...
            cout << "pT: " << pTSys[ii] << " Sys%(+/-): " << TMath::Sqrt(y[i][ii]*y[i][ii]-sysEP[ii]*sysEP[ii]) << " / " << TMath::Sqrt(y[i][ii]*y[i][ii]-sysEM[ii]*sysEM[ii]) << endl;
          }
          else {
            sysP[ii] +=0;
            sysM[ii] +=0;
  ...
  lbl[2] = new TPaveText(.43,.18,.88,.3,Form("NB NDC%i",sysNum));
  sprintf(textLabel,"sys_{sys}^{2} = |r_B - r_B^{sys}|^{2} - |stat_{r_B}^{2} - stat_{r_B^{sys}}^{2}|");
  lbl[2]->AddText(textLabel);
  lbl[2]->SetFillColor(kWhite);
  ...
  for(int i = 0; i < sysNum; i++){
    ...
    leg[i] = new TLegend(.4,0.8,0.9,0.89);
    ...
    gr[i]->GetYaxis()->SetRangeUser(-0.5,0.5);
