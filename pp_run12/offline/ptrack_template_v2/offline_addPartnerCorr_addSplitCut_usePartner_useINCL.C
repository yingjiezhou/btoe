...
void offline_addPartnerCorr_addSplitCut_usePartner_useINCL(const char* FileName="May5", const char* FileNameR="default", Bool_t doPurity = kFALSE, Bool_t doRec = kFALSE)
{
  ...
  else sprintf(name,"output/data/out_addPartnerCorr_withSplitCut_default_v2.root"); // modified PHE loop
  if(doRecGpt) sprintf(name,"output/data/out_addPartnerCorr_withSplitCut_gpt_v2.root");
  ...
  for(Int_t trig = 0; trig < numTrigs; trig++){
    ...
    for(Int_t ptbin = 0; ptbin < numPtBins; ptbin++){
      ...
      HHScale = (Float_t)(inclNorm)/(Float_t)hhNorm;
      Float_t Norm = (Float_t)inclNorm - 1/epsilon[ptbin]*((Float_t)USNorm -(Float_t)LSNorm) - HHScale*hadPur*hhNorm;
