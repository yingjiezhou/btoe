...
void offline_plot(const char* FileName="May5", const char* FileNameR="default", Bool_t doPurity = kFALSE, Bool_t doRec = kFALSE)
{
  ...
  //  Bool_t doRec = kFALSE; // phe cocktail, 5%, fraction
  ...
  // Open ROOT File
  char name[1000];
  //  sprintf(name,"production/npe_tree_%s.root",FileName);
  
  if(doPurity || doRec) sprintf(name,"output/data/npe_tree_%s_10default_default.root",FileName);
//  else sprintf(name,"output/data/npe_tree_%s_10default_%s.root",FileName, FileNameR);
  else sprintf(name,"output/data/out_addPartnerCorr_origin_default.root"); // modified PHE loop
  if(doRecGpt) sprintf(name,"output/data/out_addPartnerCorr_origin_gpt.root"); // modified PHE loop

  //  if(doPurityAdc0) sprintf(name,"output/npe_tree_%s_4default_trig.root",FileName);
  //  if(doPuritySigmae) sprintf(name,"output/npe_tree_%s_4default_nsigmae.root",FileName);
  //  if(doPurityPoe) sprintf(name,"output/npe_tree_%s_4default_poe.root",FileName);
  //  if(doRecMass) sprintf(name,"output/npe_tree_%s_4default_masscut.root",FileName);
  //  if(doRecGpt) sprintf(name,"output/npe_tree_%s_4default_gpt.root",FileName);
  ...
  //  if(doPuritySigmae)sprintf(name, "forYingjie_pTrig/InEdNdpTAndPurityforYingjiensigmaecutSys.root");
  ...
  //  sprintf(name, "forYingjie_tightcut_pTrig/PHEReceffforYingjie.root"); // with gpt cut
  //  if(doRecGpt)   sprintf(name, "forYingjie_loosecut_pTrig/PHEReceffforYingjie.root"); // with gpt cut
  //  if(doRecGpt)   sprintf(name, "forYingjie_loosecut_pTrig_update/PHEReceffforYingjie.root"); // with gpt cut
  ...
  for(auto i=0; i<9; i++){
    if(doRec) {
      ...
      //      hrect->SetBinError(i+1, hrecTmp->GetBinError(i+1));
    }
    else {
  ...
  //  double effPar[3];
  //  FILE* pFile;
  //  if(doRec) pFile = fopen(Form("PHE_Plots/effParFits_%s.txt", FileNameR),"r");
  //  else pFile = fopen("PHE_Plots/effParFits.txt","r");
  //  fscanf(pFile,"%lf %lf %lf",&effPar[0],&effPar[1],&effPar[2]);
  //  fclose(pFile);
  //
  //  double purPar[3];
  //  if(doPurity) pFile = fopen("purParFits_shift.txt","r");
  //  else pFile = fopen("purParFits.txt","r");
  ...
  npeComp = new TCanvas("npeComp","npeComp",150,0,1150,1000);
  ...
  for(Int_t trig = 0; trig < numTrigs; trig++){
    ...
    for(Int_t ptbin = 0; ptbin < numPtBins; ptbin++){
      ...
      //
      //      cout<<LSNorm<<"   "<<USNorm<<"     "<<HHScale<<endl;
      ...
      leg->AddEntry(USIM[ptbin][trig],"Unlike Sign","lpe");
      ...
      //      TH1F *UNLIKE  = (TH1F*)USIM[ptbin][trig]->Clone();
      //      TH1F *LIKE  = (TH1F*)LSIM[ptbin][trig]->Clone();
      TH1F *UNLIKE  = (TH1F*)ULDP->Clone();
      TH1F *LIKE  = (TH1F*)LSDP->Clone();
      //      LIKE->Add(UNLIKE, -1.);
      TH1F *HADRON = (TH1F*)HHDP[ptbin][trig]->Clone();
      TH1F *USmLS  = (TH1F*)LIKE->Clone();
      USmLS->Add(UNLIKE,-1);
...
//      HADRON->Scale(1./((Double_t)Norm));
      ...
      usmlsTrig[ptbin][trig]    ->SetName(Form("usmlsTrig_%i_%i",ptbin, trig));
      hadTrig[ptbin][trig]   = (TH1F*)HADRON->Clone();
      hadTrig[ptbin][trig]   ->SetName(Form("hadTrig_%i_%i",ptbin, trig));
      npeTrig[ptbin][trig]   = (TH1F*)unscaleINCDP->Clone();
  ...
  for(Int_t ptbin = 0; ptbin < numPtBins; ptbin++)
  {
    ...
    usmlsTrig[ptbin][2]    -> SetLineColor(kRed);
    usmlsTrig[ptbin][2]    -> SetMarkerColor(kRed);
    ...
    if(ptbin == 0)
    {
      ...
      usTrig[ptbin][0]->SetTitle("Unlike Sign Trigger DeltaPhi Comparison, no partner");
      lsTrig[ptbin][0]->SetTitle("Like Sign Trigger DeltaPhi Comparison, no partner");
...
//    usmlsTrig[ptbin][0]->GetYaxis()->SetRangeUser(-0.11,0.01);    npeTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001, 1600);
//    npeTrig[ptbin][2]->GetYaxis()->SetRangeUser(.001, 1700);
//
//
...
//    usmlsTrig[ptbin][2]->GetYaxis()->SetRangeUser(-0.11,0.01);
    ...
    npeTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001, 1600);
    npeTrig[ptbin][2]->GetYaxis()->SetRangeUser(.001, 1700);
    ...
    npeComp->cd(ptbin+1);
    if(ptbin<2)npeTrig[ptbin][0]  -> Draw("p");
    else npeTrig[ptbin][2]  -> Draw("same p");
