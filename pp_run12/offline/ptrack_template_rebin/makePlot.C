void drawtext();
void makePlot(const char* FileName="Dec31_1"){
...
//  gStyle->SetOptFit(1111);
  ...
  auto fyjd = new TFile(Form("FFOutput/sysChange_default_Mar3_1_FIT.root", FileName),"READ");
  auto hd = (TGraphAsymmErrors *)fyjd->Get("HT0");
  auto hnorm = (TGraphAsymmErrors *)fyjd->Get("normsmall");
  
  auto fyj = new TFile(Form("FFOutput/sysChange_fitNorm_Mar3_1_FIT.root", FileName), "READ");
  auto hbyj = (TGraphAsymmErrors *)fyj->Get("sys");
  auto hdef = (TGraphErrors*)fyj->Get("sysChange");

  //========================================================================
  
  char name[1200];
  sprintf(name,"FFOutput/sysChange_fitNormPlus_Mar3_1_FIT.root");
  auto fyjUp = new TFile(name,"READ");
  auto hbyjUp = (TGraphAsymmErrors *) fyjUp->Get("sys");
  auto hdefUp = (TGraphErrors*)fyjUp->Get("sysChange");

  sprintf(name,"FFOutput/sysChange_fitNormMinus_Mar3_1_FIT.root");
...
//  gStyle->SetOptStat(0);
  ...
  TCanvas* c1 = new TCanvas("c1","test",50,50,1200,1200);
  c1->cd();
...
//  leg->AddEntry(stat6,"Run 5/6 p+p 200 GeV","pe");
  ...
  leg->AddEntry(hdf,"2para. fit","pe");
  leg->AddEntry(hbyjUp,"Norm.(central+err)","pe");
  leg->AddEntry(hbyj,"Norm.(central)","pe");
  ...
  auto csys = new TCanvas("csys", "", 50,50,1200,1200);
  csys->cd();
