...
void PrintInc(){
  ...
  auto f  = new TFile("rootfile/default_gpt_dist_2d_v2.root", "READ");
  TNtuple *nf = (TNtuple*)f->Get("incCheck");
  Int_t entries = (Int_t)nf->GetEntries();
  char *ptbin = "1";
  char *filename = "zoom_inc";
  bool isZoom = kTRUE;
...
//    if(gpt>0.2 && gpt<0.3) {
      //        if(correct_dPhi(ElectronPhi-HadronPhi)>-0.02 && correct_dPhi(ElectronPhi-HadronPhi)<0.02 && ElectronEta-HadronEta >-0.01 && ElectronEta-HadronEta<0.01 )
