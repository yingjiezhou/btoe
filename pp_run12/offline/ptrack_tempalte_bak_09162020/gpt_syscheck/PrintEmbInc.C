...
void PrintEmbInc(){
  ...
  char *ptbin = "1";
  char *filename = "full_inc_emb_woTag_removeBadEvent_nTrk";
  bool isZoom = true;
  
  float lowpt = 2.5;
  float highpt = 3.5;
  
  auto f  = new TFile("Hist/DFF9129961F49C9E5633ED8BF554ADF5_0.root", "READ");
  TNtuple *nf = (TNtuple*)f->Get("incCheck");
  Int_t entries = (Int_t)nf->GetEntries();
  //===================================================
  
//  auto mh3DelPhiPhotUS2 = (TH3F*)f->Get("mh3DelPhiIncTrig1");
  //===================================================
//
  ...
  //  bool isULS;
  float isULS;
  float gpt,ElectronCharge,ElectronpT,ElectronEta,ElectronPhi,HadronCharge,HadronpT,HadronEta,HadronPhi,pCharge,pEta,pPhi,hpion,hkaon,hproton,helectron,trig,nsigmae,deltaeta,deltaphi,hpid,mcpt,hmcpt,mceta,hmceta,mcphi,hmcphi,mcdeta,mcdphi,mcy,hmcy,nTrk;
  ...
  nf->SetBranchAddress("dphi",&deltaphi);
  nf->SetBranchAddress("deta",&deltaeta);

  nf->SetBranchAddress("hpid",&hpid);
  nf->SetBranchAddress("mcpt",&mcpt);
  nf->SetBranchAddress("hmcpt",&hmcpt);
  nf->SetBranchAddress("mceta",&mceta);
  nf->SetBranchAddress("hmceta",&hmceta);
  nf->SetBranchAddress("mcphi",&mcphi);
  nf->SetBranchAddress("hmcphi",&hmcphi);
  nf->SetBranchAddress("mcdeta",&mcdeta);
  nf->SetBranchAddress("mcdphi",&mcdphi);
  nf->SetBranchAddress("mcy",&mcy);
  nf->SetBranchAddress("hmcy",&hmcy);
  nf->SetBranchAddress("nTrk",&nTrk);
  ...
  if(!isZoom){
    mh3DelPhiPhotLS = new TH3F("mh3DelPhiPhotLS","",600,-3.14,3.14,400,-2,2,200,0,20);
    mh3DelPhiPhotLS->Sumw2();
    mh3DelPhiPhotLS2 = new TH3F("mh3DelPhiPhotLS2","",600,-3.14,3.14,400,-2,2, 200,0,20);
    mh3DelPhiPhotLS2->Sumw2();

    mh3DelPhiPhotUS = new TH3F("mh3DelPhiPhotUS","",600,-3.14,3.14,400,-2,2, 200,0,20);
    mh3DelPhiPhotUS->Sumw2();
    mh3DelPhiPhotUS2 = new TH3F("mh3DelPhiPhotUS2","",600,-3.14,3.14,400,-2,2, 200,0,20);
    mh3DelPhiPhotUS2->Sumw2();
  }
  ...
  for (Int_t ki=1;ki<entries;ki++)
  {
    ...
    fprintf(finc,"%5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f\n", ElectronEta-HadronEta, correct_dPhi(ElectronPhi-HadronPhi),ElectronpT,ElectronEta,ElectronPhi,HadronpT,HadronEta,HadronPhi,hpid,mcpt,hmcpt,mceta,hmceta,mcphi,hmcphi,mcdeta,mcdphi,mcy,hmcy, nTrk);
...
//  usgptr->GetYaxis()->SetRangeUser(-0.1, 0.1);
//  usgptr->GetXaxis()->SetRangeUser(-0.1, 0.1);

  //=========================================================
  
  auto c1 = new TCanvas();
  if(!isZoom) c1->SetLogz();
  //  c1->Divide(2, 2, 0, 0, 0);
  //  c1->Divide(2);
  //  c1->cd(1);
  usgptr->Draw("colz");
