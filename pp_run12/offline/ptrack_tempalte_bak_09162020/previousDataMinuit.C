...
//void getPrelimnaryResult(TGraphAsymmErrors* pstat, TGraphAsymmErrors* psyst); // not working
...
//Double_t pC0[numPtBins],pC1[numPtBins];
...
Double_t Scale0[numPtBins],Scale2[numPtBins];
Double_t eScale0U[numPtBins],eScale2U[numPtBins], eScale0D[numPtBins],eScale2D[numPtBins];
...
void previousDataMinuit(const char* FileName="Mar3_1", const char* FileNameR="default")
{
...
//  Bool_t makePDF = checkMakePDF();
  ...
  if(makeROOT){
    sprintf(fname,"FFOutput/%s_FIT.root",FileNameR);
...
//  getPrelimnaryResult(pstat, psyst);
  
  char name[1000];
  //  sprintf(name,"/Users/zach/Research/pythia/200GeVTemplate/currentTemplate.root");
  sprintf(name,"/Users/zhouyingjie/Local/PWG/NPE/1_pp_zamiller/minBiasTemplates111915/readTreeOutput/input/pythia_npe_tree_Feb4.root");
  TFile *fT = new TFile(name,"READ");
  
  //  sprintf(name,"/Users/zach/Research/rootFiles/run12NPEhPhi/currentData.root");
  sprintf(name,Form("npe_tree_%s_processed.root", FileName));
  ...
  Double_t lowpt[numPtBins],highpt[numPtBins];
  for(Int_t c=0; c< numPtBins; c++){
    lowpt[c] = anaConst::lpt[c];
    highpt[c] = anaConst::hpt[c];
  }
  
  Double_t hptCut=anaConst::hptCut;
...
//        posDPhi = projB[ptbin]->GetXaxis()->FindBin(abs(binDPhi)); // get +dPhi bin
        ...
        binErr = projB[ptbin]->GetBinError(q);
        posBinErr = projB[ptbin]->GetBinError(posDPhi);
//        w1 = 1./(binErr*binErr);
//        w2 = 1./(posBinErr*posBinErr);
//        addErr = (w1*binVal+w2*binVal2)/(binVal+binVal2);
//        projB[ptbin]->SetBinError(q,addErr);
//        projB[ptbin]->SetBinError(posDPhi,addErr);
    ...
    if(highpt[ptbin] < 5)
    {
      ...
      Scale0[plotCount0] = p00[ptbin];
      eScale0U[plotCount0] = e00U[ptbin];
      eScale0D[plotCount0] = e00D[ptbin];
      plotCount0++;
    }
...
//      cout<<Rb0[plotCount0]<<" "<<eb0U[plotCount0]<<" "<<eb0D[plotCount0]<<endl;
      Scale0[plotCount0] = p20[ptbin];
      eScale0U[plotCount0] = e20U[ptbin];
      eScale0D[plotCount0] = e20D[ptbin];
      plotCount0++;
    }
...
//    gMinuit->SetFCN(chi2_C); // combine data fit zyj
...
//    plotCount++;
  ...
  Int_t l=0;
  char line[1000];
  Double_t xF[100],yF[100],minF[100],maxF[100];
  ...
  TCanvas* c2 = new TCanvas("c2","Scale factor",150,0,1150,1000);

//  TGraphErrors *gr0     = new TGraphErrors(plotCount0-1,ptOFF1,Rb0,dx,eb0);
  
  TGraphAsymmErrors* gr0 = new TGraphAsymmErrors(plotCount0-1,ptOFF1,Rb0,dx,dx,eb0D,eb0U);
  gr0->SetName("gMINOS");
...
//  TGraphErrors *grC     = new TGraphErrors(plotCount,pT,RbC,dx,ebC);
  //  TGraphErrors *grF     = new TGraphErrors(l-1,xF,yF);  // FONLL result zyj
  //  TGraphErrors *grFmax  = new TGraphErrors(l-1,xF,maxF);
  //  TGraphErrors *grFmin  = new TGraphErrors(l-1,xF,minF);
  TGraphErrors *grP     = new TGraphErrors(p-1,xP,yP,0,dyP); // preview's run5/6 result, only stat. error here zyj
  TGraphAsymmErrors *grPS = new TGraphAsymmErrors(p-1,xP,yP,dxPS,dxPS,dyPSL,dyPSH); //preview's run5/6 result, only sys. error here zyj
  
  TGraphErrors *grPr    = new TGraphErrors(2,pTP,RbP,0,EbP); //Run 12 Data, Run 5/6 Templates) zyj
  TGraphErrors *grPPr   = new TGraphErrors(2,pTPP,RbPP,0,EbPP); //Run 5/6 Refit (prev Template) zyj
  ...
  //    "FFOutput/sysChange_fitNormPlus_11_12_FIT.root", // zyj, should be for all sys. case?
  ...
  //  /*gr0->SetMarkerStyle(20); // bottom fraction, zyj
...
//  grC->SetMarkerStyle(24); // combined result HT0+HT2 , zyj
//  grC->SetMarkerSize(1);
//  grC->SetLineColor(kMagenta);
//  grC->SetMarkerColor(kMagenta);
//  //
  grP->SetMarkerSize(2.5); // preview result
  //  grF->SetLineStyle(1);
...
//  leg2->SetTextSize(.05);
  ...
  leg2->AddEntry(pstat,"STAR Prelimary Run 12","pe");
...
//  leg2->AddEntry(grPPr,"Run 5/6 Refit (prev Template)","pe");
  ...
  auto f1 = new TF1("f1", "pol0", 2, 12);
  c2->cd();
  gPad->SetLeftMargin(0.12);
  gN0->SetTitle("Norm. fit parameter");
  gN0->GetXaxis()->SetRangeUser(2, 12);
  gN0->GetXaxis()->SetTitle("NPE #it{p}_{T}(GeV/#it{c})");
  gN0->GetYaxis()->SetTitleOffset(1.1);  //1
  gN0->GetYaxis()->SetTitle("Norm factor");
  gN0->GetYaxis()->SetRangeUser(0, 3);
  gN0->Fit("f1");
  gN0->Draw("ape");
...
//      grFmin->Write("FONLLmin");
      pstat->Write("Run12stat");
      psyst->Write("Run12sys");
      gr0->Write("bfrac");
      gr0->Write("scale");
...
//      grSys->Write("grSys");
  ...
  if(makePDF)
  {
    ...
    sprintf(name, "%s_%s.pdf[", FileName, FileNameR);
    temp->Print(name);
    sprintf(name, "%s_%s.pdf", FileName, FileNameR);
