...
void offline_wPartnerCorr(const char* FileName="May5", const char* FileNameR="default", Bool_t doPurity = kFALSE, Bool_t doRec = kFALSE)
{
  ...
  else sprintf(name,"output/data/out_wPartnerCorr_noSplit_default.root"); // modified PHE loop
  if(doRecGpt) sprintf(name,"output/data/out_wPartnerCorr_noSplit_gpt.root");
  ...
  for(Int_t ptbin = 0; ptbin < numPtBins; ptbin++)
  {
    ...
    usmlsTrig[ptbin][0]->GetYaxis()->SetRangeUser(.001,0.13);
