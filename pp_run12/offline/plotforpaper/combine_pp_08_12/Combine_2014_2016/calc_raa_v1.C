void calc_raa(){
    ...
    // DUKE                                                                                                                                                                                                            
    TFile *fill1 = new TFile("NPE_MC/Ana/root/NULLer.root");
    ...
    TFile *fill = new TFile("NPE_MC/Ana/root/NULL.root");
    ...
    TFile * npeRaa = new TFile("NPE_MC/Ana/finalBtoERaa.root");
...
//    fb22->Draw("same E1");
    ...
    TFile* fi1 = new TFile("NPE_RAA/NPE_RAA.root");
    raa_stat= (TGraphErrors*)fi1->Get("raa_stat");
    raa_sys= (TGraphErrors*)fi1->Get("raa_sys");
    TFile *input_Total_Raa=new TFile("NPE_RAA/NPEHTRaa080-1.root","READ");
...
//proper asym method  
    ...
    for(int i= 5 ; i<9; i++){
	    ...
	    double null = null_hyp_bin->GetBinContent(i+1);
	...
	for(int j= 0 ; j<=i; j++){
	    ...
	    double systj = ratio_sys->GetErrorYlow(i);
	    ...
	    else if(fabs(i-j)==1) format3[i+j*4]= 0.25*systi*systj;
