void pl_delPhiPy(const char* FileName="Feb24")
{
  ...
  char name[1000];
  sprintf(name, "output/sim/pythia_npe_tree_May18.root", FileName);
  ...
  Float_t high[] = {0.3, 0.32, 0.32, 0.25, 0.25, 0.25, 1, 2, 1.6};
  ...
  //  c->Divide(3, 3);
  TLegend* leg = new TLegend(0.25,0.68,0.46,0.82);
  leg->SetBorderSize(0);
...
//    hdPhiRawce[i]->SetLineColor(kBlack);
    ...
    hdPhiRawce[i]->SetLineColor(kBlue);
    hdPhiRawce[i]->SetMarkerColor(kBlue);
  ...
  //  leg->AddEntry(hdPhiRawbe[3],"b#bar{b}#rightarrow NPE", "lp");
  //  leg->AddEntry(hdPhiRawce[3],"c#bar{c}#rightarrow NPE","lp");
  
  hdPhiRawce[2]->SetLineWidth(2);
  hdPhiRawbe[2]->SetLineWidth(2);

  hdPhiRawce[2]->GetYaxis()->SetTitleSize(0.08);
  hdPhiRawce[2]->GetYaxis()->SetTitleOffset(0.78);
  hdPhiRawce[2]->GetXaxis()->SetNdivisions(510);
  
  hdPhiRawce[2]->GetXaxis()->SetLabelSize(0.05);
  hdPhiRawce[2]->GetYaxis()->SetLabelSize(0.05);
  
  hdPhiRawce[2]->GetYaxis()->SetNdivisions(505);
  hdPhiRawce[2]->GetYaxis()->SetRangeUser(0, 0.22);
  hdPhiRawce[2]->GetXaxis()->SetRangeUser(-TMath::Pi(),TMath::Pi());
  
  hdPhiRawce[2]->SetLineStyle(6);
  hdPhiRawbe[2]->SetLineStyle(8);
  
  leg->AddEntry(hdPhiRawce[2],"D decay","l");
  leg->AddEntry(hdPhiRawbe[2],"B decay", "l");
  sprintf(label,"#it{p}_{T}#in [%1.1f,%1.1f] GeV/c",lowpt[2],highpt[2]);
  TCanvas *c2 = new TCanvas("c2","c2");
//  gPad->SetBottomMargin(0.02);
  gPad->SetTopMargin(0.08);
  c2->SetBorderMode(0);
  gPad->SetLeftMargin(0.15);
  hdPhiRawce[2]->Draw("hist ][");
  hdPhiRawbe[2]->Draw("same hist ][");
  leg->Draw("same");
  lat1.DrawLatex(-2.8, 0.2,label);
  char label2[100];
  TLatex lat2;
  sprintf(label2,"PYTHIA8 simulation");
  lat2.DrawLatex(0, 0.2,label2);
  c2->SaveAs("pl_delPhiPy.pdf");
