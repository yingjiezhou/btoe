void CalcPPRefComp(){
  ...
  double x3[7] = {2.5+0.1,3.5+0.1,4.5+0.1,5.5+0.1,6.5+0.1,7.5+0.1,8.5+0.1};
  double x2[7] = {2.5+0.2,3.5+0.2,4.5+0.2,5.5+0.2,6.5+0.2,7.5+0.2,8.5+0.2};
//  double x2[7] = {2.5+0.1,3.5+0.1,4.5+0.1,5.5+0.1,6.5+0.1,7.5+0.1,8.5+0.1};
  ...
  auto fy = new TFile("ppBfraction.root", "READ");
  auto h33 = (TH1F*)fy->Get("zach_stat");
  auto h33_sys = (TGraphAsymmErrors*)fy->Get("zach_sys");
  ...
  h3->GetXaxis()->SetTitle("p_{T} (GeV/#it{c})");
  h3->GetYaxis()->SetTitle("#it{b}#rightarrow e / NPE");
  
  h3->SetLineWidth(1);h2->SetLineWidth(1);h1->SetLineWidth(1);
  h2->SetLineColor(kRed);h2->SetMarkerColor(kRed);h2->SetMarkerStyle(24);
  h3->SetLineColor(kGreen+2);h3->SetMarkerColor(kGreen+2);h3->SetMarkerStyle(24);
  h2_sys->SetLineColor(kRed);h2_sys->SetMarkerColor(kRed);h2_sys->SetMarkerStyle(22);
  h3_sys->SetLineColor(kGreen+2);h3_sys->SetMarkerColor(kGreen+2);h3_sys->SetMarkerStyle(25);
...
//  leg->AddEntry(h2,"Run12(Yingjie)","PE");
  leg->AddEntry(h33,"Run6 & Run12 average(w/ prelimnary run12)","PE");
  leg->AddEntry(h3,"Run6 & Run12 average(w/ Yj run12)","PE");
...
//  h2_sys->Draw("same []");
  ...
  c1->SaveAs("Zach_YJ_comp.pdf");
//  auto sf = new TFile("ppBfraction.root", "RECREATE");
//  sf->cd();
//  h3->Write("Yingjie_stat");
//  h3_sys->Write("Yingjie_sys");
//  sf->Close();
}
