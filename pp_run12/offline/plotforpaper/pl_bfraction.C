...
void pl_bfraction(const char* FileName="pl_bfraction"){
...
//    "FFOutput/sysChange_pythia_May5_FIT.root",
  ...
  TString filename[20] = { "pileup", "purity", "rec", "gpt", "masscut", "nsigmae", "poe",  "trigEff", "fitrange", "pythia"};
  int nSys;
...
//       cout << "pt: " << x[i][ii] << " %: " << y[i][ii]<< " %: "<<sysEP[ii] << " %: " << sysEM[ii]<< endl;
      if(y[i][ii] > -1 && y[i][ii] < 1) // Add up the squares of all positive contrib in each pt
      {
        if(y[i][ii]>sysEP[ii] && y[i][ii]>sysEM[ii]){
          cout << "pt: " << x[i][ii] << " %: " << y[i][ii]<< " %: " << y[i][ii]-sysEP[ii] << " %: " << y[i][ii]-sysEM[ii]<< endl;
          sysP[ii] += (y[i][ii]-sysEP[ii])*(y[i][ii]-sysEP[ii]);
          sysM[ii] += (y[i][ii]-sysEM[ii])*(y[i][ii]-sysEM[ii]);
        }
        else {
          sysP[ii] +=0;
          sysM[ii] +=0;
...
//  sprintf(name, "Plots/sys_change_%s.pdf[", FileName);
//  temp->Print(name);
//  sprintf(name, "Plots/sys_change_%s.pdf", FileName);
//  for(int i = 0; i < sysNum; i++){
//    temp=fp[i];
...
//  grPS->Draw("same pe2");
  ...
  fpp->SaveAs(Form("%s.pdf", FileName));
  return ;
}
