void runget_sym_bin_total(const char* FileName="AfterSub_pythia_npe_tree_May18")
{
  ...
  //  TFile *fsy = new TFile(Form("Done_%s.root", FileName), "RECREATE");
  ...
  for(int i=0; i<9; i++){
    ...
    hdPhiRawbe[i] = (TH1D* )fpy->Get(Form("hdPhiRawceJpsi_%i", i));
    hEventTallyce[i] = (TH1D* )fpy->Get(Form("ceEventTally_%i",i));
    hEventTallybe[i] = (TH1D* )fpy->Get(Form("ceEventTallyJpsi_%i",i));
...
//    hdPhiRawbe[i]->Draw("same PE");
    ...
    leg->AddEntry(hdPhiRawce[i],"Jpsi #rightarrow ee", "lp");
//    leg->AddEntry(hdPhiRawce[i],"c#bar{c}#rightarrow NPE","lp");
  ...
  sprintf(name, "Total_Scale_afterSym_%s.pdf[", FileName);
  temp->Print(name);
