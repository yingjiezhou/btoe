void runget_sym_fig(const char* FileName="Feb24")
{
...
//    c->Update();
  ...
  sprintf(label,"#it{p}_{T}#in [%1.1f,%1.1f] GeV/c",lowpt[3],highpt[3]);
  TCanvas *c2 = new TCanvas("c2","c2");
  hdPhiRawce[3]->Draw("PE");
  hdPhiRawbe[3]->Draw("same PE");
  leg->Draw("same");
  lat1.DrawLatex(-3, 0.03,label);
  char label2[100];
  TLatex lat2;
  sprintf(label2,"PYTHIA8 simulation");
  lat2.DrawLatex(-3, 0.35,label2);
//  c2->SaveAs("sim_Sample.pdf");
