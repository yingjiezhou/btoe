void templ_MinusJpsieMB_AddJpsie(const char* FileName="pythia_npe_tree_May18", const char* FileName2="pythia_npe_dist_Jun9_old", const char* FileName3="Jpsi_sys/rootfile/Jpsie_Singlet")
{
  // npe(pythia_npe_tree_May18.root) - jpsi(pythia_npe_dist_Jun9_old.root) from MB;
  // add single jpsi Jpsie_Singlet.root from charmonium;
...
//  gStyle->SetOptStat(0);
  ...
  sprintf(name, "%s.root", FileName3);
  TFile *fpy22 = new TFile(name,"READ");
...
//  TH1D *hdPhiRawbeSym[14];
  ...
  TH1D *h22dPhiRawce[14];
  TH1D *h22dPhiRawbe[14];
  TH1D *h22EventTallyce[14];
  TH1D *h22EventTallybe[14];
  
//  TFile *fsy = new TFile(Form("AfterSub_%s.root", FileName), "RECREATE");
  
  TFile *fsy = new TFile("output/pythia_npe_tree_May18_hf_SingletJpsi.root", "RECREATE");
...
//  c->Divide(3, 3);
  ...
  for(int i=0; i<9; i++){
    ...
    h22dPhiRawce[i] = (TH1D* )fpy22->Get(Form("hdPhiRawce_%i", i));
//    h22dPhiRawbe[i] = (TH1D* )fpy22->Get(Form("hdPhiRawbe_%i", i));
    h22EventTallyce[i] = (TH1D* )fpy22->Get(Form("ceEventTally_%i",i));
    h22EventTallybe[i] = (TH1D* )fpy22->Get(Form("beEventTally_%i",i));
...
//    hEventTallybe[i]->Add(h2EventTallybe[i], -1);
  ...
  for(int i=0; i<9; i++){
    hdPhiRawceJpsi[i] = (TH1D*)h22dPhiRawce[i]->Clone(Form("hdPhiRawceJpsi_%i", i));
//    hdPhiRawceJpsi[i]->Add(h22dPhiRawbe[i], 1);
    
    hEventTallyceJpsi[i] = (TH1D*)h22EventTallyce[i]->Clone(Form("ceEventTallyJpsi_%i",i));
//    hEventTallyceJpsi[i]->Add(h22EventTallybe[i], 1);
  }
