...
void templ_jpsie_h(const char* FileName="Jpsie_Singlet", const char* FileName2="Jpsie_Octet")
{
...
//  Float_t highpt[5] = {2, 4, 6, 8, 12};
  ...
  for(int i=0; i<6; i++){
    ...
    hdPhiRawce[i]->GetXaxis()->SetTitle("#Delta#phi_{J/#psi#rightarrow e -h}");
    hdPhiRawce[i]->GetYaxis()->SetTitle("#frac{1}{N_{J/#psi#rightarrow e}}#frac{dN}{d(#Delta#phi)}");
...
//    leg->AddEntry(h2dPhiRawbe[i],"b#bar{b}#rightarrow NPE(Octet)", "lp");
...
//    drawLatex(0.18, 0.52, "|y_{J/#psi}| < 1.", 22, 0.05, 1);
    ...
    if(i <9) lat1.DrawLatex(-3, 0.2,label);
