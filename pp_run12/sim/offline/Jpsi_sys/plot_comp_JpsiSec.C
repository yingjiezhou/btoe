#include "/Users/zhouyingjie/style.h"
void plotCompJpsiSec(){
  setstyle();
//  gROOT->ProcessLine(".x ~/myStyle.C");
  TFile* file = new TFile("rootfile/JpsidNdydpTSpectrumpp.root","READ");
  auto GJpsi = (TGraphErrors*) file->Get("Jpsi");
  
  auto hFunJpsierr = (TH1F*) file->Get("FunJpsierr");
//  hFunJpsierr->Rebin(4);
//  hFunJpsierr->Scale(1./4.);
  
  auto f = new TFile("rootfile/Jpsi_sec.root", "READ");
  auto hjeptN = (TH1D*)f->Get("hjeptN");
  
  hjeptN->Scale(0.0594); //jpsi decay fraction
...
//  auto tmp = 5.13e-7/hjeptN->GetBinContent(hjeptN->FindBin(2.5));
  auto tmp = 1.;
  ...
  TH1F *htmp=histo("htmp",20,0,6,1e-15,1e-3,"p_{T} (GeV/c)","d^{2}#sigma/(2#pip_{T}dp_{T}dy) (mb GeV^{-2} c^{3})",0.075,0.85,0.065,1.15);
  ...
  Style(hFunJpsierr,1,kBlue+1,3,8,kBlue+1,0.9,"psame");
...
//  leg->AddEntry(hjeptN,Form("J/#psi#rightarrow#it{e^{+}e^{-}}(from Yingjie*%.2f)", tmp),"p");
  leg->AddEntry(hjeptN,"J/#psi#rightarrow#it{e^{+}e^{-}}(from Yingjie)","p");
...
//  cc->SaveAs("pic/JpsiSec_Jpsi_over_NPE.pdf");
  //============================
  auto c = new TCanvas();
  c->cd();
//  auto hJpsiNPEratio  = (TH1F*)hjeptN->Clone("hJpsiNPEratio");
//  cout<<hJpsiNPEratio->GetBinContent(hJpsiNPEratio->FindBin(5.))<<endl;
//  cout<<hFunJpsierr->GetBinContent(hFunJpsierr->FindBin(5.))<<endl;
//  hJpsiNPEratio->GetXaxis()->SetRangeUser(0. ,20);
//  hFunJpsierr->GetXaxis()->SetRangeUser(0. ,20);
  auto htmpr = (TH1F*)hFunJpsierr->Clone("htmpr");
  for(auto i = 0; i<hFunJpsierr->GetNbinsX(); i++){
    if(i<2000) htmpr->SetBinContent(i+1, hjeptN->GetBinContent(i+1));
    else htmpr->SetBinContent(i+i,1.);
  }
  
  
//  cout<<hJpsiNPEratio->GetNbinsX()<<endl;
//  cout<<hFunJpsierr->GetNbinsX()<<endl;

  htmpr->Divide(hFunJpsierr);
  htmpr->Rebin(10);
  htmpr->Scale(0.1);
  htmpr->GetXaxis()->SetRangeUser(0, 6);
  htmpr->GetXaxis()->SetTitle("p_{T} (GeV/c)");
  htmpr->GetYaxis()->SetTitle("YJ/Shenghui");
  htmpr->Draw();
  c->SaveAs("JpsiSec_YJ_shenghui.pdf");
//  hFunJpsierr->Draw("same");
//
//  return;
}
