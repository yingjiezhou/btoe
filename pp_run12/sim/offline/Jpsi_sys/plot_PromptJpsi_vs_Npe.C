void plotPrompt_JpsiNpeRatio(const char* FileName="jpsi_newTune", const char* FileName2="hf_newTune")
{
...
//         cout<<hEventTallybe[i]->GetBinContent(1)+hEventTallyce[i]->GetBinContent(1)<<"    "<<hEventTallybe2[i]->GetBinContent(1)+hEventTallyce2[i]->GetBinContent(1)<<endl;
    auto rt = hEventTallyce[i]->GetBinContent(1);
    rt /= (hEventTallyce2[i]->GetBinContent(1) //);
           +hEventTallyce[i]->GetBinContent(1));
//           +hEventTallybe2[i]->GetBinContent(1)+hEventTallybe[i]->GetBinContent(1));
...
//  hjpsiNPEr->GetYaxis()->SetTitle("prompt J/#psi#rightarrow e/(c+b)#rightarrow NPE");
  hjpsiNPEr->GetXaxis()->SetRangeUser(2, 10);
  hjpsiNPEr->GetYaxis()->SetRangeUser(0, 1);
  hjpsiNPEr->Draw("pe");
