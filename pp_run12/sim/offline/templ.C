void templ(const char* FileName="npe_wE_EE")
{
  ...
  const char* PdfFileName= "templComp_wE_EE";
  char name[1000];
  ...
  Float_t high[] = {0.06, 0.06, 0.06, 0.1, 0.1, 0.25, 0.5, 0.5, 0.5};
...
//      hdPhiRawce[i]->SetTitle("");
    ...
    if(i <3) lat1.DrawLatex(-3, 0.04,label);
    if(i >2) lat1.DrawLatex(-3, 0.08,label);
