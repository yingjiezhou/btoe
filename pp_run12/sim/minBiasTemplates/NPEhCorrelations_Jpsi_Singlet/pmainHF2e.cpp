  ...
  while (ievent < maxNumberOfEvents) {
    ...
    for (int i = 0; i < event.size(); i++) {                                      //event.size means total track number
      ...
      if (abs(event[i].id()) == 443) {
        ...
        //                cout<<"jpsi mother particle id :  "<<ic_id<<" and flavor is: "<<flavor<<endl;
        //                cout<<"grandmothers.size() : "<<grandmothers.size()<<endl;
        ...
        if(grandmothers.size()==1)
        {
          ...
          //                    cout<<"jpsi grandmother particle id :  "<<orig_id<<" and flavor is: "<<c<<endl;
          if(flavor_grandmon == 5)
          {
            hf2eDecay.bce_id[nbce] = event[ielectron].id();
            hf2eDecay.bce_status[nbce] = event[ielectron].status();
            hf2eDecay.bce_pt[nbce] = event[ielectron].pT();
            hf2eDecay.bce_pz[nbce] = event[ielectron].pz();
            hf2eDecay.bce_phi[nbce] = event[ielectron].phi();
            hf2eDecay.bce_eta[nbce] = event[ielectron].eta();
            hf2eDecay.bce_y[nbce] = event[ielectron].y();
            hf2eDecay.bceMom_id[nbce] = ic_id;
            nbce++;
            
            hf2eDecay.bce_id[nbce] = event[ipositron].id();
            hf2eDecay.bce_status[nbce] = event[ipositron].status();
            hf2eDecay.bce_pt[nbce] = event[ipositron].pT();
            hf2eDecay.bce_pz[nbce] = event[ipositron].pz();
            hf2eDecay.bce_phi[nbce] = event[ipositron].phi();
            hf2eDecay.bce_eta[nbce] = event[ipositron].eta();
            hf2eDecay.bce_y[nbce] = event[ipositron].y();
            hf2eDecay.bceMom_id[nbce] = ic_id;
            nbce++;
          }
          else
          {
            hf2eDecay.ce_id[nce] = event[ielectron].id();
            hf2eDecay.ce_status[nce] = event[ielectron].status();
            hf2eDecay.ce_pt[nce] = event[ielectron].pT();
            hf2eDecay.ce_pz[nce] = event[ielectron].pz();
            hf2eDecay.ce_phi[nce] = event[ielectron].phi();
            hf2eDecay.ce_eta[nce] = event[ielectron].eta();
            hf2eDecay.ce_y[nce] = event[ielectron].y();
            hf2eDecay.ceMom_id[nce] = ic_id;
            nce++;
            
            hf2eDecay.ce_id[nce] = event[ipositron].id();
            hf2eDecay.ce_status[nce] = event[ipositron].status();
            hf2eDecay.ce_pt[nce] = event[ipositron].pT();
            hf2eDecay.ce_pz[nce] = event[ipositron].pz();
            hf2eDecay.ce_phi[nce] = event[ipositron].phi();
            hf2eDecay.ce_eta[nce] = event[ipositron].eta();
            hf2eDecay.ce_y[nce] = event[ipositron].y();
            hf2eDecay.ceMom_id[nce] = ic_id;
            nce++;
          }
