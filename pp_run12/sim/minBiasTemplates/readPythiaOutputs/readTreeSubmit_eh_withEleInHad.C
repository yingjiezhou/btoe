...
void readTreeSubmit_withE(TString filelist="npe.list",TString outFile="ehCorr_withE.root")
{
  ...
  for(Int_t ptbin = 0; ptbin < numPtBins; ptbin++)
  {
    ...
    hdPhiRawce[ptbin] = new TH1D(Form("hdPhiRawce_%i",ptbin),"hdPhiRawce",100,-TMath::Pi(),TMath::Pi());
    hdPhiRawbe[ptbin] = new TH1D(Form("hdPhiRawbe_%i",ptbin),"hdPhiRawbe",100,-TMath::Pi(),TMath::Pi());
    hdPhiRawbce[ptbin] = new TH1D(Form("hdPhiRawbce_%i",ptbin),"hdPhiRawbce",100,-TMath::Pi(),TMath::Pi());
  ...
  for(int i = 0; i < nentries; i++){
    ...
    int ceNtrigcount=0;
    maxptbin = 0;
    totalh = numberofcElectrons+ numberofbcElectrons +numberofbElectrons +numberofHadrons;
    ...
    for(int trki = 0; trki < numberofcElectrons; trki++){
      ...
      for(int trkj = 0; trkj < totalh; trkj++){
        if(trkj < numberofHadrons){
          ...
          if(trkj>=numberofHadrons && trkj<numberofHadrons+numberofcElectrons){
            assoh_id = (int)leaf_ce_id->GetValue(trkj-numberofHadrons);
            assoh_status  = (int)leaf_ce_status->GetValue(trkj-numberofHadrons);
            assoh_pt      = leaf_ce_pt->GetValue(trkj-numberofHadrons);
            assoh_pz      = leaf_ce_pz->GetValue(trkj-numberofHadrons);
            assoh_phi     = leaf_ce_phi->GetValue(trkj-numberofHadrons);
            assoh_eta     = leaf_ce_eta->GetValue(trkj-numberofHadrons);
            assoh_y       = leaf_ce_y->GetValue(trkj-numberofHadrons);
          }
          if(trkj>=numberofHadrons+numberofcElectrons && trkj<numberofHadrons+numberofcElectrons+numberofbElectrons){
            assoh_id = (int)leaf_be_id->GetValue(trkj-(numberofHadrons+numberofcElectrons));
            assoh_status  = (int)leaf_be_status->GetValue(trkj-(numberofHadrons+numberofcElectrons));
            assoh_pt      = leaf_be_pt->GetValue(trkj-(numberofHadrons+numberofcElectrons));
            assoh_pz      = leaf_be_pz->GetValue(trkj-(numberofHadrons+numberofcElectrons));
            assoh_phi     = leaf_be_phi->GetValue(trkj-(numberofHadrons+numberofcElectrons));
            assoh_eta     = leaf_be_eta->GetValue(trkj-(numberofHadrons+numberofcElectrons));
            assoh_y       = leaf_be_y->GetValue(trkj-(numberofHadrons+numberofcElectrons));
          }
          if(trkj>=numberofHadrons+numberofcElectrons+numberofbElectrons && trkj<numberofHadrons+numberofcElectrons+numberofbElectrons+numberofbcElectrons){
            assoh_id = (int)leaf_bce_id->GetValue(trkj - (numberofHadrons+numberofcElectrons+numberofbElectrons));
            assoh_status  = (int)leaf_bce_status->GetValue(trkj - (numberofHadrons+numberofcElectrons+numberofbElectrons));
            assoh_pt      = leaf_bce_pt->GetValue(trkj- (numberofHadrons+numberofcElectrons+numberofbElectrons));
            assoh_pz      = leaf_bce_pz->GetValue(trkj - (numberofHadrons+numberofcElectrons+numberofbElectrons));
            assoh_phi     = leaf_bce_phi->GetValue(trkj - (numberofHadrons+numberofcElectrons+numberofbElectrons));
            assoh_eta     = leaf_bce_eta->GetValue(trkj - (numberofHadrons+numberofcElectrons+numberofbElectrons));
            assoh_y       = leaf_bce_y->GetValue(trkj - (numberofHadrons+numberofcElectrons+numberofbElectrons));
          }
        }
        if(celectron_id == assoh_id) continue;
    ...
    for(int trki = 0; trki < numberofbElectrons; trki++){
      ...
      for(int trkj = 0; trkj < totalh; trkj++){
        ...
        if(belectron_id == assoh_id) continue;
        if(assoh_eta > hEtaCut || assoh_eta < -hEtaCut) continue;
    ...
    for(int trki = 0; trki < numberofbcElectrons; trki++){
      ...
      for(int trkj = 0; trkj < totalh; trkj++){
        ...
        if(bcelectron_id == assoh_id) continue;
        if(assoh_eta > hEtaCut || assoh_eta < -hEtaCut) continue;
