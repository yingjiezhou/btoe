void plotDirectJpsi_comp(){
  ...
  TFile* file = new TFile("rootfile/rootfile_NPESec_fromData/JpsidNdydpTSpectrumpp.root","READ");
...
//  TGraph* gs = new TGraph(8, xs, ys); // from shenghui, jpsi cross section
  ...
  auto ft = new TFile("rootfile/directJpsi_MB_withTree.root", "READ");
  ...
  auto ftc = new TFile("rootfile/directJpsi_HQbc_charmonium.root", "READ");
  auto h1tc = (TH1D*) ftc->Get("hjeptN"); // jpsi pt spectrum
  h1tc->Rebin(10);
  
  for(int i = 0; i< h1tc->GetNbinsX(); i++){
    double pt = h1tc->GetBinCenter(i+1);
    double tmpp = 1./(2* TMath::Pi() * pt* h1tc->GetBinWidth(i)); ////!!!!! notice here divide by dpT
    tmpp = tmpp / 1. ; // rapidy
    tmpp = tmpp/1000. * 0.059; // number of root file
    h1tc->SetBinContent(i+1, h1tc->GetBinContent(i+1) * tmpp);
    h1tc->SetBinError(i+1, h1tc->GetBinError(i+1)*tmpp);
  }
  
  h1tc->SetMarkerColor(kBlue+2);
  h1tc->SetLineColor(kBlue+2);
  //=====================
...
//  leg->AddEntry(gs,"Data, J/#psi#rightarrow#it{e^{+}e^{-}}(from shenghui)","l");
  ...
  leg->AddEntry(h1,"PYTHIA,STAR HF tune v1.1 MB, direct J/#psi#rightarrow#it{e^{+}e^{-}}","p"); // use my old tune, to get direct jpsi
  leg->AddEntry(h1t,"updated, PYTHIA,STAR HF tune v1.1 MB, direct J/#psi#rightarrow#it{e^{+}e^{-}}","p"); // use my old tune, to get direct jpsi
  leg->AddEntry(h1tc,"updated, PYTHIA, HF v1.1 HQc+charmonium, direct J/#psi#rightarrow#it{e^{+}e^{-}}","p"); // use HQ, charmonium, to get direct jpsi
