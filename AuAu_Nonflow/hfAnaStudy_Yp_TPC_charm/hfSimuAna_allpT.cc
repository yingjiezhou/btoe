...
double ptBin[nPtBins+1]={2.0, 4.0, 6.0, 10.0, 11.0, 12.0};
  ...
  for(int iEvent=0; iEvent<nevents; iEvent++) {
    ...
    for(int iElec=0; iElec<numOfElectrons; iElec++) {
      ...
      for(int iBH=0; iBH<numOfHadrons; iBH++) {
        ...
        if(fabs(pdgCode)!=211 && fabs(pdgCode)!=321 && fabs(pdgCode)!=2212) continue; //only consider charged pions, kaons, protons
        ...
        if(pSave.Perp() < hadronPtCutMin || pSave.Perp() > hadronPtCutMax) continue;
