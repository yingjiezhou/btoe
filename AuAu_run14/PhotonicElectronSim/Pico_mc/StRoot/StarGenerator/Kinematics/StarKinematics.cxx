  ...
  for ( Int_t i=0; i<ntrack; i++ )
    {
      ...
      TVector3 momentum; {
        //momentum.SetPtEtaPhi( pt, eta, phi );
        momentum.SetXYZ(pt*TMath::Cos(phi), pt*TMath::Sin(phi), sqrt((pt*pt + m*m) * TMath::TanH(eta) * TMath::TanH(eta) / (1-TMath::TanH(eta)*TMath::TanH(eta))) * (eta < 0 ? -1 : 1)); // flat Rapidity mode
        }
