...
StPxlSimMaker::StPxlSimMaker(const Char_t* name) : StMaker(name) , mPxlSimulator(0), mUseFastSim(kFALSE), mUseFastSimRaw(kFALSE), mUseDIGMAPSSim(kFALSE) , mUseIdealGeom(kTRUE), mUseDbGeom(kFALSE), mUseRandomSeed(kFALSE),mAddPileup(kFALSE),mAddPileup2(kFALSE)
{
}
...
Int_t StPxlSimMaker::Init()
{
   ...
   if(mAddPileup2)
   {
           mPileupAdder2 = new StPxlPileupAdder();
   }
...
Int_t StPxlSimMaker::InitRun(Int_t RunNo)
{
  ...
  if(mAddPileup2)
   {
           mPileupAdder2->init(mPileupFile2,pxlDbDataSet);
   }
...
Int_t StPxlSimMaker::Make()
{
   ...
   if(mAddPileup2)
   {
           mPileupAdder2->addPxlHits(*mcPxlHitCol);
   }
