...
StMcElectronTrack::StMcElectronTrack()
{
...
Int_t StMcElectronTrack::GetEntries()
{
  if(fChain) return fChain->GetEntriesFast();
  return 0; 
}

Int_t StMcElectronTrack::GetEntry(Long64_t entry)
{
  if (!fChain) return 0;                  
  return fChain->GetEntry(entry);
}           
void StMcElectronTrack::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("isHFT", &isHFT, &b_isHFT);
   fChain->SetBranchAddress("mcHFT", &mcHFT, &b_mcHFT);
   fChain->SetBranchAddress("trueHFT", &trueHFT, &b_trueHFT);
