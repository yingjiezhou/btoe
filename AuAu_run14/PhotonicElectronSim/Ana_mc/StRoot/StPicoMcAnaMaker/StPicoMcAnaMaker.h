...
class StPicoMcAnaMaker : public StMaker
{
 public: 
  ...
  bool trueHftTrack(StPicoMcTrack const * const trk);
...
inline bool StPicoMcAnaMaker::trueHftTrack(StPicoMcTrack const * const trk)
{
  if(trk->Pxl1Truth()==0 || trk->Pxl2Truth()==0 || trk->IstTruth()==0 ) 
    return false;
  return true;
}
