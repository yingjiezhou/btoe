  ...
  mEta(std::numeric_limits<float>::quiet_NaN()), mMcMomentum(0.0,0.0,0.0), mOrigin(0.0,0.0,0.0)
{
  ...
  if(mcTrk->parent()){
    ...
    //test by xiaozhi
    //    if(mcTrk->parent()->geantId()==1)
     // if(mcTrk->geantId()==4)

     //   cout<<" parent"<< mParentId<<endl;

    cout<<     mcTrk->parent()->geantId()<<endl;
   
    
    
     // if(mctrack(mcTrk->parentId())//    ))) ->GePid()) 
     // mctrack(mcTrk1->parentId()))) ->GePid()<<endl;
       // {    
       // 	 cout<<   mcTrk->parent()->key()<<endl;
       // 	 cout<<" daughter  "<<mcTrk->key()<<endl;
       // 	 throw (-1);
       // }
