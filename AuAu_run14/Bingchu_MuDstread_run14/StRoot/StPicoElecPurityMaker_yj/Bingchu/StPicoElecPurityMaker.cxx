...
Int_t StPicoElecPurityMaker::Finish() {
  ...
  for(int t=0; t<4; t++) // THnSparse must be manually written to file
  {
    ...
    //   mnsigmaE_pT_all[t][1]->Write(); // HFT 的为啥不要？
...
Int_t StPicoElecPurityMaker::FillHistograms(Int_t trig, StPicoEvent* event)
{
  ...
  if(!pass_HFT_EventCuts(event,trig))  // zyj revised
  ...
  // For sample overlap
  if(trig == 0) trigCounter += 1; // 这个和  hNEvents[trig]->Fill(2)的数值;对比就可以看出是不是有的trigger 的代码是一样的
  ...
  if(fillhistflag){
    ...
    mVz_tpc[trig]->Fill(vztpc); // 虽然在这里填了，但是没有在finish 函数里面fout->cd(); write()
...
*/
  ...
  for(int i=0; i<numberoftracks; i++){
    ...
    if(isGoodTrack&&bPass_HFT_GoodEvents_Cuts) // 不满足HFT event cut，
      hNTracks[trig]->Fill(4);

    if(isGoodTrack&&bPass_HFT_GoodEvents_Cuts&&HFT_trackflag)// 不满足HFT event cut（更紧的cut），是HFT trakc
    ...
    mnsigmaE_pT_all[trig][0]->Fill(mpt, nsige );// 不经过sigma 的判选，所以是all，是inclusive 的e， 不是pure e，里面还包含了mis ID 的hadron
    if(fabs(nsigpi)<1)
      {
    ...
    if(HFT_trackflag&&bPass_HFT_GoodEvents_Cuts)
      {
	mnsigmaE_pT_all[trig][1]->Fill(mpt, nsige ); // 不需要经过sigma cut 的
	if(fabs(nsigpi)<1)
	  {
	...
	if(fabs(nsigk)<2)
	  {
	    ...
	    mnsigmaK_diff[trig][1] ->Fill(mpt,nsigk-nsige ); // I add a _diff
	  }
	if(fabs(nsige)<2)
	  {
    ...
    // below is e selection  zyj
    // 那其实没必要存dca 了，因为inclusive 分析代码中存了，或者说还有别的用处？
    double sparseFill[10] = {mpt, nsige, nsigpi,nsigp,nsigk,meta,cent9_grefmult,event->primaryVertex().z(),dcaXY,dcaZ}; // 各个量都拿到了
	...
	mnSigmaE_TOF[trig][0]->Fill(sparseFill, reweight); // 这个和上面的重复填图了吧？？有问题
	if(trkHFTflag == 1&&bPass_HFT_GoodEvents_Cuts)
          {
            mnSigmaE_TOF[trig][trkHFTflag]->Fill(sparseFill, reweight);
          }
...
// --------------- dVz Study Loop ---------------
Int_t StPicoElecPurityMaker::dVzStudy(StPicoEvent* event){ //相对于FillHistograms 简化版的函数，只是填了Vz信息，而且是经过各种BEMC，SMD cut 之后的信息
...
Bool_t StPicoElecPurityMaker::checkTriggers(StPicoEvent *ev, int trigType)
{
  ...
  {
    if(ev->isTrigger(*trg)) // 跟你你自己在外面设置的每个trigger对应的数字,默认的vector<int> triggers[5]; //0-HT0, 1-HT1 ... 4-MB 所以如果你不在外面设置的话，一定是0，最后就是你在外面给什么， 她就会找你要的trigger 的事件
      return true;
  }
  return false;
}
...
Bool_t StPicoElecPurityMaker::passBEMCCuts(StPicoEvent* event, StPicoTrack* track, int trig)
{
  ...
  if( mpt > bemcPtCut && mpoe > poeCutLow && mpoe < poeCutHigh && adc0>getadc0Cut(trig)
     && dsmadc> getDsmAdcCut(trig))
...
{ // 不区分不同的trigger 之前adc0 的区别
  ...
  if(emcpidtraitsid>=0){
    ...
    e0     = emcpidtraits->e0();
    btowId = emcpidtraits->btowId();
    zDist = emcpidtraits->zDist();// 放到SMD 里面用了
    phiDist = emcpidtraits->phiDist();
    nEta = emcpidtraits->nEta();
    nPhi = emcpidtraits->nPhi();
    mpoe = track->gMom(event->primaryVertex(),event->bField()).mag()/emcpidtraits->e0();
...
//----------------------------------------------------------------
Bool_t StPicoElecPurityMaker::passEventCuts_NodVz(StPicoEvent* event, int trig) //所有trigger 都没有dVz cut，这里只会在trigger = 3 的情况下使用，所以功能和前面的函数没什么区别
{
  Double_t vzvpd = event->vzVpd();
  Double_t vztpc = event->primaryVertex().z();
  Double_t dvz = vzvpd - vztpc;
...
{ 
  ...
  setPoECut(0.3, 1.5); // 0.3 < p/E < 1.5 yes
  ...
  setSMDCuts2(1,1,3.,0.08); // nEta>=, nPhi>=, zDist<, phiDist<  //更符合是PHE 的cut
}
