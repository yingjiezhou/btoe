...
void StPicoElecPurityMaker::DeclareHistograms() {
  ...
  for(int tr=0; tr<numTrigs; tr++)
  {
    ...
    HadronDcaZ_Pt[tr][1] = new TH2F(Form("HadronDcaXy_Pt_%i",tr),"",400,0,20,400,DcaLim_l,DcaLim_h);
    PiDcaXy_Pt[tr][0] = new TH2F(Form("PiDcaXy_Pt_%i",tr),"",400,0,20,400,DcaLim_l,DcaLim_h);
    PiDcaZ_Pt[tr][1] = new TH2F(Form("PiDcaZ_Pt_%i",tr),"",400,0,20,400,DcaLim_l,DcaLim_h);
    KaonDcaXy_Pt[tr][0] = new TH2F(Form("KaonDcaXy_Pt_%i",tr),"",400,0,20,400,DcaLim_l,DcaLim_h);
    KaonDcaZ_Pt[tr][1] = new TH2F(Form("KaonDcaZ_Pt_%i",tr),"",400,0,20,400,DcaLim_l,DcaLim_h);
    mnsigmaE_pT_all[tr][0] = new TH2F(Form("mnsigmaE_pT_all%i",tr),"",400,0,20,400,3000,-10,8);
    mnsigmaE_pT_all[tr][1] = new TH2F(Form("mnsigmaE_pT_HFT_all%i",tr),"",400,0,20,3000,-10,8);
    TOF_beta_p_all[tr][0] = new TH2F(Form("TOF_beta_p_all%i", tr), "",400,0,20,600,0,3);
    TOF_beta_p_all[tr][1] = new TH2F(Form("TOF_beta_p_all%i", tr), "",400,0,20,600,0,3);
...
Int_t StPicoElecPurityMaker::FillHistograms(Int_t trig, StPicoEvent* event)
{
  ...
  for(int i=0; i<numberoftracks; i++){
    ...
    // track dca info
    //tpc pid on pi, k, p,
    HadronDcaXy_Pt[tr][0]->Fill(mpt,dcaXY);
    HadronDcaZ_Pt[tr][0]->Fill(mpt,dcaZ);
    
    if(fabs(nsigpi)<2.0) {
      PiDcaXy_Pt[tr][0]->Fill(mpt,dcaXY);
      PiDcaZ_Pt[tr][0]->Fill(mpt,dcaZ);
    }
    if(fabs(nsigk)<2.0){
      KaonDcaXy_Pt[tr][0]->Fill(mpt,dcaXY);
      KaonDcaZ_Pt[tr][0]->Fill(mpt,dcaZ);
    }
    
    //tpc pid on pi, k, p---wHFT
    if(HFT_trackflag&&bPass_HFT_GoodEvents_Cuts){ // I will use this infromation zyj
      HadronDcaXy_Pt[tr][1]->Fill(mpt,dcaXY);
      HadronDcaZ_Pt[tr][1]->Fill(mpt,dcaZ);
      if(fabs(nsigpi)<2.0) {
        PiDcaXy_Pt[tr][1]->Fill(mpt,dcaXY);
        PiDcaZ_Pt[tr][1]->Fill(mpt,dcaZ);
      }
      if(fabs(nsigk)<2.0){
        KaonDcaXy_Pt[tr][1]->Fill(mpt,dcaXY);
        KaonDcaZ_Pt[tr][1]->Fill(mpt,dcaZ);
      }
    }
    ...
    if(trig==0&&passTOFCuts(event, track,trig)&&passBEMCCuts_noTrigger(event,track,tri))
    {
      ...
      if(tofpidid>=0){
        ...
        TOF_beta_p_all[tr][0]->Fill(mpt,tofbeta);
        if(HFT_trackflag&&bPass_HFT_GoodEvents_Cuts)
        {
          TOF_beta_p_all[tr][1]->Fill(mpt,tofbeta);
        }
      }
    }
