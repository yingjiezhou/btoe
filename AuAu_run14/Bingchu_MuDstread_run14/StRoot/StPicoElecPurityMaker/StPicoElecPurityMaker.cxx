...
Int_t StPicoElecPurityMaker::Finish() {
  ...
  for(int t=0; t<4; t++) // THnSparse must be manually written to file
  {
    ...
    mnsigmaE_pT_all[t][1]->Write(); // zyj add
...
Int_t StPicoElecPurityMaker::FillHistograms(Int_t trig, StPicoEvent* event)
{
  ...
  for(int i=0; i<numberoftracks; i++){
	...
	if(fabs(nsigk)<2)
	  {
	    ...
	    mnsigmaK_diff[trig][1] ->Fill(mpt,nsigk-nsige );
	  }
	if(fabs(nsige)<2)
	  {
    ...
    if((trig>0)&&passBEMCCuts(event, track, trig))
      {   	
    ...
    if(trig==0&&passTOFCuts(event, track, trig)&&passBEMCCuts_noTrigger(event,track,trig))   //    continue;
      {
