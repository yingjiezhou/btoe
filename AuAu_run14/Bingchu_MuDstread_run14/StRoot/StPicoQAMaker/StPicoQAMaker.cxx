...
Int_t StPicoQAMaker::Make() {
   ...
   if(fillhistflag){	
      ...
      map<Int_t, Int_t>::iterator iter = mTotalRunId.find(RUNID); // 要保证既不是Badrun 也是属于total run 里面的run
      if(iter != mTotalRunId.end())
         runIndex = iter->second; //从0 开始，第77个run
      else{
         runIndex = -1;
   ...
   if(fillhistflag){
      ...
      */
      ...
      if( Ispasseventcuts(event) ){ // 下面这些量都是对track 都进行循环，每次相加得到,比如说对应这个event 有多少条track 是match 到 tof 的。 每个event 不一样
