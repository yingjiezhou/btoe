...
{
  memset(mAnaTreeAllArrays, 0, sizeof(void*)*__NANATREEARRAYS__); // 令其为0，清空先
...
void StPicoAnaTreeMaker::createArrays() {
  for ( int i=0; i<__NANATREEARRAYS__; i++) {
    clonesArray(mAnaTreeAllArrays[i],StAnaTreeArrays::anaTreeArrayTypes[i],
                StAnaTreeArrays::anaTreeArraySizes[i],StAnaTreeArrays::anaTreeArrayCounters[i]);
  }
  mAnaTree->set(this);//zyj
}
//-----------------------------------------------------------------------
...
//======================================================================================================
  ...
  if(f.IsOpen()&&!mCalcRecenter)
  {
    ...
    TProfile2D *temp_cos_farwest = (TProfile2D*)f.Get("etapluszplusQx"); //这个是读取的,和自己建立的同名字的直方图没关系
...
Int_t StPicoAnaTreeMaker::Make() {
  ...
  for(int i=0;i<__NANATREEARRAYS__;i++){
    if(!mAnaTreeAllArrays[i]) continue;// 也可以换成mAnaTreeArrays[i]?
...
Int_t StPicoAnaTreeMaker::MakeWrite() {
  ...
  if(passEvent(mPicoDst)){
    if(mCalcRecenter) fillRecenterCor(); // false
    else{
      fillEventHeader();
      fillTracks();
      fillPairs();
      fillEmcTrigger();
      if(mTriggerSelection==mtd) fillMtdTrigger(); // we only use ht and mb trigger, so here is false
      //int counts = mAnaTreeArrays[anaTreeETrack]->GetEntries() + mAnaTreeArrays[anaTreeMuTrack]->GetEntries() + mAnaTreeArrays[anaTreePartETrack]->GetEntries();
      mTTree->Fill();
    }
  }
...
bool StPicoAnaTreeMaker::passEvent(StPicoDst *pico){
  ...
  if(trigFound){
    for(int i=0;i<32;i++){
      if(mTriggerWord>>i & 0x1) mhnEvents->Fill(4+i);//mTriggerWord一直向右移，位与运算
    }
...
void StPicoAnaTreeMaker::fillRecenterCor() {
  ...
  for(int n=0;n<nTracks;n++){
    ...
    if(eta > 0.0 && vertZ > 0.0){
      etapluszplusQx->Fill(runIndex, centrality, cosCor); // 这个是填的，
...
void StPicoAnaTreeMaker::fillTracks() {
  ...
  for(int i=0;i<nTracks;i++) {
    ...
    //partener e
    if(isPartE(t)){ // 不满足上面更松的sigma cut 条件，还是可以到这一步，存为partner, 如果满足上面的条件，肯定也满足下面的条件，所以？？
...
void StPicoAnaTreeMaker::fillPairs() {
  ...
  int nPartElec = partEidx.size(); // 就是上一个函数fillTracks() 里面存的partner 的个数
  int index = 0;
...
void StPicoAnaTreeMaker::fillEmcTrigger() {
  for(UInt_t i=0;i<mPicoDst->numberOfEmcTriggers();i++){
    ...
    int flag = emc->flag();
    int trgId = emc->id(); //??trigger id to same with track tower id
    int adc = emc->adc(); //dsm adc
    int adc0 = 0;// will get later from track
    int eId = -1;
  ...
  for(UInt_t i = 0;i<mAnaTreeArrays[anaTreeEmcTrigger]->GetEntries();i++){
    ...
    int trgId = emcTrg->id();// already know
    int nElec = mAnaTree->numberOfETracks();
    for(int j=0;j<nElec;j++){ // loop for all track, to map with emctrigger id
      ...
      if(trkEmcId==trgId){// why trigger id need to same with track tower id
        int adc0 = eTrk->adc0(); // only StElectronTrack can get this adc0 directly
        eTrk->setEmcTriggerId(i); //new
        int adc  = emcTrg->adc();
        emcTrg->setEId(j);
        emcTrg->setAdc0(adc0);
...
Bool_t StPicoAnaTreeMaker::passPhoEEPair(StPicoTrack *t1, StElectronTrack *t2, Int_t index1, Int_t index2, Int_t idx1InPico)
{
  ...
  if(id1==id2) return false; //  因为都是从0 开始，而且所以相比于上面的EEpair ，这里有可能是会一样的，需要判定, b
...
Int_t StPicoAnaTreeMaker::makeTriggerWord(StPicoEvent* ev){
  ...
  for(auto trg = triggers.begin(); trg < triggers.end(); ++trg)
  {
    ...
    if(ev->isTrigger(*trg))
      mTriggerWord |= (1U << bitCount);// mTriggerWord= mTriggerWord | (1U << bitCount) 首先通过了肯定是1，还可以看是通过哪个trigger
    bitCount++;
  }
  return mTriggerWord;
}
