...
void StNpeAnaMaker::declareHistograms() {
  ...
  hnEvents_GoodEvents = new TH1F("hnEvents_GoodEvents","hnEvents_GoodEvents",10,0,10);
...
Int_t StNpeAnaMaker::Make() {
  ...
  hnEvents->Fill(1.5); // counting the total number of events before bad run reject,因为虽然是bad run，上一步还是会产生AnaTree root file，但是是空的
  ...
  // 之前是bad run 的，产生的AnaTree 都是空的，event 数目为0，所以这里不会对event 进行减少
  hnEvents->Fill(2.5); // counting the total number of events after bad run reject
  Int_t bTrg=-999;;
  if(pass_MB_trigger_selection(mEvent,bTrg)) //isMinBias(mEvent)
    {
      ...
      if(isGoodEvent(mEvent,bTrg)&&MB_flag)  //for event trigger selection, 减了一点点事件,在Ana 里面更松：treeMaker->setVzCut(-8,8); treeMaker->setVzDiffCut(-4,4);
	{
	  hnEvents_GoodEvents->Fill(4.5);
	  hist->mh2hVzVsVpdVz[bTrg]->Fill(mEvent->primaryVertex().z(),mEvent->vzVpd()); // after cut
...
void StNpeAnaMaker::Fill_MB_Trigger_histogram(StAnaTree *mAnaTree,int bTrg)
{
  for(unsigned int i=0;i<mAnaTree->numberOfETracks();i++)
	...
	{
	  Fill_MB_Trigger_inclusive_histogram_without_HFT(eTrk,bTrg);  // MB inclusive e DCA
	  if(eTrk->isHFTTrack())
...
void StNpeAnaMaker::Fill_HT_Trigger_histogram(StAnaTree *mAnaTree,int bTrg)
{
 ...
 //inclusive e
    ...
    if(
       ...
       &&pass_cut_GoodTrack(eTrk)
       &&pass_cut_nsigmaE(eTrk)
       // &&pass_cut_Match_BSMD(eTrk) // MB has this cut,
       // &&pass_cut_BSMD(eTrk) //no need here, because the BSMD cut has no significant improvement for the electron purity but lost almost 50% statistics
  ...
  for(unsigned int ipair=0; ipair< mAnaTree->numberOfPhoEEPairs();ipair++){	
    ...
    //PHE partner e no need this cut, but track 1 is yes 
  ...
  if(eTrk1->charge()!=eTrk2->charge())
    {
      ...
      hist->mh2Poe_unlike[bTrg]->Fill(eTrk1->gMom().perp(),eTrk1->gMom().mag()/eTrk1->e());
    }
  if(eTrk1->charge()==eTrk2->charge())
    {
      ...
      hist->mh2Poe_like[bTrg]->Fill(eTrk1->gMom().perp(),eTrk1->gMom().mag()/eTrk1->e());
    }
}
////////////////////////////////////////////////////
  ...
  if(eTrk1->charge()!=eTrk2->charge())
    {
      ...
      hist->mh2Dz_HFT_unlike[bTrg]->Fill(eTrk1->zDist(),eTrk1->gMom().perp());
      hist->mh2Dphi_HFT_unlike[bTrg]->Fill(eTrk1->phiDist(),eTrk1->gMom().perp());
      hist->mh2Poe_HFT_unlike[bTrg]->Fill(eTrk1->gMom().perp(),eTrk1->gMom().mag()/eTrk1->e());
    }
  if(eTrk1->charge()==eTrk2->charge())
    {
      hist->mh3InvMassVsPtVsPairdca_HFT_like[bTrg]->Fill(PairMass,eTrk1->gMom().perp(),PairDca);
      ...
      hist->mh2Dz_HFT_like[bTrg]->Fill(eTrk1->zDist(),eTrk1->gMom().perp());
      hist->mh2Dphi_HFT_like[bTrg]->Fill(eTrk1->phiDist(),eTrk1->gMom().perp());
      hist->mh2Poe_HFT_like[bTrg]->Fill(eTrk1->gMom().perp(),eTrk1->gMom().mag()/eTrk1->e());
    }
}
...
void StNpeAnaMaker::Fill_MB_Trigger_inclusive_histogram_with_HFT(StElectronTrack* eTrk,int bTrg)
{
  ...
  if(eTrk->gMom().pseudoRapidity()>0)
  hist->mh3pT_dcaXY_Phi_HFT_EtaPlus[bTrg]->Fill(eTrk->dcaXY(),eTrk->gMom().perp(),eTrk->gMom().phi());
  if(eTrk->gMom().pseudoRapidity()<0)
  hist->mh3pT_dcaXY_Phi_HFT_EtaMinus[bTrg]->Fill(eTrk->dcaXY(),eTrk->gMom().perp(),eTrk->gMom().phi());
  
  if(eTrk->charge()<0)
  hist->mh2dcaXYVsPt_HFT_Electron[bTrg]->Fill(eTrk->dcaXY(),eTrk->gMom().perp());
  if(eTrk->charge()>0)
  hist->mh2dcaXYVsPt_HFT_Positron[bTrg]->Fill(eTrk->dcaXY(),eTrk->gMom().perp()); 
...
{
  if(!evt) return kFALSE;
  if(isBHT1(evt)&&!isBHT2(evt)&&!isMinBias(evt)) 
    {
      bTrg = 1;
      return kTRUE;
    }
...
{
  if(!evt) return kFALSE;
  if(isBHT3(evt)&&!isBHT1(evt)&&!isBHT2(evt)&&!isMinBias(evt)) 
    {  
      bTrg = 3;
      return kTRUE;
    } 
...
{
  Float_t Pt=trk->gMom().perp(); // StElectronTrack 继承了StPicoTrack的性质
