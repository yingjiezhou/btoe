  ...
  mIoMode(mode), mOutputFile(0), mOutputHistFile(0), mChain(0), mTTree(0), mSplit(99), mCompression(9), mBufferSize(65536*4), mSaveHadron(false)
{
  ...
  streamerOff();
  zeroArrays();
  createArrays(); //mAnaTree->set(this);// 都是初始化
  ...
  mDoEvtPlane = kTRUE;
  mRunList = "/star/data01/pwg/xiao00/backup/Run14/run14_data/runNumberList_run14AuAu200";
  mRecenterFile = "/star/data01/pwg/xiao00/backup/Run14/run14_data/recenter_correction.root";
...
//-----------------------------------------------------------------------
void StPicoAnaTreeMaker::setBranchAddresses(TChain* chain) { // only use in kRead mode
  if(!chain) return;
  chain->SetBranchStatus("*",0);// 初始化，都是0
  TString ts;
  for ( int i=0; i<__NANATREEARRAYS__; i++) {
...
void StPicoAnaTreeMaker::openRead() {
  ...
  if (dirFile.find(".list")!=string::npos)  {  // for all root file
  ...
  } else if (dirFile.find(".anaTree.root")!=string::npos)  { // for one root file
    mChain->Add(dirFile.c_str());
  } else {
    LOG_WARN << " No good input file to read ... " << endm;
  }
  if(mChain) {
    setBranchAddresses(mChain); // 因为这个函数，给了mAnaTreeAllArrays 这些array值，自然下一步的mAnaTree也就有了值，这是个静态函数
    mAnaTree->set(this); //就因为里面拿了StPicoAnaTreeMaker 所以这里就可以拿到mAnaTree？
  }
  return;
}
//-----------------------------------------------------------------------
void StPicoAnaTreeMaker::openWrite() {
  ...
  if (mSplit) bufsize /= 4;
  mTTree = new TTree("anaTree","StAnaTree",mSplit); // new tree, will fill anaTree Branch
  ...
  } // kWrite 模式是把mAnaTreeAllArrays 这些填上去，这样就存了mAnaTree,以便于kRead模式的时候用
...
Int_t StPicoAnaTreeMaker::MakeWrite() {
  ...
  if(passEvent(mPicoDst)){
    if(mCalcRecenter) fillRecenterCor();
      ...
      fillEventHeader();
      fillTracks();
      fillPairs(); // after fillTracks() we can use this , fillTracks() 这个函数，已经将track 挑选出来了，所以这里就可以配对了，可以看StAnaTree 这个类，是static 的，
      fillEmcTrigger();
      if(mTriggerSelection==mtd) fillMtdTrigger();
      //int counts = mAnaTreeArrays[anaTreeETrack]->GetEntries() + mAnaTreeArrays[anaTreeMuTrack]->GetEntries() + mAnaTreeArrays[anaTreePartETrack]->GetEntries();
      mTTree->Fill(); // fill the new PicoAnaTree , contain anaTree Branch
    }
  }
  return kStOK;
}
  ...
  for(int i=0;i<nTracks;i++) {
    ...
    if(mSaveHadron){
      if(isHadron(t)){ 
        ...
        new((*(mAnaTreeArrays[anaTreeHTrack]))[counter]) StHadronTrack(mPicoDst,t,i);// 在StNpeAnaMaker 中HT trigger里面会用到这个hadron
      }
    }
  }
}
...
Bool_t StPicoAnaTreeMaker::isGoodTrack(StPicoTrack* t)
{
  ...
  // 上面是取得track 的各个变量，下面开始对track 进行筛选了，判断是不是好的track
